...
0. **What writes a HERO's attribute record?** The newly-named gate, and the one thing
   standing between us and a working hero. **The `0x0074`-chunk hypothesis is already tested
   and REFUTED** (§12) — do not re-run it. `attribState` is a separate `0x43c`-stride record
   holding `attrib[51]` of 20 bytes, and nothing in `0x0074` reaches it. **Go at the TEMPLATE
   system next**: `AcctTemplate:422/423/440`, `TemplatesCode:168`, `TemplatesHelpers:368` all
   bound a struct with `attribCount` + `attrib[]` + `attribValue[]` (rank cap 12, count cap
   16) — that struct is a build, and those asserts are the way in. The still-missing piece is
   whatever INSERTS an attribState entry; its only out-of-module stride site is generic
   `Array.cpp` growth, so it is a vector push with no message traced into it yet.
...
3. **The hero route's remaining blocker** is skill-bar/attribute delivery (item 0 above).
   The other two are closed. `0x0074`'s other 17 fields are still unexplained.
