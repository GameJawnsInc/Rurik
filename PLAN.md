...
* **DESKWORK-D1**: steps 1–8 landed (PLAN-LOG) — the bare-machine c2s
  ...
  **CONFIRM-2 ran 2026-09-24** (studies/deskwork/CONFIRM-2026-09-24.md, PLAN-LOG): the
  ...
  picture. **Open**: **the field shield's client run** — the FIELD's player `0x006D` is now
  withheld as retail's wire does (0 of 44; PLAN-LOG "the field shield", `--field-player-weapons`
  the KNOWN-BAD arm); that the sword-and-shield body keeps its shield at load, swings, and draws
  no assert is owed to three launches (a load, an attack, the flag's A/B); the henchman kick, c2s `0x00A8 [agent]`, named on our client, unarmed,
...
* **SANDBOX, two residues of the hostile-cap lift** (2026-09-24; PLAN-LOG "R-SANDBOX, hostile
  level and rank caps lifted", "Two things NOT done"): (a) **no server-load guard in
  `area_population` for a content row past 255** — a hand-written `[spawn.*]` row at 256
  bypasses the compiler and first fails inside `send()` at spawn time, dropping the
  connection; the recon asked for a load-time refusal, and it is `authsrv.py`'s arc (its
  population tests read that file). (b) **the level-0 quirk** — `agent_level` treats 0 as
  None, so a level-0 hostile armours at 0 but strikes as 20 (the fallback 60); level 0 is a
  real retail value (map 146's defs 1428 / 1433 / 1434 / 1442) and the window offers it;
  which arm is right wants a retail witness of a level-0 foe's damage, not a guess.
