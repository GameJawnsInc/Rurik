...
3. ~~**The hero route's remaining blocker**~~ **CLOSED — the hero is authored end to end**
   ...
   **refuted** (§16) — do not re-try it. The live question is desk work: `0x00524C40` is a
   get-or-create for the commander container at `ctx+0x20` while `0x00524DB0` (what the click
   uses) only looks up; find whether the `activeHeroes` party scan skips our hero or
   registers it under a different key. `--hero-ai-mode` is wired but **untested**.
