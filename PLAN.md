...
3. **The hero route is untouched** (§7.2) and its blockers stand: which `0x01C2` `u16` is
   the agent id, what creates the `charHeroData` record, and skill-bar delivery (NOT FOUND).
   The trailing-`0x0072` diagnostic is still the cheapest first move.
