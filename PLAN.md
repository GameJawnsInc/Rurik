...
0. **What writes a HERO's attribute record?** The newly-named gate, and the one thing
   standing between us and a working hero. Test `0x0074`'s two unexplained 5-dword groups
   (record `+0x4c`/`+0x60`) first — 20 bytes is the right size for an attribute block, and
   the arm is cheap: vary the chunk, re-send the trailing `0x0072`, watch which assert fires.
