...
### Combat — steps 0-9 landed, and the two remaining items are BOTH one caged run (2026-08-15)
...
changed is in R4a and R4b above; what is LEFT is small, specific, and needs the
client:

0. **NEW, 2026-08-15, and it comes first because it is a crash: verify the
   `0x003A` fix.** The message was going out INTERLEAVED against a wire that is
   COLUMN-MAJOR, and it asserted the client dead on `CharData.cpp:202` in every
   session that sent it — including two instrumented runs reported healthy,
   because the assert box is modal *inside* the client and liveness polling
   reports ALIVE. The fix is in and green offline (`studies/combat/PLAN.md`
   §14) and **the client has not been run since**, so the cure is UNVERIFIED.
   One ordinary harness session, watched with `crashwatch.ps1 <pid> 300`
   rather than `Get-Process`, settles it — and it is the same session item 1
   needs, so run them together.
1. **`--probe attributes`** — L6's attributability criterion. The wire half is
   done and tested (`0x003A` carries real ranks), but *does the panel show
   rank R* is unverified, and per H5 no tool in this repo can read the HUD. The
   probe is written for it and its **step 3 reverses the five ranks across the
   same names**, which is the discriminator: a panel that does not follow means
   step 2 passing was a coincidence. The ranks are deliberately distinct
   (12/9/6/3/1) so a mis-ordered column shows the right numbers against the
   wrong names — and note that a mis-ordered PAYLOAD never gets that far, per
   item 0: it asserts the client dead before the panel is reached.
2. **The cast-lifecycle loopback acceptance** — `0x00E4→0x00E5→0x00E3→0x00E6`
   is offline-tested against six live cycles, but has never been played at a
   client. Whether the recharge sweep and the cast animation actually render is
   the open half.
...
installed. `391e8ca` made the harness pick by BUILD and NAME.
