...
0m. **THE MONSTER-AI DIVE LANDED, and it leaves four desk tasks that need NO capture,
    ...
    1. **Read the immediate operand at `0x0080dfae`** (`ChCliApi:4331`, `mode <
       CHAR_AI_MODES`). One capstone pass. `CHAR_AI_MODES` is the *only* named
       server-side-shaped AI concept anywhere in the shipped image, in a subsystem
       whose `Srv` half was not shipped, and it reaches a wire-shaped boundary in
       two format strings that pair an `aiMode` with an `agent`. **If the
       cardinality is 3 and matches the compass Guard/Fight/Avoid-Combat stances,
       the lead closes as UI** and the binary negative becomes total. It is the
       cheapest open question in the whole study and the rival is NOT refuted.
