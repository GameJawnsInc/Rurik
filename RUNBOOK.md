...
| Pressing **M** (world map) kills the client on `Assertion: worldMapDims.x == mapDims.x * DXT_BLOCK_SIZE` (`GmMapView.cpp:1731`) within ~2 s, and the crash dialog then **takes focus, so every later scripted key goes to the dialog** — the harness still logs each press `sent` and can score the run PASS | **A known server gap, not archive state and not damage**: the world map sizes its texture from `charContext+0x5B4`, which only the fog-of-war init pair `0x008B`+`0x008A` writes, and the plain server flow never sends it — so `mapDims` is 0 and the compare dies. Proven by a matched pair (`studies/minimap/FINDINGS.md` §6f.2: same archive, pair sent → the map opens); reproduced on 38797 2026-08-24, `vault/captures/harness/20260824T065948/crash-dialog.txt` | Do not script `key:m` in a plain run, and never ahead of arms you still need — one M press voided a whole probe run (`studies/playercomposite/FINDINGS.md` §9.22). A run that needs the world map sends the pair first: `probes.py` `compass_fog_nomark` (init only) or `compass_fog_mark`. The durable fix — the server sending the pair at map entry — is not built; note the probe payload is ArenaNet's continent-1 RLE replayed verbatim, so a general fix synthesizes an all-fogged payload per continent from rung S8's closed format |
