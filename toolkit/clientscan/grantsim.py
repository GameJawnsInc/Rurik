...
`0x0046E870` (nine instructions, a 256-dword lookup table at `LUT_VA`, which
is in `tablesqrt.py` now with both scans and is re-exported here).
...
# =========================================================================
# REALFIX-C0 -- the decision radius and gate 1's cut: `tablesqrt.py`
# =========================================================================
#
# The client's table sqrt, the two exhaustive boundary scans it makes possible
# and the constants they answer with are in `toolkit/clientscan/tablesqrt.py`
# now -- banner, derivation and `LUT_VA` together, because `LUT_VA` is this
# tree's only class-(a) build pin for that table and a pin belongs in the file
# that reads it. Re-exported HERE, at the site they were cut from, so nothing
# that already says `grantsim.<name>` had to change:
#   - THIS FILE reads eight of them in code that stays -- `MATCH_RADIUS` (five
#     sites, among them `LEAD_FIXED`'s definition and `score()`'s `radius=`),
#     `GATE1_CUT` (two), and `LUT_VA`, `MATCH_SCAN`, `GATE1_SCAN`,
#     `GATE1_NOMINAL`, `derive_match_radius()`, `derive_gate1_cut()` in
#     `print_radius()`, whose `_say` lines print `hex(LUT_VA)`.
#   - `test_grantsim.py` §C0 reads three more as `GS.` attributes --
#     `read_sqrt_lut`, `MATCH_BOUNDARY_DISTSQ`, `GATE1_BOUNDARY_DISTSQ` -- and
#     re-runs both scans against the pinned image rather than trusting the
#     constants. Nothing else in the tree reads any of the fourteen.
# The three public names with no reader outside the leaf today -- `MATCH_NOMINAL`,
# `approx_sqrt`, `approx_sqrt_bits` -- are re-exported anyway, for the reason
# the `field4screen` block below gives: dropping a public name from
# `grantsim`'s namespace would be a rename wearing a move's clothes. `_bits`,
# `_float`, `_scan_boundary` and `_LUT_CACHE` stay PRIVATE to `tablesqrt.py`.
# `_LUT_CACHE` in particular is a MUTABLE MEMO and no test touches it, so it is
# deliberately not re-exported: there is one cache, in the module that fills it.
from tablesqrt import (                                        # noqa: F401,E402
    LUT_VA, MATCH_SCAN, GATE1_SCAN, MATCH_NOMINAL, GATE1_NOMINAL,
    MATCH_RADIUS, GATE1_CUT, MATCH_BOUNDARY_DISTSQ, GATE1_BOUNDARY_DISTSQ,
    read_sqrt_lut, approx_sqrt_bits, approx_sqrt,
    derive_match_radius, derive_gate1_cut)
...
# REALFIX-F1b's FIELD-4 POLICY PRE-SCREEN lives in `field4screen.py` now. The
# banner that argues for the instrument, the eleven `FIELD4_*` constants and the
# fifteen public functions moved there together and verbatim; `_nearest_tap` went
# with them and is deliberately NOT re-exported, because it is private to the
# screen and nothing outside it has ever named it.
#
# Re-exported HERE, at the site they were cut from, because three readers reach
# them by these names and every one of them would break on a qualified call.
# Counted off the AST, not from memory:
#   - THIS FILE: `print_planecarry()` below, and nothing else in it, calls
#     `field4_census(` and `field4_race(` and reads `FIELD4_POLICIES`,
#     `FIELD4_WORD_AGREEMENT` (3x), `FIELD4_PAIR_GAP` and `FIELD4_RESIDUAL_NOTE`
#     -- all by the bare name.
#   - `test_grantsim.py` §10 reads ten off the module: `GS.FIELD4_SCREEN` (11
#     sites), `GS.FIELD4_POLICIES` (4), `GS.FIELD4_PAIR_GAP` (5),
#     `GS.FIELD4_MEASURED` (3), `GS.FIELD4_F1B_EXPECTED` (3),
#     `GS.FIELD4_ARRIVAL_GATE` (2), `GS.FIELD4_PAIRS` (2),
#     `GS.FIELD4_PUBLISHED_NEAREST` (2), `GS.field4_replay` (2),
#     `GS.field4_race` (1) -- plus `GS.print_planecarry`, which stays here.
#   - `test_position_trust.py` §16 reads two, `grantsim.FIELD4_SCREEN` and
#     `grantsim.FIELD4_PAIRS`, to rebuild the server's banner rows. `from X
#     import Y` binds the SAME object, so the dict and the tuple it walks are
#     the ones `field4screen.py` defines and the rebuild stays byte-identical.
# The other thirteen names have no reader outside the leaf today. They are
# re-exported anyway: this is a pure refactor, and dropping a public name from
# `grantsim`'s namespace would be a rename wearing a move's clothes.
from field4screen import (                                     # noqa: F401,E402
    FIELD4_PAIRS, FIELD4_PUBLISHED_NEAREST, FIELD4_MEASURED,
    FIELD4_RESIDUAL_NOTE, FIELD4_F1B_EXPECTED, FIELD4_SCREEN, FIELD4_PAIR_GAP,
    FIELD4_ATTRIB_GAP, FIELD4_ARRIVAL_GATE, FIELD4_WORD_AGREEMENT,
    FIELD4_POLICIES, field4_grants, field4_taps, field4_observed,
    policy_shipped_zerolead, policy_f1_planecarry, policy_f1b_arrivalcarry,
    field4_replay, field4_seed, field4_arrival_check, field4_race,
    field4_word_check, field4_client_writes, field4_anchored, field4_capture,
    field4_census)
