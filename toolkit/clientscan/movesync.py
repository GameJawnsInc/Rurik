...
# --- the fence on 0x006055E0: MOVED to `movefence.py` -----------------------
#
# The whole fence section -- the `gate_reach` classifier, the three-way jump
# tally, the appender witness and the three read-back parsers -- was cut from
# HERE to `toolkit/clientscan/movefence.py` on 2026-09-11, verbatim and with its
# constants, because its consumers rewrite `UNREAD_REFUSE_SHARE`,
# `APPEND_DEDUP_CHAIN` and `APPEND_TARGET` through `globals()` and a constant
# left on this side of the split would have gone unmutated with nothing in the
# tree able to see it (`srclint.py:18`).
#
# Re-exported, not re-implemented: `selftest()` and `main()` below call
# `print_fence` by its bare name, `probedoc_fixtures.py` reads `FENCE_KEY`,
# `classify_reach`, `print_jump_tally`, `print_appender_witness` and
# `print_fence` off THIS module, and `test_probedoc.py` §3 pins the three
# printers' signatures and §4 asserts `hasattr(movesync, ...)` on the witness.
# `state_fields` and `REACH_ALIASES` are deliberately NOT re-exported: nothing
# executable reads them from here (both of their greps are docstring prose), and
# a name `test_movesync.py` patches on `movefence` must not also answer here.
#
# THESE ARE READ-ONLY ALIASES. `classify_reach` and `print_appender_witness`
# exist in both modules; the sections that call them live in `movefence`, so a
# test that wants to break one patches `movefence`, never this binding. Never
# assign through the names below.
from movefence import (  # noqa: F401,E402
    FENCE_KEY, classify_reach, print_jump_tally, print_appender_witness,
    print_fence)
...
# --- the fence selftests: MOVED to `movefence.py` ---------------------------
#
# `_rec`, `_fake_pair`, `_line_with`, `_floor` and the four `_selftest_*`
# sections C4/C5/C9 went with the code they bind, in the same 2026-09-11 move as
# the section above. Re-exported here because `selftest()` below runs the four
# by bare name and builds its fence fixtures with `_rec`, and because
# `probedoc_fixtures.py` builds its FIX-10..FIX-13 samples with
# `movesync._rec`. `test_movesync.py` §19 wraps the four through `movesync` and
# §20 asks BOTH modules what sections exist, so a fifth section added to
# `movefence.py` and left unwrapped still goes red there.
#
# READ-ONLY ALIASES, as above: `_selftest_spellings` is patched on `movesync`
# by §19's whole-selftest control (the summation it is about is in `selftest()`
# below), and the four sections' own controls patch `movefence`.
from movefence import (  # noqa: F401,E402
    _rec, _selftest_jump_tally, _selftest_appender_witness,
...
# --- 2026-09-14: the server's own hard sets of the player (the JARIN shrine) --
# Appended at the END on purpose: studies/movement/PROBE-GATEFIRE.md cites this
# file by line number (test_citelint / test_probedoc), and a block inserted
# above `steps` moved every citation below it by 77 lines.
...
def hard_sets(s2c, player_of):
    ...
    the wire. `mark_server_sets` flags the row and the callers that build a
    verdict population skip a flagged row (resyncscore.retail_tracks's
    hard_idx, test_movesync sec.16) -- the row is KEPT, aligned with its
    reports, and counted; nothing is dropped. `hard_step` itself is
    untouched, so the line numbers PROBE-GATEFIRE.md cites hold. What this
