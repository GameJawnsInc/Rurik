...
def rows():
    ...
    # ChCliBase fields, a SEPARATE table from agent_offset on purpose -- they
    # belong to a different object and are emitted as C_* rather than A_*. See
    # the [char_offset] block's header comment in content/movecode.toml.
    coffs = tables.get("char_offset") or {}
    if not sites:
...
def emit(sites, offs, coffs, exe_path):
    out = []
    a = out.append
    ...
    a("    int           deref_char_gate;/* ecx is a ChCliBase: read the two WALK GATE")
    a("                                     words, +0x64 flags and +0x10C m_status. */")
    ...
    a(" * deref_char_gate, stride, deref_out, deref_out_path, shape */")
    ...
    for n in names:
        ...
        a(f"    {{ 0x{r['rva']:08X}u, \"{n}\", "
          ...
          f"{1 if r.get('deref_char_gate') else 0}, "
    ...
    for n in sorted(offs):
        ...
        a(f"#define A_{n.upper():<20} 0x{r['offset']:02X}u"
          f"   /* {r.get('name','')} */")
    a("")
    a("/* ChCliBase offsets -- a DIFFERENT object from the agent above, which is why")
    a(" * these are C_ and those are A_. Confusing the two is the `+0xC4` mistake. */")
    for n in sorted(coffs):
        r = coffs[n]
        a(f"#define C_{n.upper():<20} 0x{r['offset']:03X}u"
          f"   /* {r.get('name','')} */")
    a("")
...
def main():
    ...
    if args.check:
        ...
        return 0
    text = emit(sites, offs, coffs, path)
