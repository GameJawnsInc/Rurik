...
def skill_name_strings(profession=8, exe=None, dat=None, first=FIRST_FREE_RECORD):
    """{record index: name} for one profession's whole roster, in skill-id order.

    The record ORDER is skill id ascending, so a record index maps back to a skill
    deterministically -- `reskin.py` has to write the matching string id into each
    skill's row+0x98, and the two sides agree only because both sort the same way.
    """
    ...
    return {first + n: named[sid] for n, sid in enumerate(sorted(named))}
