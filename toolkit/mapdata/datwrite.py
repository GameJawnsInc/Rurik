...
# The C-6 oracle -- `looks_compressed`, `declaration_fault` and the
# compression-8 prologue byte -- lives in `datdecl.py` since 2026-09-11, and
# `import gwdat` (with the six-line comment that justified it) went with them.
# Re-exported here under their own names: `Writer.replace` below calls
# `declaration_fault(...)` and `looks_compressed(...)` as bare names and `main()`
# calls `declaration_fault(...)` for `--overwrite`; `datalloc.py`, `datmove.py`
# and `overlay.py` read them off this module as `datwrite.declaration_fault` /
# `datwrite.looks_compressed`; `test_datwrite.py:1026,1809` reads
# `datwrite.HUFFMAN_PROLOGUE_BYTE`. `COMPRESSION_CODES` and
# `HUFFMAN_RETAIL_MARKER` moved with them and are deliberately NOT re-exported --
# measured tree-wide 2026-09-11, neither has a reader outside `datdecl.py`.
#
