...
fail this repository forbids.
  ...
  FRAMING PROVEN, CONTENT OPAQUE -- carried verbatim:

    tag 7   `(dx//32)*(dy//32)` blocks of `{u32 k, u8 payload[k], u8 tail[128]}`.
            The block WALK is a genuine refutable check and it passes 59,051 of
            59,051. **A round-trip over tag 7 proves the block walk and says
            nothing whatever about shadows** -- which is why the contents are
            still carried verbatim here. They are no longer opaque: see
            `trnshadow.py`, which decodes `payload` as a 272x272 one-bit shadow
            bitmap run-coded a row at a time, re-encodes it to ArenaNet's own
            bytes, and derives the 128-byte tail from it. The tail is data, not
            padding: 46,383 distinct values corpus-wide (8,313 blocks all-zero,
            605 all-0xFF).
...
class ShadowBlock:
    ...
    Both members are OPAQUE. The block walk is proven (59,051/59,051 blocks
    close on their record's declared end, with the block count derived from
    dims and not read from the file); the contents are not decoded and are
    carried verbatim. `k` is never zero in the corpus and ranges 544..13,332
    over 4,546 distinct values. The 128-byte tail takes 46,383 distinct values
    corpus-wide, so it is data rather than padding.

    The client's own assert names the destination `TrnMap::m_shadowData`. The
    onward link to `TrnCodecShadow:245` rests on filename adjacency only and is
    UNVERIFIED -- do not repeat it as source-code.
    """
    ...
    def __len__(self):
        return BLOCK_K.size + len(self.payload) + len(self.tail)

    def __eq__(self, other):
        return (isinstance(other, ShadowBlock)
                and self.payload == other.payload and self.tail == other.tail)

    def __repr__(self):
        return f"<ShadowBlock k={len(self.payload)}>"
...
class Terrain:
    ...
    def validate(self):
        ...
        wrong_tail = sum(1 for b in self.shadow if len(b.tail) != SHADOW_TAIL)
        if wrong_tail:
            bad.append(f"{wrong_tail} tag-7 block(s) do not carry a "
                       f"{SHADOW_TAIL}-byte tail")
        return bad
    ...
    def _payload(self, tag, nth):
        ...
        if tag == TAG_SHADE:
            return self.shade
        if tag == TAG_SHADOW:
            return b"".join(BLOCK_K.pack(len(b.payload)) + b.payload + b.tail
    ...
    def blank(cls, dim_x=CHUNK_SIZE, dim_y=CHUNK_SIZE, height=0.0, tiles=3):
        ...
        NOT A CLIENT-LOADABLE MAP, and the reason is worth stating: its tag-7
        blocks carry a zero-length shadow payload, and `k == 0` occurs in 0 of
        59,051 retail blocks. Whether the client's shadow reader accepts that is
        UNVERIFIED and this module makes no claim about it. What `blank()` is
        for is the authoring direction of the codec -- encode from typed values
        that never came out of an archive, decode the result back, and compare
        -- which is the one check in this file that needs no ArenaNet bytes at
        all and runs on a bare machine.
        """
        _gate_dims(dim_x, dim_y)
        if not 1 <= tiles <= 0xFF:
        ...
        return cls(dim_x, dim_y,
                   ...
                   bytes(cells // 4),
                   bytes(cells),
                   [ShadowBlock(b"", bytes(SHADOW_TAIL))
...
def _decode_shadow(blob, body, size, blocks):
    ...
    for i in range(blocks):
        ...
        out.append(ShadowBlock(blob[p:p + k], blob[p + k:p + k + SHADOW_TAIL]))
        p += k + SHADOW_TAIL
    if p != end:
