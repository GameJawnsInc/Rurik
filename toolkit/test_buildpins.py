...
check(len(live) == 151,
      "151 class-(a) occurrences -- the census",
...
check(len(files) == 17, "across 17 files", f"{len(files)}: {sorted(files)}")
...
check(max(collections.Counter(r["file"] for r in live).values()) == 44,
      "and the largest remaining block is movetap.py's 44 (its own 28 before "
      "REALFIX-I1, and msgshape.py's 25 before that)",
