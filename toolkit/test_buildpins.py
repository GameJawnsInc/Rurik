...
check(len(live) == 294,
      ...
      f" **236 on 2026-09-13, +1 and no new file, and the split is MEASURED off the two vault baselines rather than narrated:** updatecheck's before-20260911T190121.json holds 235 rows and before-20260913T211431.json holds 236, keyed on (file, symbol, value), with NOTHING gone and exactly one row arrived -- clientscan/pinned.py, kind build-number, symbol BUILDS, value 38888. That is b46dc9f0 registering build 38888 on 2026-09-13 after the live driver refused the stale 38849 build, and it is the cheapest row the census takes, the same +1 the 38849 registration paid at 189 above: a build NUMBER is a build-coupled constant, derived and asserted against a fresh read of the binary by test_buildid.py, counted because it is real. It was paid by the session that ran the suite next rather than by the one that registered the build, which is the pattern the 204 entry names -- treat this red as a bill -- and this time the bill arrived with company: 38888 is the first build since 38797 whose image GREW (+9,216 B), and the same run found RegisterMsgs, both AgentView allocators and the source-path census all moved on it (test_msgshape / test_avevents / test_srctree, rows measured in the same commit), and test_quests reporting all twelve of its cited quest-log sites drifted and areatable's extent at 897 where four builds said 888 -- which is that arc's bill, left red by name for it rather than paid here."
      f" **294 on 2026-09-14, +58 and a 23rd file, and the split is MEASURED the way the 233 was: a git worktree at 88a0fd1d (the 236 above) scanned with this instrument against this tree, per file.** clientscan/framebus.py 21 -> 74: build 38888 (the first image since the pin whose SIZE changed) moved every quest-handler and completion-family VA, so framebus.TABLES now carries one row per vaulted build -- 38797, 38833, 38849, 38888 -- each MEASURED and each reproducing the recorded frame-bus pairing on its own bytes (test_framebus sec. 3, test_quests sec. 20); 38833 and 38849 got rows because the completion bodies had moved on them unnoticed since August. authsrv/authsrv.py 0 -> 5: MAP_ID_COUNT_BY_BUILD, the manifest sentinel per build (883/888/888/888/897), keyed by build NUMBER -- a build number is itself class (a), the rule buildid.py set in the 64 row -- and guarded at the point of use: test_quests sec. 19 re-derives every row through areatable and sec. 19b through the client's own store of it. The server path gained five integers and no import.")
check(len(files) == 23, "across 23 files", f"{len(files)}: {sorted(files)}")
...
check(max(collections.Counter(r["file"] for r in live).values()) == 74,
      ...
      "since 2026-09-14; movetap.py's 49 before that, 44 at REALFIX-I1's "
...
check((bare, bare_files) == (73, 8),
      ...
      "and a rebase is undetectable in every one. This is a BILL like the 236 "
