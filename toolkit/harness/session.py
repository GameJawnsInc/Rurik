...
# moved to `runargs.py`. Re-exported here BY NAME -- never behind a module
# prefix -- and both halves of that matter: `main` calls nine of these as bare
# Names and `run_client` three (ten distinct), and `test_harness.py` parses THIS
# file's syntax tree requiring the Name `prompts_operator` inside both of them
# and `hold_implies_keep_open` inside `main`, while reaching eleven of the
# fourteen off this module as `session.<name>`. `persist_args` and `hop_aliases`
...
# re-exported here BY NAME -- never behind a module prefix -- because
# `run_client` picks the ladder off `--until` and calls `judge` as bare names,
# and `_play` builds its own login predicate out of `by_any` the same way. No
# other module in the tree reads any of the four; the only mention elsewhere is
# prose in `studies/review/FINDINGS.md`, whose line citation of MAP_CHECKPOINTS
# was already stale before this move.
from runladder import (  # noqa: F401,E402
    LOGIN_CHECKPOINTS, MAP_CHECKPOINTS, by_any, judge)
