...
# Floor 72, measured from a green run on 2026-08-13 (was 64 before section 9's eight).
# Sections 0, 2-7 and 9 need no vault, no socket and no client -- 40 checks -- because
# neither a scoring defect nor a wrong-level plan read is a property of any one capture.
# Two sections do need more and both declare their skips: section 1's cross-check of
# NOT_IN_RECV_TABLE against the client's own receive table wants capstone and the pinned
# client (2), and section 8's rebuild of the observed set wants the live captures (3). A
# machine missing either lands under the floor and goes RED, deliberately, the way
# test_mapexport treats a vault-less run: those two sections are the ones that pin the
# sweep's DENOMINATOR and the ten opcodes that tear the game channel down, and a plan
# built on a constant nothing confirmed is exactly the wish this repo keeps refusing.
LEDGER = checks.Ledger("smsgsweep: the loopback sweep's readout", floor=72)
...
def main():
    ...
    # ---- 9. the probe puts the plan's overrides ON THE WIRE ------------------
    print("\n9. --set reaches the wire")
    ...
    if pr is not None:
        ...
        LEDGER.ok(broken == base_a and [broken] != got,
                  ...
                  f"{broken} -- `p.get('set')` is {{}} against every plan that has ever "
                  f"existed, so apply_set returned its input untouched and the run "
                  f"measured all-zero while the report said otherwise")
