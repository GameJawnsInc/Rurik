...
`codescan.py --dis/--xrefs/--field`, `asserts.py`, `msgshape.py`):
  ...
  * WHO READS THE FLAGS: the drop-down, GmAgentDoll (the paper doll's
    figure) and the roster-blob packer -- and NOTHING in the agent-view or
    composite layers. `codescan --field 0x7C8` over the whole image finds
    exactly the writer, the tester, the getter and three context resets (the
    +0x7C8 hits at 0x006Exxxx are another struct); the AvApi dresser
    0x007DFCE0 has three callers, all ChCliApi message workers. So the WORLD
    model follows the mode only because the SERVER sends the body's visual
    array without the hidden piece: RECONSTRUCTION from that negative, and
    the tapes corroborate the regime pattern -- 0x006E's head slot is 0 on
    756 of 2,245 outpost bodies and 0 of 48 explorable ones, 0x0048's cape
    bit is 0 on 855 of 2,246 outpost bodies and 0 of 48 explorable ones
    (every live connection, vis_census 2026-09-23).
...
CODE_STRING_ID = {0: 0x32C, 1: 0x15BDE, 2: 0x32D, 3: 0x32E, 4: 0x32F, 5: 0x330,
                  6: 0x331}
