...
AND THAT CEILING IS WHY A ROW HAS A REGIME. See `smsgpayload.py`'s regime block:
...
it. Read `smsgpayload.py` before touching `record` or the ledger's key format.
...
# ------------------------------------------------- the regime: smsgpayload.py
# THE REGIME ALGEBRA MOVED, VERBATIM, to `smsgpayload.py` -- the 32-line essay, the four
# regime words, and everything that reads a payload or builds a ledger key. It is
# re-exported HERE, at the site it left, because that is the namespace its readers use:
# `plan`, `record`, `done_opcodes`, `measured_under`, `migrate`, `capture_resolver`,
# `print_report` and `main` below all call these by bare name through this module's
# globals; `probes.py:_smsgsweep_steps` reads `smsgsweep.degenerate` and
# `smsgsweep.apply_set` as attributes of this module; and `test_smsgsweep.py` reads 19
# of them as `sw.<name>`. Explicit names and never `import *`: srclint raises
# `StarImport` and REFUSES TO JUDGE the file, which would drop this one out of
# `test_srclint`'s tree-wide unbound-name coverage without saying so.
from smsgpayload import (                                          # noqa: F401,E402
    ALLZERO, ENCSTRING, OTHER, UNKNOWN, REGIMES,
    apply_set, check_sets, corpus_encstring, degenerate, encodable,
    fills_a_string, ledger_key, parse_key, planned_regime, regime_of_payload,
    regime_of_row, set_type, sets_for,
)
...
# `unmigrated` went with the regime algebra above; re-exported at the site it left,
# where `print_report` and `main` below call it by bare name and `test_smsgsweep.py`
# reads it as `sw.unmigrated`.
from smsgpayload import unmigrated                                 # noqa: F401,E402
