...
def main():
    ...
    # 0x009F HENCHMAN_ADD joined the handled set on 2026-09-23 (DESKWORK-D1 step
    # 5), so it moved from the allowlist row below to this one.
    LEDGER.ok({0x0009, 0x003D, 0x00C1, 0x001F, 0x009F} <= set(seen) & set(game),
              "the busiest retail opcodes, the kick and the henchman add are in "
              "the census AND handled (0x0009, 0x003D, 0x00C1, 0x001F, 0x009F)",
              f"census & arms: {sorted(f'0x{o:04x}' for o in set(seen) & set(game))}")
    LEDGER.ok({0x0008, 0x000B, 0x000D, 0x00B1} <= set(seen) & set(DROPPED_ON_PURPOSE),
              "and the loudest still-dropped of the 2026-09-23 triage are in the "
              "census AND on the allowlist (0x0008, 0x000B, 0x000D, 0x00B1)",
              f"census & allowlist: "
