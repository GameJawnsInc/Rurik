...
def build_parser(*, doc, GAME_SRV_HOST, GAME_SRV_PORT, HOST_FIELD_ENCODING,
    ...
    ap.add_argument("--no-caster-opening", action="store_true",
                    help="DESKWORK-D8 REVERT (step 4): a spell-bar hostile "
                         ...
                         "stands inside its cast range (retail's Elementalist "
                         "4440, 2 of 2), walks ONE 0x0029 leg to range and "
                         "halts when outside it, and never sends a 0x002A. "
                         "The standing Hatcher is a Monk with four spells, so "
                         "`--enemy` alone spawns a caster now.")
