...
def build_parser(*, doc, GAME_SRV_HOST, GAME_SRV_PORT, HOST_FIELD_ENCODING,
    ...
    ap.add_argument("--no-town-weapon-strip", action="store_true",
                    help="THE REVERT ARM for the TOWN WEAPON (DESKWORK-D1, 2026-09-23): "
                         ...
                         "outpost 0x006E never carries a weapon (0 of 2,245 bodies over "
                         "47 live outpost connections, the owner's own 50 loads with a "
                         "weapon in the equipped bag among them) and none of its "
                         ...
                         "0x006D in a town: retail sends the own body none (0 of 47 "
                         "outpost, 0 of 44 field connections), and the client keeps one "
                         "hand store per agent that 0x006D, 0x006E and 0x006F all "
                         "write, so that message re-armed the empty-handed body and "
                         "kept the OLD weapon across F2 -- OBSERVED on our client. This "
    ...
    ap.add_argument("--town-player-weapons", action="store_true",
                    help="THE REVERT ARM for the town weapon's CONFIRM-2 fix (DESKWORK-D1, "
                         ...
                         "a TOWN as well, with the strip otherwise on -- CONFIRM-2's own "
                         "picture (runs 20260924T084418 / 084811): the body armed at load "
                         "from this message two frames after the empty-handed 0x006E, and "
                         "the OLD weapon kept across an outpost F2 while the doll changed. "
                         "KNOWN-BAD: retail never sends the own body a 0x006D (0 of 47 "
                         "outpost, 0 of 44 field connections; the 0x0022 controlled agent "
                         "over every live tape), and the client's one hand store per agent "
                         "(record+0x24, setter 0x0081BE10, three callers: the 0x006D, "
                         "0x006E and 0x006F workers) takes the last writer. The default "
                         "withholds it in a town (send_player_weapons), so the town body is "
                         "bare at load and stays bare across F1-F4, as retail's is; a field "
                         "still sends it (its removal there is a field run's question). "
