...
def build_parser(*, doc, GAME_SRV_HOST, GAME_SRV_PORT, HOST_FIELD_ENCODING,
    ...
    ap.add_argument("--secondary-bits", default=None, metavar="MASK|all|ids",
                    help="OVERRIDE the mask GAME_SMSG 0x00B6 carries in the spawn "
                         ...
                         "reads it wherever 0x0199 field 3 == 0 with >= 2 entries "
                         "-- the whole enable rule BY THE STATIC READ (GmDeckBuilder "
                         "0x00502543); retail used it in map 248, the Great Temple "
                         "(type 13, the PvP hub), and a type-10 outpost such as 148 "
                         "is the runsheet's R1; RUNS.md §13's '15 arena maps only' "
                         "is CONTESTED, studies/profession/SECONDARY.md.")
    ...
    ap.add_argument("--spawn-secondary", type=int, default=None, metavar="N",
                    help="SANDBOX-B4: the SECONDARY profession the spawn burst's "
                         ...
                         "load, the way a stored bar wins over --skills; clear it "
                         "with `charstore.py --account E --character N --secondary 0` "
                         "(the key is removed and this value answers again -- a "
                         "stored 0 used to WIN over it, the fix pass CD-2). "
    ...
    ap.add_argument("--no-hero-add", action="store_true",
                    help="THE REVERT ARM for SANDBOX-N2's add: ignore c2s 0x001E "
                         ...
                         "until --reset-hero-kicks). The default handles it IN THE "
                         "COMMANDER RIG (--party, or --hero-activate without "
                         "--hero-rig-legacy): an owned, kicked hero rejoins with "
                         "that rig's own load messages in its load order -- the "
                         "inventory container re-declared if a kick destroyed it, "
                         "its character block, 0x0072, 0x009A under --hero-char, "
                         "in a field its body, then 0x00B0 with the hero counted "
                         "and a bare 0x01C2 -- and under --persist the stored kick "
                         "clears. In the legacy rig (plain --hero) the add REFUSES "
                         "with nothing sent, because that load holds no hero "
                         "record to activate. RECONSTRUCTION: no tape carries a "
                         "c2s 0x001E, and the client sends it in an OUTPOST only "
                         "(MissionCliGetMap gate). An unowned hero, a hero already "
                         "in the party and an eighth hero are refused with nothing "
                         "sent (retail's refusal reply NOT FOUND).")
    ap.add_argument("--no-hero-unlock-mask", action="store_true",
                    help="THE REVERT ARM for 0x0018: send OpenTyria's eight "
                         ...
                         "per 32 indices with BIT = HERO INDEX (CORROBORATED: 15 "
                         "of the 17 live connections carrying both 0x0018 and "
                         "0x0073 agree; the 2 contrary carry [224] with hero 6 "
                         "alone, so the mask is the ACCOUNT's unlock set) -- "
                         "built from the heroes this run owns as a stand-in for "
                         "the account's, a labelled POLICY. What the client reads "
                         "the mask for is NOT FOUND statically; any effect on the "
                         "hero lists is UNVERIFIED. With no hero authored the "
                         "payload is unchanged either way.")
    ap.add_argument("--party-size-no-heroes", action="store_true",
                    help="THE REVERT ARM for the load's party size: the instance "
                         "load's 0x00B0 PLAYER_PARTY_SIZE counts 1 + henchman and "
                         "leaves the heroes out, as every run before 2026-09-23. "
    ...
    ap.add_argument("--no-item-moves", action="store_true",
                    help="THE REVERT ARM for DESKWORK-D1 step 8: ignore c2s 0x004F "
                         "ITEM_MOVE (an item dragged out of the equipped bag into a "
                         "bag cell) and 0x0030 EQUIP_ITEM (the inventory "
                         "double-click), and read no stored item cell at the dress "
                         "-- every run before 2026-09-23. The default keeps a "
                         "per-connection item store (itemstore.py) seeded by the "
                         "dress, answers a move with retail's 0x014B [key, item, "
                         "bag, slot] (+ 0x006F [agent, slot, 0] in a field; OBSERVED "
                         "4 of 4), an equip into an empty slot with 0x014B + 0x006F "
                         "(4 of 4, a field) and onto an occupied one with 0x0152 "
                         "[key, occupant, item] (4 of 4, an outpost; the visual "
                         "added in a field is RECONSTRUCTION), mirrors a changed "
                         "hand into the server's swing model when the item has a "
    ...
    ap.add_argument("--no-visibility-status", action="store_true",
                    help="THE REVERT ARM for the inventory panel's per-slot DISPLAY "
                         ...
                         "load, ignore c2s 0x0057, dress the body's 0x006E with "
                         "every piece whatever the mode, and pass an equip's 0x006F "
                         "unfiltered -- every run before that day. KNOWN-BAD: the "
                         "client reads an unsent zero, so the "
                         ...
                         "a town -- the doll's own rule), sends an equip's 0x006F "
    ...
    ap.add_argument("--no-town-weapon-strip", action="store_true",
                    help="THE REVERT ARM for the TOWN WEAPON (DESKWORK-D1, 2026-09-23): "
                         "dress the player's world body's 0x006E with the hands "
                         "(visuals 0 and 1) in a TOWN and send a weapon-set switch's "
                         "0x006F there -- every run before that day. KNOWN-BAD: retail's "
                         "outpost 0x006E never carries a weapon (0 of 2,245 messages, "
                         "1,981 bodies, over 47 live outpost connections, the owner's own "
                         "50 loads with a weapon in the equipped bag among them) and none of its "
                         "fourteen outpost hand changes -- four weapon-set switches, "
                         "four double-click weapon equips, an off-hand unequip, five "
                         "PvP-panel placements -- was answered with a hand 0x006F, "
                         "while the field's four switches all were and the own field "
                         "body carries every hand its bag holds (40 of 40 leads, 23 of "
                         "23 off hands); so our town body stood armed where retail's "
                         "is empty-handed. The default leaves visuals 0/1 out of the "
                         "town 0x006E, drops a player 0x006F into visual 0/1 in a "
                         "town (the ONE gate every item batch and the set switch pass) "
                         "AND, since CONFIRM-2 (2026-09-24), withholds the load's player "
                         "0x006D in a town: retail sends the own body none (0 of 46 "
                         "outpost connections with a controlled agent, 0 of 44 field -- "
                         "OBSERVED), and the client keeps one visual-equipment store per "
                         "agent that 0x006D, 0x006E and 0x006F all write, so that message "
                         "is the re-arming of the empty-handed body that kept the "
                         "OLD weapon across F2 (OBSERVED, CONFIRM-2 section 7: withholding "
                         "it was the whole difference on screen). This "
    ...
    ap.add_argument("--town-player-weapons", action="store_true",
                    help="THE REVERT ARM for the town weapon's CONFIRM-2 fix (DESKWORK-D1, "
                         ...
                         "a TOWN and a town only, with the strip otherwise on -- CONFIRM-2's own "
                         "picture (runs 20260924T084418 / 084811): the body armed at load, "
                         "from this message two frames after the empty-handed "
                         "0x006E, and the OLD weapon kept across an outpost F2 while the "
                         "doll changed (OBSERVED, CONFIRM-2 section 7: the run with this "
                         "flag OFF was the whole difference on screen). KNOWN-BAD: retail "
                         "never sends the own body a 0x006D "
                         "(0 of 46 outpost connections with a controlled agent, 0 of 44 "
                         "field; the 0x0022 controlled agent over every live tape), and the "
                         "client's one visual-equipment store per agent (record+0x24, "
    ...
    ap.add_argument("--no-map-travel", action="store_true",
                    help="THE REVERT ARM for DESKWORK-D1 step 7's world-map travel: "
                         ...
                         "served, non-explorable content map with a known spawn "
                         "that is not the one you are on and whose navmesh loaded "
                         "at startup; every other request is refused with NOTHING "
                         "sent (retail's refusal reply is NOT FOUND on any tape). "
                         "Also skips the startup prewarm of the destinations' "
                         "meshes (~13 s for the content set, under --map only). "
    ...
    ap.add_argument("--no-map-unlock", action="store_true",
                    help="THE REVERT ARM for DESKWORK-D1 step 7's unlock state: "
                         "send no s2c 0x0094 at login (the pre-arc behaviour). The "
                         "client's arr4 -- its known-maps bitmap -- is then written "
                         "only by 0x0099 MAP_UPDATE_CURRENT's handler, which sets "
                         "ONE bit per message (bts at 0x0059d239), so the world map "
                         "shows the map you are on and every map zoned into this "
                         "client session, and NOT the other served outposts. The "
                         "default sends 0x0094's five map-id bitmaps once per LOGIN "
                         "(retail: 29 of 29 on a login's first connection, 0 of 61 "
                         "transfer arrivals; not resent on a re-entry after our own "
                         "transfer) with arr4's bit set for every travelable content "
                         "map (bit == map id; arr0-3 sent empty, UNVERIFIED -- empty "
                         "on 27 of 29 tapes). Every one of the 10 live 0x00B1 "
                         "destinations had its bit set before the click, 9 by the "
                         "login's 0x0094 and one by an in-session 0x0099 [281, 1]. "
                         "ONE 0x0094 site in the server, counted over every "
                         "spelling, since a duplicate sender of unlock state wiped a "
                         "library and crashed a client (2026-09-15). maptravel.py.")
    ...
    ap.add_argument("--no-henchman-add", action="store_true",
                    help="THE REVERT ARM for DESKWORK-D1 step 5: ignore c2s 0x009F "
                         "HENCHMAN_ADD -- the party window's Add Henchman does "
                         "nothing and no standing henchman is marked hireable (no "
                         "0x0071), the behaviour before this step (only the launch "
                         "--henchman single roster row existed). The default marks "
                         "each `hireable` spawn row's henchman with 0x0071 so the "
                         "panel lists it, and answers the click with 0x00B0 "
                         "PLAYER_PARTY_SIZE then the 0x01BF roster row -- OBSERVED, "
                         "retail's own order and bytes, 3 of 3 on 20260819T132414 "
                         ":53419 (henchparty.py). An agent that is not a hireable "
    ...
    ap.add_argument("--enemy-weapon", default=None, metavar="ITEM",
                    help="WEAPONS-W6a: the content item key the --enemy fixture hostile "
                         "holds (hostile_bow, hostile_bolt, starter_hammer ...). Its type "
                         "sets the swing interval; a ranged item makes it SHOOT from its "
                         "range instead of chasing into melee.")
    ...
    ap.add_argument("--no-bow-classes", action="store_true",
                    help="WEAPONS-Q2 REVERT: a type-5 bow's 609 class is ignored -- every "
                         ...
                         "its 10 %% off the target's armour -- the reading every run "
                         "before 2026-09-19 made.")
    ...
    ap.add_argument("--no-skill-damage-word", action="store_true",
                    help="do not send 0x009F [10, player, skill] ahead of a "
                         "skill's damage word at the player -- the server as "
                         "it was until 2026-09-22. Retail sends it 92 of 92, "
                         "always naming the observer (never the observer's "
    ...
    ap.add_argument("--no-instant-announce", action="store_true",
                    help="announce a Stance / Shout / type-16 skill the way this "
                         ...
                         "speech bubble, no [21] for the seven skill_visual rows "
    ...
    ap.add_argument("--no-secondary-change", action="store_true",
                    help="SECONDARY-B5, the master revert of the K panel's "
                         ...
                         "--secondary-bits (so the drop-down stays greyed) and then "
                         "at 57e89956's own site, AFTER the player's 0x00A6; c2s "
                         "0x0041 SET_SECONDARY_PROFESSION counted UNHANDLED as that "
                         "tree did, nothing sent; and a STORED secondary not read -- "
                         "this server's bytes AND order until 2026-09-25 (57e89956, "
                         "pinned against the real load burst recorded from a git "
                         "archive of it). Retail: 0x00B6 on 95 "
    ...
    ap.add_argument("--per-wearer-batch-order", action="store_true",
                    help="send a party-wide shout's batch per wearer -- apply, "
                         ...
                         "wearer first -- as this server did until 2026-09-25. "
                         "The known-bad arm of DESKWORK-D5's batch-order item "
                         "(skills 56.9): retail sends every 0x0042 and only then "
                         "every 0x0027, each run in ascending agent id (43 of 43 "
                         "apply+speed batches; 39 of 39 multi-word batches "
                         "ascending, the caster's word first only when it has "
                         "the lowest id; the 6 two-apply batches ascending with "
                         "the caster's apply second); the two-apply-plus-speed "
                         "case is on no tape and is RECONSTRUCTION from those "
                         "two halves.")
    ...
    ap.add_argument("--player-max-always", action="store_true",
                    help="declare the player's property 42 before EVERY "
                         ...
                         "2026-09-22. Retail never puts the observer's own "
