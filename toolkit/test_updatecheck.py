...
if exe:
    ...
    check(_bp_census is not None and len(state["pins"]) == _bp_census,
          ...
          f" 2026-09-11, and the census did NOT move while ONE ROW CHANGED FILE: REFACTOR-TABLESQRT moved LUT_VA out of clientscan/grantsim.py into clientscan/tablesqrt.py together with the table sqrt it addresses. THIS is the half the count above cannot see and the reason this file is not only a spelling check: buildpins.baseline() keys on (file, symbol), so the move showed up as PIN GONE clientscan/grantsim.py LUT_VA plus PIN NEW clientscan/tablesqrt.py LUT_VA 0x93cac8 and exited 1 -- the signal reserved for an ArenaNet build, fired by a refactor. The baseline was re-taken in that same commit and both lines were quoted in it, so the exit 1 is recorded as this move rather than left for the next session to read as an update.")
