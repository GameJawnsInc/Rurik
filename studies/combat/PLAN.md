...
| 4 | ✅ 2026-08-14, `0ddfbd0` — `+0x44..+0x68` decoded as u32 (duration0/15, skill_arguments bitfield, scale0/15, bonus_scale0/15; +0x50 stays NOT FOUND). Reproduces the 4-skill FINDINGS anecdote byte-exact, and asserts its green-render rule (bit set AND endpoints differ). Emitted to the content rows; **nothing consumes them until step 8**. `test_skilltable` floor 26→34. Still open from C7: the wiki third witness for the endpoint values |
| 5–10 | ⬜ |
