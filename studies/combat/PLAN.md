...
| ~~`0x00EE` attr_id 10~~ | **RESOLVED as NOT-A-KILL-SHAPE, §13**: the `[10,0]`+`[0,X]` pair is a broadcast burst marked by `0x009C`, 6 of 7 sightings far from any death. **And attr_id 10 is NAMED, 2026-08-20: it is MORALE**, per the corpus's one player death carrying `[10, -15]` beside `0x009C [agent, 85]` and both pool maxima moving 15% — so `[10, 0]` is a morale no-op riding an experience award, and `0x009C` ("n=1, first-witness, uncatalogued" when the corpus was two captures) is the per-agent morale percentage, 83 sightings now | what the BURST itself is remains unknown; the morale reading explains its two morale-shaped members, not why it fires | [studies/morale/FINDINGS.md](../morale/FINDINGS.md) closed the naming; the burst still wants a capture with marks on it |
