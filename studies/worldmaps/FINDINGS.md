...
both arms, orthogonal to compression — since closed on main, `20cbd91`:
...
because `deploy.py:1234-1249` SKIPS the environment assertion when the staged
...
in `toolkit/` — `deploy.py:332` (passed to `stripbuild.build` as the flood
seed), `deploy.py:1266` (the readback assert) and `mapscale.py`. None is the
