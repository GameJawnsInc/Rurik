...
**CLOSED 2026-09-24 (DESKWORK-Q4), and the fix above was not enough on its own.** A3's extent projection landed on 2026-08-17 (`datplan.classify_runs`, `test_datplan` §9) and could not reach this case: the extent is read off the header, and here the header is what the live file-id table destroyed. `0xF5923800` stayed the largest usable run on `dat_study_38833` until today, and the same shape at `0xF8A5E200` (285,184 B) was the largest usable run on every 38833-line archive including `vault/run/slice`. The third rule reads the ROWS: `datplan.mft_content` parses a run at six 4-byte phases as 24-byte records and `classify_runs` withholds a run with at least 8 records byte-identical to live table rows AND a majority of its non-zero records. Measured over every usable run of all 32 archive-size `.dat` files in the vault — the 21 `Gw.dat` copies under `client/`, `dat_*/`, `run/`, `run-live/` and the 11 under `exports/` [OBSERVED, `datplan --free` on read-only copies; the lane swept the 21, its review the 32, and the fixer re-scored every run named in this paragraph directly]: `0xF5923800` scores 120,507 of 120,533 (100.0 %), `0xF8A5E200` 11,859 of 11,882 (99.8 %), `0xF8FD7C00` 6,675 of 6,698 (99.7 %), and the 38888-line archives' `0xFC84F200` (610,816 B) 25,445 of 25,450 (100.0 %) — all four now withheld. The floor is not zero everywhere: the 38797-line copies carry a dozen small runs with 1..20 identical records at 1..76 %, decoded record by record as CONTIGUOUS row spans inside otherwise compressed-payload bytes — slivers of an old table block in a region the client has since reused for files. That is the majority term's justification. The runs it ALONE spares — at least 8 identical rows, under one half of the non-zero records — are fifteen addresses across the vault, every usable run of the 21 copies re-scored through `classify_runs` on 2026-09-24 [OBSERVED; the review's RV2-2 — the first version of this sentence named two, and two runs it named elsewhere as spared by this term, `0xF5595400` (6 of 106) and `0xF6DD5400` (4 of 42), are spared by the COUNT term]: on the 38797 line `0x323A400` (25 blocks, 9 of 533), `0x731D000` (41, 13 of 874), `0x9662A00` (1 block, 8 of 20 = 40 %), `0xF53F7800` (19, 12 of 405), `0xF542FE00` (18, 13 of 383), `0xF567E200` (3, 20 of 63 = 32 %, rows 173569..173588 at the run's end) — the union over that line's eight copies (`run-live/2026-07-29` keeps only `0x731D000`; its live sessions wrote over the rest) — and, only on copies the client has run, `0xF5731C00` (39, 11 of 832); on `client/2026-04-30` `0x731D000`, `0x9660C00` (16, 8 of 340), `0xDC280800` (233, 16 of 4,970), `0xF53F6E00` (24, 12 of 511), `0xF5677800` (56, 20 of 1,194) and `0xF8449200` (921, 19 of 19,647); on the 38888 line `0xF9A62600` (1,038 blocks, 20 of 22,144) and `0xF9EF4A00` (below). Everything under one percent in that list is payload with a few row-shaped records in it; the 38833 and 08-20 lines have no usable run with a single identical row. §10's CONTROL is the `0x9662A00` / `0xF567E200` shape. The residue is named at the constants in `datplan.py` rather than tuned: single-block slivers at or above one half are WITHHELD by the rule although they are not tails (`0xDC29D800`, 16 of 21 = 76 %, rows 72325..72340; `0xF53F9C00` 12 of 21; `0xFAAE2000` 18 of 21) at a block each, and the mark says only what was measured; and `0xF9EF4A00` on the 38888 line (10 blocks, 69 of 213 = 32 %) is LEFT USABLE although its last 78 records are the contiguous rows 177714..177791 — 135 records of payload sit in front of them, and since `best_fit` writes from the run's start, a 7..10-block payload (3.1..5.1 KB; W6's 4,820 B map is one) placed there would overwrite those rows. They are stale rows whose generation's head is under the file in front of them; whether the client can ever return to such a generation is this study's standing UNVERIFIED, so the rule claims only that the region already went to a file once, not that overwriting it is safe. A per-block rule would catch both lists consistently. After the guard the largest usable run on the slice archive is 17,408 B; W6's 256×256 authored map compresses to 4,820 B, so bytes still do not bind there and rows do (0 slack + 1 erased; `compose --plan` now prints both, and `--fresh` restores the source's 17). Placement, measured rather than assumed: `best_fit` is smallest-fit, so before the rule a 4,820 B payload on the slice went to a 12-block run at `0x7857200` and the 8,192 B area reservation to a 23-block run at `0x4C19000`; only a payload over 160,768 B reached `0xF8A5E200`, and `plan_move` on `dat_study_38833` accepted 1.1 MB and 2 MB moves into `0xF5923800` — those are what the rule now refuses. §4.4's (b) is closed and (a) stands.
