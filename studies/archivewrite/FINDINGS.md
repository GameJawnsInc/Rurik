...
**CLOSED 2026-09-24 (DESKWORK-Q4), and the fix above was not enough on its own.** A3's extent projection landed on 2026-08-17 (`datplan.classify_runs`, `test_datplan` §9) and could not reach this case: the extent is read off the header, and here the header is what the live file-id table destroyed. `0xF5923800` stayed the largest usable run on `dat_study_38833` until today, and the same shape at `0xF8A5E200` (285,184 B) was the largest usable run on every 38833-line archive including `vault/run/slice`. The third rule reads the ROWS: `datplan.mft_content` parses a run at six 4-byte phases as 24-byte records and `classify_runs` withholds a run with at least 8 records byte-identical to live table rows AND a majority of its non-zero records. Measured over every usable run of all 21 archives in the vault [OBSERVED, `datplan --free` on read-only copies]: `0xF5923800` scores 120,507 of 120,533 (100.0 %), `0xF8A5E200` 11,859 of 11,882 (99.8 %), `0xF8FD7C00` 6,675 of 6,698 (99.7 %), and the 38888-line archives' `0xFC84F200` (610,816 B) 25,445 of 25,450 (100.0 %) — all four now withheld. The floor is not zero everywhere: the 38797-line copies carry a dozen small runs with 1..20 identical records at 1..76 %, decoded as CONTIGUOUS row spans (72325..72340 in one block at `0xDC29D800`) inside otherwise compressed-payload bytes — slivers of an old table block in a region the client has since reused for files, which is the majority term's justification and its CONTROL in §10. After the guard the largest usable run on the slice archive is 17,408 B; W6's 256×256 authored map compresses to 4,820 B, so bytes still do not bind there and rows do (0 slack + 1 erased; `compose --plan` now prints both, and `--fresh` restores the source's 17). `plan_move` on `dat_study_38833` no longer reaches `0xF5923800`; §4.4's (b) is closed and (a) stands.
