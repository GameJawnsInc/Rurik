...
## 1z-bs. THE CLICK PATH, AT DESK — **the waiver's kept branch splits by ORDERING, not by mover; the newest-is-a-stop clause ships; Q15 narrows to one branch.** HANDOFF-WAIVER §2.1 named the click path as the one route that could still overturn Q15, on a count that turns out to be a classification of the report stream. Joined on the guard's OWN state instead — its click contract, the REAL stale window, its 2 Hz transition log — the click path is waiver-dead exactly where the body moves, and the corpus's kept branch divides cleanly: where the newest report is a **stop** the next report finds the body at the same point in **151 of 151** windows; where it is a **walk-start** the body has walked on in **14 of 87**, every one at a client movement speed. That is 1z-bn's own argument applied to the newer member alone, and it shipped as `WAIVER_NEWEST_MUST_BE_STOP`, with a revert flag, sixteen new checks, and zero historical fires altered. On every capture held it is indistinguishable from deleting the waiver — Q15 is still the owner's, and it is now a question about one branch.

**Method.** Desk only, no run. Two scratch censuses, then a ten-lane adversarial fan-out (three blind recounts writing their own scanners from the row contract with the orchestrator's figures withheld; four code-lens refuters given one claim each and told to refute it; two design lanes; a completeness critic that re-measured every disputed number). What survived is below; what did not is in §1z-bs.9. New instrument: [`review/waiverclick.py`](review/waiverclick.py). Corrected instrument: `review/waiverlive.py` (§1z-bs.9). Every displacement in this section is the **chord to the next accepted report** — 1z-bq.2's method, an upper bound on where the body was at any decision instant inside the window — not a `w0score` live-column reading (owner-play captures carry no tape).

### 1z-bs.1 ★★★ The click path is narrower than §2.1 said, and its three numbers do not survive

**The guard's click contract, read from the code (OBSERVED, r1 lane; every clause verified):** `async_dest` has three writers in `agtrack_guard.py` — `__init__`, `on_report` (unconditionally `None`, outside the accepted/refused branch) and `on_click` — and none in `on_placement`, `on_emit`, `on_speed` or `tick`. authsrv calls `on_click` from exactly one site (the 0x003E arm, every decoded click while the guard exists) and `on_report` from exactly one (`_take_client_position`, for **refused** reports too). `_agtrack_guard_call` takes a plain blocking lock — no timeout, so no skipped feed — and disables the guard for the session on any exception, so a live guard cannot carry a drifted `async_dest`. The guard is seeded before it is published. `stationary()`'s first line is `if not STATIONARY_WAIVER or self.async_dest is not None: return False`. **So while a click is pending — a 0x003E after the last report of any kind — the waiver cannot be load-bearing, whatever the pair says.** (One hole in the *coverage* of that state, not in the contract: §1z-bs.6.)

**HANDOFF §2.1's "of the 180 stale coincident `{0x003D → 0x003D}` pairs, 124 have a click in flight and 19 are keyboard-only" is retracted.** All three numbers fail (r6, r7, critic):

- **180** is `waiverlive.py`'s figure, and that instrument scored "stale" with the pair's **own** gap (t_b − t_a) — the gap *before* the newer report — where the waiver's window is (t_b + 0.347222 s, the *next* accepted report). That is trap (3) of HANDOFF §4, inside the instrument the handoff recommended. Corrected: **154** closed windows on wall-clock rows; 167 if an open-ended last pair is closed on the capture's last row; 175 if pre-telemetry captures are read via their `t` field. Three lanes produced those three "different" counts from one population by varying exactly those two knobs (the critic reproduced all three), and the refused pair moves 1053 → 1054 → 1097 the same way. **Every count below states its knobs.** §1z-bq.1's table (180 / 175 / 1205) is the proxy's; the direction of its finding stands.
- **124** is reproducible only as "a 0x003E anywhere from ~10 s before the pair's older report through its newer one" — a plateau at 124 for lookbacks of 9.5–12 s; the principled definitions give 64 (a click between the two reports) or 0 (a click as the most recent movement input, which never happens — the client sends no 0x003D/0x0047 while pathing a click). It was a fitted number.
- **19 / 37** reproduces from nothing: `kbd_leg` rows exist in 6 of the 43 captures that hold the population.

**Under the guard's contract the population reads: of 154 closed `{0x003D → 0x003D}` windows, 50 are click-pending at the window's start, 17 go pending inside it, and 87 are waiver-LIVE for the whole window.** The click path is not where the waiver lives; the majority of the population is live, and the reason nothing ever fired there is the absence of a **want** (§1z-bs.4), not the click latch.

### 1z-bs.2 ★★★ The kept branch splits by ORDERING — the still branch and the walking one

`waiverclick.py` section D (closed windows, wall-clock rows, waiver-live throughout: no click and no refused report anywhere in the window):

| ordering (older → newer) | windows | still (≤ 1 u) | disp to next report p50 / p90 / max | > 100 u | implied speed within 5 % of a client movement family |
|---|---|---|---|---|---|
| `{0x003D → 0x0047}` **newest = STOP** | **151** (114 guard-era) | **151** | 0.0 / 0.0 / **0.0** | 0 | — |
| `{0x003D → 0x003D}` **newest = WALK-START** | **87** (29 guard-era) | 64 | 0.0 / 123.6 / **355.9** | **14** (9 guard-era) | **14 of 14** |
| `{0x0047 → 0x0047}` | **0** | | | | never coincides in 1,311 captures |

**The two kept orderings are opposites, and pooling them hides it** (r1's pooled "free/kept p90 0.0 u" was that artifact). A pair whose newest report is a stop is followed by a still body every time. A pair whose newest report is a walk-start is followed by a *walking* body in one window of six — and the positive control that could have failed did not: the fourteen implied speeds (displacement / dt) sit within 5 % of the client's forward 284.96, backward 187.89 or sideways 215 u/s families, thirteen of them within 2 %, e.g. 355.9 u / 1.25 s = 285 u/s. Zero plane changes in any of them. Zero fires in any of them. The critic's fuller variant (pre-telemetry captures read via `t`) adds two cases over **500 u** (513 and 515 u, in `authsrv-20260821T08*`), which the promoted instrument's wall-clock knob excludes and this section therefore quotes as the variant, not the headline.

**For contrast, the click-pending kept windows — where the waiver is dead by construction:** `{0x003D → 0x0047}` pending, 12 windows, 7 over 100 u, max **7,310.7 u** (the r1 specimen: ten clicks over 25.6 s and no report; a re-pin there would have used a point 7.3 km behind the body, and `async_dest` was the only thing refusing it). That is where the click path's big displacements live, and the waiver already cannot fire there.

**Reading.** 1z-bn argued that a walk-start "IS the something new" — the client announcing it is leaving — and refused only the `{stop → walk-start}` spelling because that is where its harm census had n = 19. The argument never depended on the older member: only the newer report's kind says anything about what is true after it. The corpus now says the same from the other side. **The one branch of the waiver whose a-priori argument survives is `{walk-start → stop}`** — a body told to move that reported it did not — and it is the branch with 151 of 151 still.

### 1z-bs.3 ★★★ The double walk-start — the specimen the click framing hid

The fourteen walking windows are keyboard play without the lead (`waiverclick.py --movers`). Their pairs are two `0x003D` at one point, 0.045–9.0 s apart, mostly **0.2–2 s** (ten of fourteen), with movementType transitions `1→2` / `1→3` (a W+Q or W+E chord — d2 lane: A and D are *turn* keys, the diagonals are `mt 2 = W+Q`, `mt 3 = W+E`, at ±26.57° over n = 671 transitions), `1→1` (a heading change before the body leaves), `7→7`, `8→8`, `5→4`, `1→4`. Then 103–356 u in 0.37–1.5 s. **It is not a chord phenomenon**: of 22 sub-45 ms coincident double walk-starts in the corpus, 20 are followed by 0.000 u — a parked body flicking key combinations, which is the case the waiver is *right* about (r5). The moving population is a walk-start followed, a few hundred milliseconds to two seconds later, by a second report at the same point, and then departure. The client emits the shape freely: 446 coincident `{0x003D → 0x003D}` pairs in owner play.

**What the guard does with it, replayed (test_agtrack_guard.py §15; r5 lane arms A–D independently, same answers).** A leg opening on two walk-starts at one point, 40 ms apart, under a 520 u lead at 288 u/s, answered at +1.5 s (report age 1.46 s, the sync copy ~420 u out):

| arm | `stationary()` | `repin_state` | `blocked_by` |
|---|---|---|---|
| the 1z-bn build (this clause off) | **True** | **`due` / arrival-risk** | None |
| the shipped build | False | `blocked` / arrival-risk | stale-report |
| `STATIONARY_WAIVER` deleted (Q15's other answer) | False | `blocked` / arrival-risk | stale-report |
| the 1z-bn build, a **single** walk-start opening (the refused pair) | False | `blocked` / arrival-risk | stale-report |

The 1z-bn build re-pins it: `RUN_SPEED × age` = 420 u of counterfactual rewind — the 1z-bl defect, through the pair 1z-bn kept. The shipped build and a deleted waiver answer identically. **And its exposure in the corpus is zero.** All 87 live windows come from captures with no `kbd_leg` rows; across 446 coincident double walk-starts exactly one sits within 0.3 s of a `kbd_leg arm`, at a 4.9 s gap; retrodicting the five largest specimens through the guard gives `arrival_risk` True in 0 of 156 probes each, because their grants were zero-lead and the mirror answered MATCH (r5). The branch is **reachable in principle and unexposed in fact** — which is exactly the state 1z-bn's refused pair was in until RUN-1zBL turned the lead on.

### 1z-bs.4 ★★★ No kept window has ever carried a re-pin want, and that is now a complete enumeration

Sample-and-hold join of the `agtrack_repin` transition log over every kept-pair closed window (`waiverclick.py` section C): **317 windows, 4 with a want by sample-and-hold, 0 with a fire.** The four (r3 lane, raw rows, each read independently of the scan that flagged them): **A** `20260830T215424` — a 0x003E lands **93 µs** after the stop report and no report clears it for 25.6 s, so `stationary()` was False for 100 % of the window; **B** `20260903T072931` and **C** `20260903T222122` — the standing `due` is a fire decided 0.04–0.06 s *before* the pair formed, on a report 0.09 s old (B, not waiver-carried) and on the *refused* pair 1.44 s old (C, one of 1z-bn's 25); the first sampled instant inside each window reads `none / match`; **D** `20260904T115613` — that capture ran with `STATIONARY_WAIVER = False` and was never a candidate. r3 adds the structural point that makes the null quotable: two of the three re-pin call sites fire at report age ~0 (p25 0.0003 s), so **only the 2 Hz tick can ever be waiver-load-bearing, and its transition log is a complete record of that sampler's code changes** — the corpus scan enumerates the sampled instants rather than sampling them.

The same join on the refused pair, as the vacuity control: 75 windows with a want, 25 with a fire — the 25 the waiver carried (§1z-bo.9's 22 plus RUN-1zBP's 3), all on `{0x0047 → 0x003D}`, confirmed by three independent joins and by `waiverbenefit.py` (85 fires / 25 carried / 0 kept).

### 1z-bs.5 ★★ What shipped: `WAIVER_NEWEST_MUST_BE_STOP`, and what it does not decide

`agtrack_guard.stationary()` now refuses the waiver whenever the **newest** accepted report is a walk-start, whatever the older one was; the 1z-bn clause stays beside it so each revert arm is a real A/B. **`--waiver-walkstart-pair-stands`** reverts to the 1z-bn build. **With this clause on, `--waiver-walkstart-stands` alone changes nothing** — the pair it re-admits has a walk-start as its newest report — and the `[map]` line says so when that flag is passed by itself; **reproducing RUN-1zBP's arm now needs both flags.** The capture header records the new flag with the other two (`capture_flags()` sweeps the module's uppercase bools).

- **Retrodiction: zero historical fires altered.** All 25 waiver-carried fires are on the refused pair (three independent joins; `waiverbenefit.py`). The clause's differential set over 1z-bn is the coincident `{0x003D → 0x003D}` stale window — 87 live, 154 closed — and it carries **0 wants and 0 fires**. `waiverretro.py`'s third arm (the waiver deleted, identical to the shipped build in 0 of 71 control-OK runs) **is** this clause's retrodiction, since the two are one object on every capture held; its stock/fix arms are the 1z-bn A/B and are now pinned with this clause **off** so they keep measuring what their names say.
- **Cost:** the class 1z-bn already accepted — a body that reported a walk-start and then genuinely stood still gets the unwaived gate, and the client's own arrival test decides (a ≥ 299.33 u snap of the drawn body onto the sync copy, if a want ever forms). Corpus exposure of that cost: zero. r6's objection that this is "a worse trade than 1z-bn's" rested on the *fire* population's n = 1 at 0.0 u; the population the cost lives in is the *window* population, and half of the guard-era windows there sit over a walking body (§1z-bs.2). The trade is the same one, with the same exposure.
- **Tests:** `test_agtrack_guard.py` §15 — the four orderings under the shipped build, the revert flag as a real A/B, the four arms above, the surviving `{walk-start → stop}` branch still `due`, section 9's walking arm untouched. Sections 9 and 14 are now interrogated with this clause **off**, so they keep pinning the builds RUN-1zAB, RUN-1zBO and RUN-1zBP measured. Floor 99 → **116**, from the green run.
- **What it does not decide: Q15.** On every capture held the shipped build and a deleted waiver are indistinguishable. What the clause keeps is the one branch whose a-priori argument survives; whether that a-priori argument is worth a code path is the owner's call (§1z-bs.7).

### 1z-bs.6 ★ Two latent holes found on the way, neither with corpus exposure

- **`async_dest` is not "the same contract as authsrv's click-in-flight latch"**, though its docstring said so. `state["click_moving_at"]` has **two** arming sites — the 0x003E arm and `_approach_send` — and `async_dest` has one. `_approach_send` (`ATTACK_APPROACH` ships on) walks the body on a silent leg toward its target with its own comment calling that "the click latch's exact meaning", and never calls `on_click`. Over an approach leg the guard believes nothing is in flight, and a stale coincident kept pair — the `{0x003D → 0x0047}` a standing player emits before attacking is the natural one — makes `stationary()` True while the body walks. Exposure: **9** approach 0x002A grants in 3 of 1,311 captures, none over a coincident stale pair (r1). Docstring corrected this commit; the asymmetry belongs in Q15: **deleting the waiver closes this for free; keeping it needs a second `on_click` call site.** Registered in PLAN §8.
- **The 1z-bn revert flag alone no longer reproduces RUN-1zBP** (above). Named in the flag's help, the `[map]` print and a source-locked check, because a revert arm that silently stopped reverting is the kind of thing this arc has paid for.

### 1z-bs.7 What this does to Q15 — a fifth reading, and a narrower question

The four readings in HANDOFF §2 stand (the counts grew with the corpus: 85 fires, 25 carried, all refused, zero kept). This section adds the fifth from the other side: **the waiver's kept branch, where it is live, is a still body when the newest report is a stop and a walking body one time in six when it is a walk-start; the walking half is now refused; the still half has never met a re-pin want in 151 windows and ~520 live seconds.** The click path cannot overturn it: where a click has the body moving the waiver is dead by its own first line, and where it is live after a click the body is still. The only experiment left with a positive outcome for the waiver is one in which a re-pin want arrives over a `{walk-start → stop}` pair while the drawn body has diverged from its reported point by less than the ≥ 299 u the client's own test would otherwise snap — a benefit no report chord can measure and no capture in the corpus has produced.

**Recommendation unchanged: delete `STATIONARY_WAIVER`, or equivalently leave the shipped clause in place — the two differ on nothing measured.** The question the owner is now ruling on is narrower than HANDOFF §2 put it: *is the `{walk-start → stop}` branch worth a code path for an a-priori benefit that has never been witnessed, given that keeping it also means closing the approach-leg hole?* Both answers are defensible; neither needs a run first.

### 1z-bs.8 RUN-1zBS, registered as optional, and why it is not built

[RUN-1zBS.md](RUN-1zBS.md) carries the design the d2 lane produced and the critic corrected. It is a *keyboard* run — the click path is closed at desk — and it is not runnable today: `parse_walk` takes one key per step and `hold_key` blocks for the hold, so a leg opening on two keys needs a new harness verb whose first exercise would be the load-bearing run; A and D are turn keys, so the chord is **W+Q / W+E**; coincidence is a lottery at ~10.7 % per leg-opening chord add (the reported point advances from rest within 20 ms; every coincident pair in the corpus is d = 0.000 exactly), so the exposure floor is ~**32** chord openings, not 3; and the moving population sits at **0.2–1.6 s** overlaps, not the 10–50 ms staggers a chord verb would naturally aim at. The informative arm is the **revert** (`--waiver-walkstart-pair-stands`): the shipped build can only show zero. It is registered so the next session does not re-derive it; it is optional because Q15 does not wait on it.

### 1z-bs.9 Corrections to the record, and the traps this section paid for

- **HANDOFF-WAIVER §2.1's three numbers** (180 / 124 / 19): retracted above; the handoff is rewritten this commit.
- **`waiverlive.py`** scored staleness with the wrong gap; corrected, and its verdict text now points at the ordering split rather than at "the run".
- **§1z-bq.1's table** is the proxy's (180 / 175 / 1205 → 154 / 163 / 1053 closed, wall-clock); its conclusion — the condition is live on kept orderings — stands.
- **§1z-br.3's mover split** is irreproducible and should not be cited; §1z-br.2's refutation of P1 and its structural reading are untouched.
- **§1z-br.1's second "park"** (0.0 u across 11.45 s on the `D:8` leg): the d2 lane found that leg emitted zero wire messages while the camera-relative heading rotated ~119°, and Q/E in the same run moved the body 511.8 / 511.9 u — **D is a turn key**, and that park is most likely a turn-in-place. RUN-1zBR's P4 ("the wall is real") is therefore met once, not twice; its P1 refutation rests on the report ordering and is unaffected.
- **This session's own first framing** — "the kept branch's live specimen is the chord-opened leg" — was over-read from five specimens; the r5 lane refuted it (one of five in the claimed timing window, one not a chord at all) and the critic located the moving population at 0.2–1.6 s gaps. The specimen is a double walk-start of any cause.
- **Traps**, for HANDOFF §4: a **report-stream classification is not a guard-state census** (124 → 50 + 17 + 87); a **census must print its knobs** (open/closed windows; wall-clock vs `t` rows) or three honest scanners will publish three numbers; **each JSONL row stamps its own `time.time()`**, so a report and its decoded 0x003D never share a timestamp — join nearest within 2 ms, and a lane that joined on equality got `None` for all 1,370 pairs and correctly said so rather than substituting; and **pooling two orderings with opposite behaviour** reads as "p90 0.0 u" while one of them walks.

## 1z-bt. Q15 RULED — **`STATIONARY_WAIVER` is deleted.** Owner's decision, 2026-09-05, on §1z-bs's evidence. The predicate, its two clauses, the three revert flags and the report-kind bookkeeping that served them are gone from the guard and from authsrv's CLI; the re-pin's freshness gate stands on report age alone. On every capture held this is zero behavioural change, and its client-side witness already exists: RUN-1zBO.

### 1z-bt.1 What was removed, and what stands

**Removed from `agtrack_guard.py`:** `stationary()`; the switches `STATIONARY_WAIVER` (1z-ah), `WAIVER_WALKSTART_ENDS_STILL` (1z-bn) and `WAIVER_NEWEST_MUST_BE_STOP` (1z-bs) with their comment blocks; the state `prev_pos`, `_last_report_pos`, `last_is_walkstart`, `prev_is_walkstart` and the bookkeeping in `on_placement` / `on_report` that fed them. `_repin_block`'s stale clause is now `now - client_pos_at > REPIN_MAX_REPORT_AGE` and nothing else. **Removed from `authsrv.py`:** `--no-repin-stationary-waiver`, `--waiver-walkstart-stands`, `--waiver-walkstart-pair-stands` and their `[map]` blocks. The capture header no longer carries any `agtrack_guard.*` key; the sweep that recorded them stays for whatever lives there next.

**What stands, kept as the guard's own comment:** the derivation the waiver argued against and this gate enforces — a `0x002C` Clears the record and SetPositions **both** copies (AgMsg.cpp 579 and 584), so re-pinning a silently walking body to a stale report drags the drawn body with it, and `RUN_SPEED × age` is the bound that prevents the warp an earlier build shipped (630, 189, 765 u). `on_click` and `async_dest` stand too, now feeding only `_async_est`'s glide for the budget term.

**No revert flag, deliberately.** The code is gone. A run wanting the old behaviour reads it off the captures of the 1z-ah…1z-bs era, whose flags rows name their arm, and off FINDINGS §1z-ah through §1z-bs.

### 1z-bt.2 Why "zero behavioural change" is the claim, and its witnesses

§1z-bs.5 established that the last shipped clause and deletion are one object on every capture held: `waiverretro.py`'s waiver-deleted arm was identical to the shipped build in 0 of 71 control-OK runs; the clause's differential set — 87 waiver-live `{0x003D → 0x003D}` windows — carried zero re-pin wants; the branch the clause kept — 151 `{0x003D → 0x0047}` windows, ~520 live seconds — carried zero. So the deleted build's behaviour on this corpus is already measured, and its client-side witnesses are the runs of the equivalent clause: **RUN-1zBO** (lead on: zero `0x002C`, 0 of 7 legs rewound, separation 13.0 u) and **RUN-1zBM** (lead off: 7 of 7 legs walked). A verbatim run of the deleted build would reproduce RUN-1zBO by construction; it is not registered.

**The accepted cost, stated.** A maturing lead over a stale report is no longer pre-empted: the arrival matures in the mirror and the client's own reprieve test decides (a ≥ 299.33 u snap of the drawn body onto the sync copy if no chain node lies within 100 u). Test §9 pins that maturation on RUN-1zAB run A's shape. Exposure of that cost in the corpus: no re-pin want over a still body's stale window, anywhere (§1z-bs.4).

### 1z-bt.3 What the tests pin now

`test_agtrack_guard.py` §9 replaces the old §9, §14 and §15: source locks that the switches, the predicate, the kind bookkeeping, the three CLI flags and the header keys are gone and that the gate is one comparison; RUN-1zAB run A's shape — three reports on one point under a 520 u lead at the capture's 190.08 u/s — has its risk predicted with the arrival still ~0.425 s out, its re-pin BLOCKED / `stale-report` whatever the kinds, and its arrival maturing; RUN-1zBL's leg shape BLOCKED, with the gate pinned at exactly `REPIN_MAX_REPORT_AGE`; the double-walk-start opening BLOCKED; **the `{walk-start → stop}` opening the last shipped clause still waived BLOCKED — the deletion's one behavioural delta, red on the 1z-bs build** (every other fixture has a walk-start as its newest report, which 1z-bs already refused, so the verification lane rightly noted the rest of the section is green on the predecessor too); a walking body never proposed; a fresh report DUE whatever the pair looks like; rate, rejects and unseeded named; the click glide feeding the estimate and any report ending it; the telemetry row naming the blocker; a `0x002C` clearing the keyboard leg. **Floor 116 → 74**, from the green run. `test_srclint.py` green.

### 1z-bt.4 Instruments retired or annotated

- `waiverretro.py` **retired**: its arms no longer exist in the guard; `main()` refuses with a pointer to §1z-bn.3, §1z-bo.5 and §1z-bs.5. Kept for the method.
- `waiverbenefit.py`, `waiverlive.py`, `waiverclick.py` annotated: they measure the corpus as it was, and a waiver-carried fire cannot occur on a post-1z-bt capture.
- `repincheck.py`: the waiver's licence for a stale coincident AGTRACK re-pin now applies only to captures whose flags row recorded the waiver ON (and to the unrecorded 2026-09-03 window before the header sweep landed); on any other capture such a re-pin is a **VIOLATION** — a re-introduced waiver or a broken gate, not a measurement. The first draft of this change left the licence unconditional, a check that would have passed on the broken arm; the verification fan-out caught it. `tapdrive.py` no longer passes the retired flag.
- **RUN-1zBS withdrawn.** Its purpose was to witness the waiver's refused half; there is nothing left to witness.
- **The approach-leg hole (§1z-bs.6) closes with the waiver.** No gate branches on whether a click is in flight any more; `async_dest` still shifts the estimate every veto and re-pin decision reads, and over an approach leg it stays None, so the estimate sits at the last report while the budget term widens with its age, which errs toward the veto. RECONSTRUCTION, unmeasured (9 approach grants in 3 captures). Two fixture defects the fan-out found on the way: two earlier sections passed `on_speed(288.0)` where the 0x002B field is a *multiplier* of 288 u/s (82,944 u/s, an arrival 6 ms after the grant), and `run_a` omitted the capture's 190.08 u/s so its "0.426 s early" was an arrival already overdue — both fixed, and the earliness is now measured.

### 1z-bt.5 What is still open

- **PLAN Q13** (`D1_LEAD` on by default) — the owner's; its precondition has been met since §1z-bo.
- **`gate2-offmesh` exposure** — untested; every guard replay so far used `mesh=None`.
- **The movement-type census** corpus-wide (HANDOFF §6.6) — a curiosity now, not a debt.

## 1z-bu. Q13 RULED — **the keyboard lead ships ON by default.** Owner's ruling, 2026-09-05: "turn `D1_LEAD` on by default." Applied to the lead Q13's evidence is about — MOVECODE-1z-t's term 1, `KBD_SYNC_LEAD_ON`, the 520 u navmesh-clipped lead behind `--kbd-lead`, opt-in since 1z-u — and NOT to the symbol `D1_LEAD`, which names something else. The assumption is stated here so the owner can overrule it in one line.

### 1z-bu.1 Which lead, and why the ruling is read this way

Two switches carry the word "lead":

- **`KBD_SYNC_LEAD_ON`** (`--kbd-lead`): 1z-t's term 1. The heading grant is led 520 u along the client's own reported heading and clipped to the navmesh; 520 is read off the client's `0x003D` distance trigger (chord p95 513.8 / p99 515.1 u). Opt-in since 1z-u (2026-09-03) after the operator's first ordinary session convicted it, while its four gates were built (1z-y hold and kill, 1z-z matched plane, 1z-ab the length, 1z-aa the fence-shutter audit). **This is the lead RUN-1zBM, RUN-1zBO and RUN-1zBP measured**, and the one whose "return waits on one scored run alone" (§1z-ab) — the run that then happened (§1z-bo).
- **`D1_LEAD`** (`--d1-lead`): REALFIX-A2's four-term bundle from 2026-08 — the 766 u *unclipped* lead (`reported + vec2 + 0.5·unit`), speed truth, plane truth, stop-ack. It parses alone on today's build: `--zero-lead` and `--plane-carry` resolve ON at run time by an earlier owner ruling, and the module globals' `False` is the pre-resolution literal — this section's first draft misread them as refuted defaults, which a verification lane caught. What keeps it opt-in is the code. **`D1_LEAD` wins when both leads are set** (the 0x003D arm's first branch), so a literal flip would make the 520 u lead unreachable and RUN-1zBO would stop being the default's witness. **The 1z-y gates are keyboard-only** — the hold is gated `and not D1_LEAD`, the kill arms only `a2_src == "kbd"` and pops only `kbd_leg` — so D1 by default ships a lead at 1.49× the client's trigger (+513 u signed p95 overshoot against +342 at 520) with exactly the maturation failure 1z-u convicted. Its click half (`a2_click_leg`) is unreachable under the router. And a real default would need `--no-d1-lead`, a resolved value fed to the composition matrix (the send site reads the global while the matrix reads the argument), and a router×d1 composition rule that does not exist. The arc derived 520 over its 766 for the keyboard ("WHY 520 AND NOT D1_LEAD's 766").

PLAN Q13 is titled with `D1_LEAD` because it was raised in 2026-08 (§1i) about the bundle, and its body says so twice — "the bundle question Q13 poses (adopt `D1_LEAD`?)" — so the owner may have meant the words literally; that is now said in Q13 beside the ruling. Its 2026-09-05 precondition paragraph — "its blocking precondition is now MET (§1z-bo)" — is about the keyboard lead, and so are the runs it cites; the body's own numbers (17 refused clicks, 1.40 grants per 1,000 u) are §1i's click-path figures, since answered by the router. **The ruling is applied to the lead the precondition was about**, because a literal flip would replace the measured lead with an unmeasured one. If the owner meant the bundle, that is its own arc.

### 1z-bu.2 What shipped, and what its witness is

`KBD_SYNC_LEAD_ON = True`. `--no-kbd-lead` reverts to the 1z-u…1z-bt default; `--kbd-lead` still parses, as a no-op, so every runsheet of the opt-in era keeps its meaning. The `[map]` banner names the arm either way. Terms 2 and 3 were never off. The capture header records the flag as before.

**The shipped default is RUN-1zBO's configuration exactly** — `--kbd-lead` on a build whose guard clause was, on every capture held, identical to the deleted waiver (§1z-bs.5, §1z-bt.2). Its numbers are therefore the default's: zero `0x002C` on the player, 0 of 7 legs thrown back, `setposition` 0 hits, moving-only world-0 separation **p50 13.0 u (p90 170.2, one leg parked 1.42 s at a plane-seam clip — a property of the route, §1z-bo.7)** against RUN-1zBM's lead-off **252 u**. RUN-1zBP is the known-bad arm: same binary, the waiver's clause reverted, 3 fires, 3 of 7 legs rewound, −432 u. The lead's own conviction (1z-u: a lead maturing unanswered, 498 u snap, the starved attack) was localised at §1z-bl.8 to the guard's stationary waiver, not to the lead, and that waiver is gone.

**What a default run still owes, said in advance.** §1z-bo.4's caveats stand: one state-matched trial, `gate2-offmesh` at zero exposure, and the enslavement detector (§1z-x) must score any separation before it is quoted. The operator's ordinary play is the run that matters and it cannot be scripted (the 08:46 session was the conviction; the next one is the confirmation) — score it with `w0score`'s live column and `waiverclick.py`'s want census, not by feel.
