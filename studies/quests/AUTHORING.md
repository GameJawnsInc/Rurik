...
resolve the string from the owner's own archive at run time. `enc_name` was the
same id as a PLACEHOLDER only, until 2026-08-19: Q2b's authoring half wrote our
own plain record (id 100552, 'A First Errand') and the landed quests.toml now
commits the two-word varint of a bare id of ours. This draft schema predates
that and stays as written; the landed row is the authority.
...
| **Q2b** ✅ | *(Only if Q0 is green and the owner wants authored names.)* Write our own quest-name string into the owner's own archive with `toolkit/mapdata/textwrite.py`, commit the bare id. | `textwrite.py`'s own guards (refuses `C:\gw` and `vault/dat_study`) hold; an empty merge is byte-identical; the name appears on screen. **This tool already exists, is tested, and has 188 authored skill names on a retail screen behind it** (`TESTS.md:473`, `:508`). No lane in the recon found it. | Q0 | **CLOSED 2026-08-19, both halves, same day.** Authoring: `--set 200 "A First Errand"` (the flag added for this rung — one record needs no names module) wrote string id 100552 into the reskin-roster archive, journalled, read back exactly; `content/quests.toml` commits `[0x8103, 0x0CC8]` (`codedstr.encode_id(100552)`). Screen: probe `quest_name_authored`, agent-piloted, harness `20260819T085654` — control arm rendered 'Ascalon', treatment arm rendered **'A First Errand'** in BOTH the tracker and the 'Quest Added' toast, the A/B riding `0x0049`'s last-pushed-becomes-active write in one run. The client must be the reskin-roster one (only its archive holds record 200) — the probe note carries the trap. |
...
**Q1b IS DONE, BOTH HALVES — (a) on 2026-08-17.** `toolkit/content.py` no longer calls a single cited assert refused expression, and `PLAN.md` §7 Q3's REFUSED bullet gained a forward pointer to its own refinement -- the dated ruling text itself was left alone, because a decision record is supposed to say what was decided that day. **§6.1 gained TWO Fournux rows**, not one: `textrec.py`'s string-id split `id // 1024` (the module's own docstring says it was **not** re-derived by us -- the only such rule in the repo, held up by an ORACLE rather than a second reading) and `skilltable.py`'s 0xA4 record layout (read and re-derived). MIT obliges attribution, so `THIRD-PARTY-NOTICES.md` gained the notice too -- that half was a licence obligation, not a tidiness one. Three further Fournux citations owe NOTHING and the register now says so, so the next audit does not re-decide them: `gwdat.py` cites it as a second witness, `repoint_skill.py` as one side of a CONTESTED reading we then measured, and `test_datwrite.py` read its fixture generator and took nothing. **And the class is closed, not just the instance:** `toolkit/derivlint.py` + `test_derivlint.py` now check that every upstream named in `toolkit/` is accounted for somewhere. It was open from 2026-08-13 to 2026-08-17 because the flag lived in study documents and the obligation lives in `PLAN.md`.
...
**Q6 IS BUILT AND TESTED; ITS VERDICT NEEDS A RUN (2026-08-17).** The server now replays held quests on instance load and the offline half is covered by `test_quests.py` §§17-19 (74 checks, floor 73). **The rung's real content turned out to be a LIFETIME question, not a message question:** `state = {}` is created per connection, so the held-quest set died at every map transition before anything could replay it. `QUEST_PROGRESS` now carries `quests` and `objectives_done` across connections and **deliberately does not carry `desc_sent`** — that is bookkeeping about what THIS connection has been told, and carrying it would make `_replay_quests` skip the `0x004C` that sets the description-filled flag, turning every objectives line into a silent no-op **on the second map only**.

**One deliberate divergence from the capture.** ArenaNet sends `0x0054` before `0x004C` and TRIPS ITS OWN GATE, twice in the corpus; we send `0x004C` first. Copying the observed order verbatim would reproduce a bug we can see, and it fails invisibly — an empty objective reads as the client ignoring us. The ladder named this as the rung's red and it is now an assertion.

**What still needs the owner:** the acceptance criterion is *walk through a map transition and the quest log survives*, and no client has run this. Nothing offline can substitute. Also unmeasured: WHICH quest is active on a fresh load — we take the lowest held id and say so, and with one row in the table the choice is not observable.
