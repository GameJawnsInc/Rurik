...
**Not settled: whether a real client will accept anything we author.** Every claim in this document is read-side. Our server has never sent a single quest message — `grep -n '0x0049\|QUEST_ADD\|0x0012\|REQUEST_QUEST_INFO' toolkit/authsrv/authsrv.py` returns one unrelated comment about handler thunks at line 279 and nothing else. The only quest message this project has ever put on a wire came from `probes.py`'s `compass_quest` step with **empty** strings, and it produced a quest-log `?` icon and no compass marker (`studies/minimap/FINDINGS.md:721`). The cheapest experiment in this arc — send `0x0049` with `enc_name = [0x3D64]` and see whether the log renders **Ascalon** — is a one-line change to `probes.py` and is still unrun.
