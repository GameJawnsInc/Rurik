...
from the census entirely. Three basenames collide in build 38797 (`CmpIo.h`,
`OsInput.cpp`, `PrApi.cpp`) and only `PrApi.cpp` spans two directories, so it is
the only one that can hide a module. **Fixed in the same commit**: `modules()`
keys on the full path, `--file` prints the split when a name matches more than one
file, and `test_codescan.py` §8 pins it with the counts as literals (19 and 67,
not 86) and all seven `Engine\Map\Props` files present. Sabotaged by restoring the
basename keying: four checks go red.
