...
| `0x01C2` / 450 | `PARTY_HERO_ADD`, `uint8 level` | level | a hero | `ldufr__OpenTyria/code/GameMsg.h:466-473` | **UPSTREAM** |
| `0x01BF` / 447 | `PARTY_HENCHMAN_ADD`, trailing `profession` + `level` bytes | level | a henchman | `GregLando113__GWCA/…/StoC.h:857-863`; `schema/messages.json` "447" = `{word, word, string16(20), byte, byte}` | **UNVERIFIED** — one lineage names the two trailing bytes |
| `0x0074` / 116 | `MERCENARY_INFO` = `{hero_id, level, primary, secondary}` | level | a mercenary hero | `GregLando113__GWCA/…/StoC.h:315-321`; `ldufr__OpenTyria/code/opcodes.h:220` | **UNVERIFIED** — one lineage names the fields |
