...
§6–§8 + §5c (65 bare + 3 skips; 69 + 1 skip with the vault's 57-row overlay; 72 with the
...
word. `--no-label-knockdowns` makes `skill_knocks_down` ignore a LABEL-tier row's field on
every path; the hand rows' (Hammer Bash, Heavy Blow, Earthquake) stand.
