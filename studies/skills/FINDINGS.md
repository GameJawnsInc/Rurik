...
foe's shape). Every one rides `0x009F`, 0 on `0x00A0` — and every one of the eight skills
on tape is SELF-kind (target byte 0), so the no-target form is OBSERVED for a self-kind
instant skill and RECONSTRUCTION for the 15 vault rows of these types that target a foe
or an ally (7 foe shouts, 3 ally, 3 other_ally, 1 foe stance, 1 unkeyed; none cast on
any tape — `instant_open` names one once, SKILLS-IA-C1); **property 60 for an
instant-type id: 0 of 133** (SKILLS-IA-A1); every 48 names
...
348, 1, 6, 10.0]`, `0x00E3 [30, 348, 0]` — the observer's apply (29) AHEAD of the
caster's own (30). A foe's stance (agent 101, t = 1348.673): `[48, 101, 10]`, `[21, 101,
21]` and nothing else. A team-mate's 364 (conn 54071, t = 576.967), the batch whole:
`0x00E5 [11, 364, 0, 3]` and `[21, 11, 11]` (the OBSERVER's own — it shouted inside the
same 60 ms), `[48, 13, 364]`, `[21, 13, 622]`, `0x00A5 [13, (0x6658)]`, `0x0042 [11, 364,
13, 52, 12.0]` (the observer wears it), a `0x001E` and a `0x0029`, then `0x0027 [11,
383.04]`, `[12, 399.0]`, `[13, 399.0]`, `[14, 399.0]`, `[15, 383.04]` — ascending by
agent id, the caster 13 in its id's place. **Order over all 133:** `[48]` < `[21]`
133 / 133; `[21]` < `0x00A5` 67 / 67; `0x00A5` < `0x0042` 59 / 59; `[48]` < `0x0042`
103 / 103; the caster's OWN `0x00E5` < `[48]` 86 / 86 (62 observer + 24 hero — the first
cut wrote 93 / 93 by taking any E5 in the batch, and seven team-mate batches carry the
observer's own E5, residual (5)); `0x0042` < `0x00E3` 86 / 86; `0x00F1` < `0x00E3` 8 / 8;
`0x00E3` < `0x0027` 29 / 29; no batch inverts any part. The observer's
...
from a shout's name id to the next skill's is 3 for 36 of them, 1 or 2 for 10 (no room
for a third record — name_id + 2 is the next skill's string) and 4, 5, 6, 30 or 6279 for
the other 6 (room, untested) (a stance's block is two records — 346: 25905, 25906; 25907
is the next skill's name). The id is therefore a per-row fact: `content/world.toml`'s new `skill_speech`
...
caster's E3, then every `0x0027`, **each run in ASCENDING AGENT ID** (SKILLS-IA-B2):
43 of 43 batches with an apply and a speed word put every word after the last apply;
all 39 multi-word batches are ascending by agent id — the observer's 17 (16 with an own
word; the 17th, t = 745.231, is a re-cast over an open shout with no own word, §56.5),
the team-mates' 14, the foes' 8. **"The wearer's word first" was the first cut's
reading and it is wrong as a rule** (the review's EV-2): it held 16 of 16 over the
observer's batches because the observer is the LOWEST agent id in every one of them,
and over the 22 team-mate and foe casts the caster's word is first 0 of 22 — e.g. conn
50527 t = 482.524, caster 15, words on 7, 13, 14, 15, 16, 17; conn 54071 t = 603.816, a
foe caster 9, words on 7, 8, 9, 10. A Crippled ally gets two words (199.5 then 399.0 —
the boost over the still-open Crippled, then the cure's restore, §48.2's pair, both
behind the E3). The 6 two-apply batches (the hero's 348 landing on the player and on
itself) hold the applies adjacent, 6 of 6, **and ascending, 6 of 6 — the observer's 29
ahead of the caster's own 30**, so the caster's apply is not first either; none carries
a speed word (348 moves no speed), so "two applies AND speed words", the exact case
where ours interleaved, is **UNWITNESSED**; the composed rule is RECONSTRUCTION from
two OBSERVED halves and is labelled so at the call site. What "ascending agent id"
IS on retail's side (a server walking its agent list in id order, most likely) is not
a thing the tape can say; the rule is the shape, 39 of 39 and 6 of 6.

**Shipped (`INSTANT_ANNOUNCE`, `PER_WEARER_BATCH_ORDER`; `test_instantannounce.py`, 67
...
* `apply_effect`'s party-wide path applies to every wearer in ascending agent id and
  defers `push_speed` (`state["speed_deferred"]`, the cure's restore included) across
  the applies, flushing once, ascending — every `0x0042`, then every `0x0027`. For the
  player (agent 1) that is the first cut's bytes; for a hero's or a body's shout the
  applies and the words now sit in the tape's order (a hero's 364 with the player and a
  henchman in earshot: applies 1, 30; words 1, 30, 31).
* The one-tick window between an instant skill's press (or begin) and its completion
  is no longer cancellable (the review's CD-5): `_mark_cancelled` (movement, Esc, a
  knock-down), `interrupt_player` and `interrupt_body` treat a begun instant cast as no
  cast — RECONSTRUCTION from the tape's dt = 0 (retail has no window: E4, E5 and `[48]`
  share one stamp, 62 of 62), because until this pass a movement report inside that
  tick sent `[59]` + E2 for a cast the client never saw start. A QUEUED instant cast is
  still un-queued like any other.
* `land_skill`'s early exits (the target died during the cast; a chain requirement; a
  resurrection — the last two impossible for an instant-type row, 0 of 149) send the
  hero's deferred E3 (the review's EV-4 / CD-4: the first cut dropped it on the
  dead-target exit, which b50da5c8 sent) and the `[48]` in the `[58]`'s place, both
  RECONSTRUCTION — no targeted instant skill is cast on any tape.
...
* `--no-instant-announce` restores property 60 at the press, `[58]` at the completion,
  the hero's E3 beside its E5, no bubble, the cancellable window, and NO `[21]` for the
  seven `skill_visual` rows the lane added — they carry `since = "SKILLS-IA"` and
  `send_skill_visual` skips a marked row under the flag (the review's EV-1 / CD-1: the
  first cut sent the `[21]` under both arms and called the arm "today's bytes"; 346's
  row predates the lane, is unmarked, and sends under both); `--per-wearer-batch-order`
  restores the interleave, the caster's wearer first. Each is b50da5c8's bytes exactly:
  §4 of the test compares the arm against literals RECORDED FROM A `git archive
  b50da5c8` EXPORT driven through the same fixture, not against the new tree's own
  output. The spell and attack paths are byte-identical to b50da5c8 under both arms
  (§5) — the attack literal now reaches the strike (the weapon gate off, the damage
  roll seeded): the first cut's literal was the gate's refusal, so a property-99
  announce on the attack press left it green (the review's EV-3 / CD-3).
...
`[8→1]` likewise; the window is at least no longer cancellable, above); (3) a hero's E4
and debit at the start (retail: E4, `0x00D2` / `0x00A2`, 24 / 24; ours sends neither);
(4) a 348 apply (no `skill_effect` row, §56.7 unchanged, so the hero's 348 batch carries
no `0x0042` on our wire); (5) seven of the twenty team-mate batches (conn 54071 ×2,
50527, 50513 ×2, 50286 ×2, all on the arena tape) open with the OBSERVER's own
`0x00E5 [observer, 364, 0, 3]` and `[21, observer, 11]` — the observer shouting inside
the same 60 ms; unread here beyond the E5 count above. **Client confirmation** (the
bubble drawn over the caster, the stance animation, no assert) is the runsheet's, after
landing.

**The review pass (2026-09-25, two reviewers, fifteen findings, all confirmed by
re-derivation and fixed in the same day).** The tape numbers corrected above: "wearer
first" → ascending agent id (EV-2); the caster's own E5 < `[48]` 86 / 86, not 93 (EV-6);
seven, not three, team-mate batches carry the observer's E5, and the 576.967 batch is
now printed whole (EV-6); 364's 56 announces sit on seven captures, 348's on three
(hero 7 / team-mate 3 / observer 1 — none on the arena tape) and 10, not 16, of the 52
corpus shouts have no room for a third record (EV-5); the observer's 364 casts are 34,
21 on the arena (EV-7); the no-target `[48]` is OBSERVED for self-kind skills only
(CD-6). The code corrected: the revert arm's `[21]` (EV-1 / CD-1), the flush order
(EV-2), the hero's E3 on the early exits (EV-4 / CD-4), the cancel window (CD-5). The
test corrected: the attack literal reaches the strike (EV-3 / CD-3), the three start
sites and the queued begin are driven through the real ticks (CD-2), the known-bad arm
is compared against b50da5c8's own recorded literals, and every new check was inverted
in a scratch copy and reddened.
...
Build 38797, snapshot `vault/client/2026-07-29_221c13772c7a`. Locks:
`toolkit/authsrv/test_labelconsumers.py` (NEW: 30 checks bare, 33 with the marks loaded),
`toolkit/clientscan/test_skilldesc.py` §1b + §3 (bare 77, 146 with the vault),
`toolkit/authsrv/test_skilldamage.py` §14 (80), `toolkit/authsrv/test_guards.py` (45).
`SKILLS-LU<n>` = a finding of this section; convention:
[studies/idents/CONVENTION.md](../idents/CONVENTION.md). **The vault overlay was NOT
regenerated by this pass** — the branch rule: `vault/content/` is machine-wide, and the
step-4 overlay reached it early last time and served unreviewed rows to every session.
...
at THIS skill's cast does what retail does, or less; never more. Each consumer below is
the machinery a whole class of exclusions was waiting on, and each row it moves was
...
`aoe_range` in the corpus is one of 33 values, the common ones 156 (147 rows), 240 (103),
312 (64), 1000 (80), 5000 (41). Of the eleven `RECIPIENT_NOT_A_FOE` rows of §55.2:

| id | byte | `aoe_range` | duration | what the template says (read, not carried) | ruling |
|---|---|---|---|---|---|
| 183 | 0 | 156 | 0 | all adjacent foes take fire damage | **SHIPS**, `AREA_CASTER` |
| 188 | 0 | 240 | 0 | all nearby foes take fire damage | **SHIPS**, `AREA_CASTER` |
| 840 | 0 | 156 | 10/10 flat | the caster AND all adjacent foes are Poisoned | **SHIPS**, `AREA_CASTER`; the caster's own Poison is dropped (`CLAUSE_ALSO_CASTER`, "you and all" now raises it) and the flat 10 s — a field the text does not state — is `DURATION_UNMODELLED` |
| 1113 | 0 | 156 | 5/5 | foes adjacent to the cast location take holy damage each second for 5 s | **SHIPS**, `AREA_CASTER` + `DURATION_UNMODELLED`: ONE tick at the cast, the other four are not modelled — less than retail, never more |
| 2212 | 0 | 312 | 0 | all foes in the area take holy damage; hidden objects are shown on the compass | **SHIPS**, `AREA_CASTER` + `CLAUSE_REVEAL` (a new flag; a compass hint no combat consumer reads) |
| 97 | 0 | 240 | 0 | exploit a corpse, teleport to it, poison the foes near IT | stays out — the byte rule passes now and `UNMODELLED_CLASS` (the corpse) takes it; the area is corpse-centred besides |
| 1364 | 0 | 0 | — | the caster Bleeds; the next Necromancer skill that targets a foe Bleeds it | stays `RECIPIENT_NOT_A_FOE`: no area words, no radius — a self-condition and a rider, neither a consumer here |
| 769 | 1 | 312 | 5/20 | all adjacent foes Poisoned; shadow step away from the target | stays: byte 1 is unresolved (`effects.TARGET_KINDS`), AND the record's 312 disagrees with the text's "adjacent" — the 156 sits FLAT in its bonus slot (`bonus_scale` 156/156, bit clear). Reading `aoe_range` would poison at twice the reach retail does |
| 917, 1468 | 1 | 240, 0 | | a spirit destroyed; a pet's move | stay: byte 1, and `UNMODELLED_CLASS` |
| 770 | 4 | 156 | 0 | all adjacent foes Crippled; shadow step to target OTHER ally | stays: whether "adjacent" is measured from the caster's position or the ally's (where the caster arrives) is not in the record, and the two readings put the Crippled on different foes. Refuse to guess; an ally-centred arm is the residue for it |

So the arm is **byte 0 only**: `authsrv.caster_area_row(sid)` — the record's target 0, a
Spell (`SPELL_TYPE_CODE` 5), no projectile of its own, an `aoe_range` > 0 — and
`caster_area(sid)` the radius under the flag. It is `spell_burst`'s predicate with the
centre moved and the byte changed, and it reads the SAME field, the record's own
`aoe_range` (156 adjacent, 240 nearby, 312 in the area) — no constant of ours. The gate
mirrors it (`skilldesc.caster_area_record`) and adds the wording: `AREA_FLAGS` or
`ALL_FOES` (`CASTER_AREA_WORDING`; `TOUCH` and `ALL_ALLIES` are not foe areas).

**The shape.** At the player's E5 (`burst_player_caster_area`, the first branch, ahead of
the attack branch): `foes_within(state, PLAYER, the player's position, radius)` — the
selected target is not read at all — then per foe the word (`hit_enemy`'s `exact`), its
`[20, foe, me, impact]`, the condition; a condition-only row (840) skips the word. At a
body's completion the same terms `burst_body_spell` takes, centred on the caster's
position, computed before the 58 (the refusal contract), and a condition-only body arm
(`body_caster_area_condition`). **A duration is NOT refused** the way `spell_burst`
...
**The revert is NOT the one-target path.** `--no-caster-areas` makes a byte-0 area row
land on NOBODY — the log says so — because landing it on the selected agent at any range
is exactly the over-application §55.2 excluded these rows for. `caster_area_row` is the
...
party" (41 rows in the corpus carry 5000) — and 1262 carries 156. `cast_recipient` places
every byte-0 heal on the caster alone. The consumer: `party_heal_radius(sid)` (byte 0, a
Spell, an `aoe_range`, a `scale_means` in `SCALE_MEANS_HEAL`) and, in `resolve_heal`,
`party_within` — the caster, then every living `allies_of` the caster within the radius of
...
not guess either: a PERSON read each of the four templates (run time, `--row`), and the
reading is recorded as an enum against the template's sha256[:16]
(`skilldesc.CLASS_HEAL_READINGS`, `template_sha16`): **287 `INCLUDES_CASTER`** (heal the
entire party), **2221 `INCLUDES_CASTER`** (all party members are relieved of Burning and
healed — the cure is dropped, `CLAUSE_REMOVAL` via "relieved of", `CONDITION_UNNUMBERED`
for the Burning no slot numbers), **943 `CONDITIONED`** (only the members relieved of
Burning are healed — a heal the parse cannot condition), **1262 `EXCLUDES_CASTER`** (the
caster is not healed; "creatures" besides, which is no class the server has). The gate
(`party_heal_record`) ships a byte-0 class heal only with `ALL_ALLIES` wording, a radius
AND a matching `INCLUDES_CASTER` reading; a template that changes on a later build
changes its digest and the row falls back to `HEAL_RECIPIENT_CLASS` (a stale reading
...
and a knock-down clause), **973** (a Spell, `combo_req` 4: an off-hand; Blind on the target
and its adjacent foes — `AREA_ONE_TARGET`, the adjacent ones are not reached), **1033** (a
Skill, `combo_req` 1: a dual; earth damage and a Deep Wound whose slot is INDETERMINATE —
`skill_condition` refuses it, marked) — now go through the SAME judgement at the E5 for
any type (`NONATTACK_CHAIN_GATE`): `player_chain(state).state_on(target)` against
`chain.requirement_met`. On a fail the attack's shape: `[38, target, player, FAIL]`, a
second E5 with recharge 0, the cast's recharge and E6 zeroed, and NOTHING lands — damage,
condition, effect, heal all skipped (`_na_fail` guards both `apply_effect` and
`resolve_heal`). **The fail shape for a non-attack is RECONSTRUCTION**: retail's one witness
(daggers F7, the owner's cold 780) is an off-hand attack; the family rule quoted at the
attack gate — WIKI (GWW, "Off-hand attack"): failed attacks do not cause the skill to
recharge — is read as the family's. And **974** (a Skill whose text says it counts as an
off-hand attack; `combo` 2) ADVANCES the chain as its Crippled lands on a living target,
...
construction and nothing lands; the cast still closes. **Retail's bodies DO meet it** —
TAPE, OBSERVED n=5: every live 784 announcement is a body's (agent 30 on
`20260819T132414` ×3, agent 17 on `20260913T210901` ×2), each 0.41–1.37 s after the same
body's own 782 (type 14, `combo` 1: a lead) on the same target — so NOBODY is LESS than
retail, stated; a body chain is the residue. The gate reads the RECORD's `combo_req`, so
...
flags read: "relieved of" (`CLAUSE_REMOVAL`), "you and all" (`CLAUSE_ALSO_CASTER`),
"hidden objects" / "are revealed" (`CLAUSE_REVEAL`). `check_label_rows` — the vacuity
guard — now names a row shipped WITHOUT its consumer's mark or outside its consumer's
predicate: a non-attack `combo_req` row without `CHAIN_GATED`; a byte-0 damage or
condition (not on an episode type — 434's +damage rides its arrows) without `AREA_CASTER`
on a Spell with a radius and no projectile; a byte-0 class heal without a matching
`INCLUDES_CASTER` reading. `test_skilldesc` §3 strips each mark off 784 / 183 / 287 and
watches the checker name each, forces 97 in with `AREA_CASTER` (still named for its
corpse) and 1262 in with `HEAL_PARTY` (named for its reading).

**57 = 131 − 10 − 64.** Exclusions now: `HAND_ROW` 10, `DURATION_ONLY` 41, `PERCENT_SLOT`
1 (292), `CONDITION_ON_EPISODE` 6, `PET_ATTACK` 1, `UNMODELLED_CLASS` 5 (96 **97** 106 2051
2100), `RECIPIENT_NOT_A_FOE` 5 (769 770 917 1364 1468), `RECIPIENT_NOT_AN_ALLY` 3,
`HEAL_RECIPIENT_CLASS` 2 (943 1262), `CHAIN_REQUIREMENT` 0. Marks: `AREA_BURST` 3,
`AREA_CASTER` 5, `HEAL_PARTY` 2, `CHAIN_GATED` 3, `CHAIN_STEP_ADVANCES` 1,
`AREA_ONE_TARGET` 21 (973 joined), `DURATION_UNMODELLED` 10 (840 1033 1113 joined),
`CONDITION_BIT_CLEAR_REFUSED` 2 (1033 joined 167), `INDETERMINATE_SLOT` 1 (1033),
`CONDITION_UNNUMBERED` 2 (2221 joined 228), `LITERAL_DROPPED` 5, `CLAUSE_KNOCKDOWN` 7 (784
joined), `CLAUSE_REMOVAL` 3, `CLAUSE_ALSO_CASTER` 3, `CLAUSE_REVEAL` 1, the rest as §55.3.
**46 of 57 rows marked; the eleven unmarked are §55.3's eleven** — every new row rides a
consumer that is named. By type: Spell 35, attack 7, Stance 4, Skill 4, Hex 3, Signet 3,
...
python toolkit/clientscan/test_skilldesc.py                 # the default on-disk arm is red until that emit lands
python toolkit/authsrv/test_labelconsumers.py               # 33 checks once the marks are loaded (30 + a declared skip before)
```

On a branch: `RURIK_SKILL_LABELS=<emit>` points `test_skilldesc`'s on-disk check at an
explicit emit (it names which file it read; the default is the vault and stays so), and
`RURIK_CONTENT_EXTRA=<dir holding the emit>` gives the server-side tests the fresh rows as
the last content layer. **After the merge's emit**, `test_skilldamage` §14's chain check
and `test_labelconsumers` §5b stop being vacuous (0 loaded → 3 / 5 / 2 rows). **The
regeneration has an ORDER (the fix pass, ENG-D4C-2): the vault is machine-wide, so the
57-row overlay reaches every worktree on the machine at once, and a tree whose server
lacks a consumer serves its rows through the one-target path — main at 6d74d03b lands 183
...
(ALL_FOES AREA_ADJACENT AREA_CASTER)` and `bursts over N foe(s) within 156 u of the
CASTER`; `gamesrv_args = ["--no-caster-areas"]` is the control (nothing lands, the log
says NOBODY) and `["--no-skill-labels"]` the tier's. 287 at rank 0 heals the player and
every party body within 5000 u for 30 each, one blue word each; `--no-party-heals` heals
the player alone. 784 pressed at a hostile with no lead FAILS (the fail float, no Poison),
...
- **Refuted, small: "a caster-centred area for byte 0/1 would move 12" (§55.6).** It moves
  5. Of the twelve, 97 is corpse-centred, 1364 has no area, 769's radius disagrees with
  its text, 917 / 1468 are unmodelled classes, 770 is ally-centred, and byte 1 stays
  unresolved — no byte-1 row would ship even if it were resolved.
- **Changed: the gate reads a human reading** (`CLASS_HEAL_READINGS`) for the one class the
  parse cannot split, pinned to the template's digest so it cannot outlive the text.
- **Open, per skill (the residue, in order of cheapness):** 770's ally-centred adjacency
  (1; an arm centred on `cast_recipient`'s verdict would take it, if the wiki says which
  position); the condition rider on episodes (6: 113 435 926 1041 1997 2136); byte 1's three
  heals (918 1032 1354) and its resolution (769 917 1468 — a codescan of the target-byte
  reader); 292's percent-of-loss heal; 943's conditioned heal and 1262's "creatures"; the
...
foes around the TARGET rather than the caster, or at a radius other than the record's
...
at run time only. Locks: `test_labelconsumers` (+10, floor 30 → 40 from the bare run; 41
with the vault's skills table, 44 with the marks loaded), `test_content` (+3, floor 52 → 55
bare, 58 with the vault), `test_skilldesc` (a conjunct and a FAIL arm, counts unchanged:
77 bare, 146 with the emit).
...
| 9 | nits: (a) "no word of the text is carried" beside quoted fragments in `skilldesc.py`'s readings comment and `serverargs.py`'s help; near-verbatim fixtures in `test_skilldesc` §1b; (b) `--no-condition-heal-rule` silently turns the party arm off; (c) a mistyped `RURIK_SKILL_LABELS` became a declared skip; (d) PLAN.md §8.1's bullet restated the landing | all CONFIRMED | (a) paraphrased — the comment now says what IS carried (the id, the enum, the digest); the fixtures are our own sentences that still trip each regex; the area-wording comment at the caster banner names the flag set instead of the phrasings; (b) said at the banner and in the help (the arm lives inside that rule's recipient branch; the result is the caster-alone subset either way); (c) an explicit path that is not a file FAILS, naming it; the skip is the vault's default only; (d) the bullet is a pointer plus what is still open |
...
## 60. SKILLS-LV — pass 2 of the residue: the ON-HIT condition rider, the knock-down clause as a field, the flat-constant condition slot; 60 of 131 plain rows ship, and retail's 784 completion pins a label-row knock-down at 2.0 s (2026-09-25)

**Status: SHIPPED, behind three revert flags, as label-tier rows marked by consumer.**
DESKWORK-D4's residue per skill, pass 2 — the next three items of §59.6's list in its own
...
knock-down term to the tier, §60.3).
`SKILLS-LV<n>` = a finding of this section; convention:
[studies/idents/CONVENTION.md](../idents/CONVENTION.md). **The vault overlay was NOT
regenerated by this pass** (§59's branch rule): the emit went to the pass's scratch
directory, the tests read it through `RURIK_SKILL_LABELS` and `RURIK_CONTENT_EXTRA`, and the
merge installs it (§60.6). Until it does every server on the machine serves the 57-row tier.

**Prediction first (written to the pass's scratch before any template or tape was read),
and where the read refuted it.** Predicted: (a) the six condition-on-episode rows split
into ON-HIT riders (435, 926, 1997), an ON-STRUCK rider (113) and two stances whose Blind
...
**Provenance, stated once.** What the overlay gains is three FIELDS from our own
vocabulary — `condition_rider = "on_hit"`, `rider_weapon` ∈ {`any`, `physical`, `melee`},
`knocks_down = true` (the hand rows' own field name) — and four marks. The rider's weapon
class is read off the SENTENCE's qualifier ("physical attacks", "melee attacks") and the
sentence's shape is read as a structure (which slots share it, whether it opens with a
literal "for N seconds", whether it names the wearer's attacks); no word of the text
leaves the parse, and `test_skilldesc` §3 still asserts every overlay string is in
`OVERLAY_VOCABULARY`.

### 60.1 SKILLS-LV1 — the six condition-on-episode rows are three shapes, and the sentence tells them apart

`skilldesc.slot_sentences` puts every `%strN%` in its own sentence (split at punctuation
then whitespace, `_sentence_before`'s convention); for a CONDITION slot, `condition_rider`
reads the sentence for an ON-HIT rider (foes struck by the wearer's attacks; the wearer's
or the ally's attacks cause / inflict it), an ON-STRUCK rider (the next time / whenever the
wearer is struck; anyone striking them), or neither; `sentence_governed` says whether the
EPISODE governs the sentence (it holds a DURATION-labelled slot, or opens with a literal
"for N seconds" — 435's flat 24). The rider's weapon class is the qualifier before
...
**The shape.** `authsrv.episode_condition_riders(state, agent_id, item)` reads the
ATTACKER's open episodes at every landed weapon hit and returns each row's condition
whose `condition_rider` is `on_hit` and whose `rider_weapon` the attacker's ITEM belongs
to; `apply_episode_riders` puts them on the foe the hit landed on through
`apply_condition` (the same door as every condition). Three seams, the ones every landed
weapon hit passes: `hit_enemy` for the player (a swing, a skill strike or an arrow's
arrival — never `exact`, which is a spell), `land_swing` for a body's hit on the player,
`land_swing_on_body` for a body's hit on a body. The rider row's `skill_condition` is
**None** — the row's own field says the condition is not the cast's — so nothing lands at
the E5 or at a body's completion, which is exactly the over-application §55.2 excluded
these rows for. The rank is the episode's (`ep["rank"]`, as `swing_preparation_bonus`
reads it), so 435 at rank 0 Poisons for 3 s and at 15 for 15.

**The weapon class is the item's, read by two existing facts.** `physical` is the item's
587 damage-type word in `[damage_type.classes].physical` (0 blunt, 1 piercing, 2
slashing — a sword, a bow, daggers, a spear are; a wand's chaos, a staff and the tape's
earth scythe are not); `melee` is the item's `[weapon_type]` row's `delivery` (a sword, an
axe, a hammer, daggers, a scythe are; a bow, a spear, a wand, a staff are not); `any` is
every landed attack. A body with no item row, or an item with no 587 word, is refused —
...
revert reads, and it is still safe: no hand row and no label row before this pass is a
byte-0 episode-type record with a radius (a census of the 67 such records: 57
enchantments, 4 stances, 3 hexes, 3 glyphs — none with a `skill_effect` row;
`test_labelconsumers` §5a names any that appears), and the E5 consults the arm only when
a condition or a standalone damage is READ — a rider row's is None, so 2136's shape,
which is the same record shape as 1041's, could never burst even if it shipped. At the
E5 the Blind bursts over the hostiles within 156 u of the player, then `apply_effect`
opens the stance; at a body's completion the stance opens, then
`body_caster_area_condition` bursts.

### 60.3 SKILLS-LV3 — the knock-down clauses: five become the field, two stay marked, and retail's 784 pins the length

**The gate.** `knockdown_kind` reads what a knock-down clause knocks down — the FOE(S) the
row lands on, the CASTER ("you are knocked down"), a QUALIFIED class ("knocks down
attacking foes"), a NEGATION ("cannot be knocked down") — and `knockdown_emittable` turns
the foe kind into the hand rows' own `knocks_down = true` only when the record's duration
is 0/0 (`skill_knock_down_seconds` reads a flat bit-clear duration as the fall's length:
192's 9 s shower would fall for 9 s), the record has no projectile of its own (the arrival
paths carry a STRIKE's knock-down, not a row's) and the recipients are foes (byte 5 / 16,
or the caster area). The mark `KNOCKDOWN_APPLIED` replaces `CLAUSE_KNOCKDOWN` on the row.

| id | type | byte | duration | kind | ruling |
|---|---|---|---|---|---|
...
**The consumer that was missing.** The burst arms and the attack path read `knocks_down`
already (§55.4); the single-target NON-attack land did not. `nonattack_knock_down` now
runs at four sites: the player's standalone word (231, 294 — after the word) and its
condition-only land (784 — BEFORE the condition), a body's condition-only land and its
word. **The term is the TIER's only**: a HAND row on that path keeps its pre-pass shape —
the first cut took every row, and `test_weapons`' `--no-spell-areas` arm (Earthquake 170
falling back to one target, "no knock-down", the reading every run before 2026-09-20
made) went red, which is a pre-existing flag's documented arm moving under a pass that
promised hand rows would not — so the term reads the row's tier first.
`--no-label-knockdowns` makes `skill_knocks_down` ignore a LABEL-tier row's field on every
...
`skill_condition` (through the shared `_condition_terms`) catches `skill_scale_value`'s
refusal of a bit-clear slot and, under `--no-condition-flat-constants`'s default, reads
`skill_flat_constant` instead: a bit-clear slot with EQUAL endpoints is the constant the
client prints (the reader's witnesses — Rush's 25, Frenzy's 33 — and the referee's
`AGREE_FLAT`), and 167's Blind is 10/10 numbered by its own `%str2%`. DIFFERING endpoints
still raise, so 1033's Deep Wound 5..20 stays refused — §54.3's INDETERMINATE contest is a
client tooltip run's to settle, not this reader's. The marks now say which: 167
`CONDITION_FLAT_CONSTANT`, 1033 `CONDITION_BIT_CLEAR_REFUSED` + `INDETERMINATE_SLOT`. **A
census of every loaded `skill_effect` row with a condition means (24 slots): only 167 and
1033 are bit-clear, both label rows — the reader moves no hand row**
(`test_labelconsumers` §8 re-runs it on every load).

### 60.5 SKILLS-LV5 — the gate, the checker, the count, and the corpus search

**60 = 131 − 10 − 61.** Exclusions: `HAND_ROW` 10, `DURATION_ONLY` 41, `PERCENT_SLOT` 1,
`CONDITION_ON_EPISODE` 2 (113 2136), `EPISODE_REFUSED` 1 (926), `PET_ATTACK` 1,
`UNMODELLED_CLASS` 5, `RECIPIENT_NOT_A_FOE` 5, `RECIPIENT_NOT_AN_ALLY` 3,
`HEAL_RECIPIENT_CLASS` 2. Marks: `CONDITION_RIDER_ON_HIT` 2, `KNOCKDOWN_APPLIED` 5,
`CLAUSE_KNOCKDOWN` 2, `CONDITION_FLAT_CONSTANT` 1, `CONDITION_BIT_CLEAR_REFUSED` 1,
`AREA_CASTER` 6 (1041 joined), `CLAUSE_UNBLOCKABLE` 1, the rest as §59.4. **49 of 60 rows
marked; the eleven unmarked are §55.3's eleven.** By type: Spell 35, attack 7, Stance 5,
Skill 4, Hex 3, Signet 3, Preparation 2, Enchantment 1 (1997 — the tier's first). The
census with the fresh overlay: 56 modelled, 60 label-only, 408 episode-only, 45 refused,
764 nothing.

**The checker** (`check_label_rows`, the vacuity guard) names: a `condition_rider` row
without its mark, on a non-episode type, on an episode that never opens, whose slots'
sentences are not on-hit riders or whose `rider_weapon` is not the sentence's class; a
condition means on an episode type shipped WITHOUT a rider field and without
`AREA_CASTER` (or with a rider / governed sentence — 2136 forced in with `AREA_CASTER` is
named); `knocks_down` without knock-down wording, on a non-foe kind, a timed record (192
forced), a projectile, non-foe recipients, or without `KNOCKDOWN_APPLIED`; the mark
without the field; a knock-down clause neither applied nor marked; a
`CONDITION_FLAT_CONSTANT` mark that does not match a bit-clear equal slot (or
`CONDITION_BIT_CLEAR_REFUSED` a differing one); and a field outside the three new ones.
Eight synthetic arms and five corpus arms (435 / 187 / 167 stripped of their marks; 926,
2136, 192, 3425 forced) each name their fault (`test_skilldesc` §1b, §3).
`content.LABEL_DETAILS_KNOWN` gains the four tokens, so a tree older than this pass DROPS
the rows that carry them at load (§59.7 #3's guard) — 187 231 294 784 1086 167 435 1997
1041 are not served by such a tree, which is the conservative side.
...
it); this pass's emit is staged beside its tests, and a tree older than it drops the
...
- **Changed: §55.2's `CONDITION_ON_EPISODE` is three classes**, and two of them have
  consumers. The honest count of the plain set the server acts on is 60 of 131. Nothing
  here moves the 27 % (§54.4).
- **Refuted, small: "a condition rider on episodes 6" (§59.6).** It is 2 riders + 1
  activation; 926 cannot ride anything on this build and 113 / 2136 wait on an on-struck
  consumer (and 113 on an area-of-allies enchantment and an immunity besides).
- **Refuted: "the marked clauses (7 knock-downs on label rows first)".** Five become the
...
- **Open, per skill (the residue, in order of cheapness):** 784's always-Poison land
  ...
  943's conditioned heal and 1262's "creatures"; the 210 conditional SERVED rows; the two
  knock-downs left marked (192's shower over time; 3425's foe state); 840's self-Poison;
  1113's four ticks; 1033's Deep Wound (a tooltip run); then the 924 RECOGNISED and §54.4's
  two-consumer proposal (the owner's call).
- **Open, route:** §55.5's, §59.5's and §60.6's confirmation runs; step 5 (the 68 no-slot
  rows); step 6 (the timed effect types); the 141 INDETERMINATE slots; a body chain (§59.7).

**What would refute this section.** A capture of 435 or 1997 whose condition lands at the
CAST, or on a hit by a weapon outside the class the sentence names, or on a hit that
