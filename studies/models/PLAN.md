...
| **M4** ✅ | Blender: real meshes replace proxies, instanced per file id, transform/scale/z-sign settled by rendering against terrain | **DONE 2026-08-13, criterion met.** Both maps render recognizably — Kamadan as crenellated walls, palms, awnings and a domed building; Pre-Searing as forested hills with a bridge. Instanced per file id: **516 props over 71 datablocks** (Kamadan), **864 over 152** (Pre-Searing), none shared across different ids. Props whose model does not decode keep the measured proxy, so nothing is lost (200 of 864). **The z sign is MEASURED, not assumed** — negating model z scores 73.2%/83.3% for geometry-above-terrain against 23.8%/6.5% for leaving it stored, and 0.961 vs 0.032 at object level off Blender's own meshes; the diagnostic is that a model's z runs +34 → −642, upward from its base once negated. Test floor 84 → 92. The other named unknown, which of bits 12/13 is tangent vs binormal, did **not** need settling — nothing lights the meshes yet — and moves to M5. | 1 session |
| **M5** ✅ | Textures: FA1/FA5 -> `atex.py` -> images -> Blender diffuse materials; the ten DDS get a stdlib reader | **DONE 2026-08-14, and it split into a result and a refutation.** `0x00000FA5` is decoded (857/857 corpus chunks close, eight rivals 0/315) and every reference resolves to a texture MAGIC, 1,795/1,795. The upstream "FA1 is also texture filenames" claim is **REFUTED** (1 hit in 89,013 windows vs FA5's 1,795/1,798). Full-resolution export: **472 distinct PNGs, 28.0 MB**, zero errors, stdlib-only via `png.py`; the 10 DDS are one uncompressed A8R8G8B8 file and are read. Blender gets UVs and one material per image. **What is REFUTED is the DIFFUSE binding**: the per-sub-model index is a real index (97.4% in range, varies on 208/210, `u2` eliminated) but a render puts specular maps on buildings, and `ntex/nsub` is non-integer on 146/316 so there is no fixed grouping either. The chain almost certainly runs through AMAT (`0x00000FAD`) which is undecoded. test floor 48 -> 58. | 1 session |
| **M6** ✅ | the 15% preamble decode | **DONE 2026-08-13, and it over-delivered.** The sub-model array is now COMPUTED, not searched: `MdlLoad.cpp`'s parser (`0x007952A0`) sets a cursor to `+0x54` and advances it through six gated variable-length blocks, every size from a header field. **20,661 of 20,661 geometry chunks in the archive close on the exact final byte.** The ~15% failure had nothing to do with the preamble — the client's stream CONTINUES past the collision meshes with three more blocks (H/I/J), so a walk requiring closure there could never close; of 931 old failures, 930 carry one. **Kamadan 474 → 516 of 516 props with real geometry, Pre-Searing 664 → 864 of 864, zero proxies on either.** The ambiguity is gone by construction (0x25AA9's rival parse never arises). Nine one-term sabotages all reduce closure. The retired search is reproduced live in the test so "newly readable" is a difference between two answers. | 1 session |
| **M6b** | the `f11` mystery and the flags-2817 stream census | **The f11 question SHARPENED rather than closed.** On the props the retired search could already read, the oracle is untouched — 474/474 and 664/664 — so M6 recovered models without moving it. All 24 new disagreements sit in the newly recovered models (6/42 Kamadan, 18/200 Pre-Searing, ~10%), which is a rate well above the corpus ~0.7% and is now the sharpest open question in the arc. The two populations are pinned APART, per §A5. | open |
