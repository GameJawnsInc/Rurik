...
| `authsrv.py` spawn burst | sends property 43 on `0x00A3` (float-target) | retail sends it on `0x00A2` (plain float). Left alone for now — MORALE-Q3 *(since moved: every send is `0x00A2` as of 2026-08-22, `b788ac1` — see §6 Q3 and RUNS.md §Run 4)* |
...
| MORALE-Q3 | `0x00A2` vs `0x00A3` for property 43 | This row's original text — "the client has never complained" — died with RUNS.md §Run 3's energy-readout drain, seen in the `0x00A3` tree. We moved to `0x00A2` on 2026-08-22 (`b788ac1`), Run 4 shows the drain gone on `main`, but the fix rode with the death/revive regen change and the two are unseparated. The separation arm is designed in Run 4 and **ruled not owed** — both changes are retail-pinned independently, so no answer reverts either. Open as a client-behaviour question only |
