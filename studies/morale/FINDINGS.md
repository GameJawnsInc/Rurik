...
| MORALE-Q6 | Does DP survive a map change on the wire, and what resets it? | ~~our corpus has no death followed by a zone.~~ **HALF ANSWERED — the OUTPOST half is OBSERVED (re-counted 2026-09-24, DESKWORK-Q1).** Of the 36 live captures, 7 connections carry a `0x009C` below 100 on the observer's own agent (property 41 names it, `moralescan`'s rule); 3 of them have a next hop in `tape.chain`, and all 3 zone into an outpost that loads at 100 on both `0x009C` and `0x00E9` field 10 at ~0.6 s: `20260913T210901` map 212 → 242 (85 → 100), `20260914T005758` 430 → 449 (71 → 100), `20260916T150306` 430 → 449 (85 → 100). The server's clear ships from the second (`zone_carry_apply`, slice F40 row 7, `--no-zone-carry`). **Still open: a FIELD-to-field zone** — no witness in the corpus, so whether a penalty survives one is WIKI only. (The sentence struck through was true when written.) |
