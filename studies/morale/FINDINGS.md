...
| MORALE-Q6 | Does DP survive a map change on the wire, and what resets it? | ~~our corpus has no death followed by a zone.~~ **HALF ANSWERED — the OUTPOST half is OBSERVED, n = 2 (re-counted 2026-09-24, DESKWORK-Q1; the witness count corrected the same day by the lane's fix pass, which read the morale LIVE AT THE ZONE, the last own `0x009C` before the connection ends, rather than the minimum).** Of the 36 live captures, 7 connections carry a `0x009C` below 100 on the observer's own agent (property 41 names it, `moralescan`'s rule) and 3 of them have a next hop in `tape.chain`. In **2** the penalty is still live at the zone and the next map, an outpost, loads at 100 on both `0x009C` and `0x00E9` field 10 at ~0.6 s: `20260914T005758` 430 → 449 (71 at the zone → 100) and `20260916T150306` 430 → 449 (85 → 100) — the same map pair twice. The third, `20260913T210901` 212 → 242, does **not** witness a zone clearing a penalty: its morale fell to 85 at 318.820 s, climbed back and read 100 at 618.056 s (a +10 step from 90, in the same batch as a 2,000-experience `0x00EE` field 0), 177.5 s before the connection's last message at 795.598 s. The server's clear ships from `20260914T005758` (`zone_carry_apply`, slice F40 row 7, `--no-zone-carry`). **Still open: a FIELD-to-field zone** — no witness in the corpus, so whether a penalty survives one is WIKI only — and the outpost half rests on one map pair. (The sentence struck through was true when written.) |
