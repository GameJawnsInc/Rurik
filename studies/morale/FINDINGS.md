...
| `authsrv.py` spawn burst | sends property 43 on `0x00A3` (float-target) | retail sends it on `0x00A2` (plain float). Moved 2026-08-22 (`b788ac1`, RUNS.md §Run 4); MORALE-Q3 then measured the two channels equivalent (§2.5), so the move is fidelity, not function |
