...
**FILED 2026-08-14 (rung S11), and the filing changed one of the three confidences.** All six of this arc's opcodes are now named in `schema/overrides.json` and resolve through the codec — `0x002B` COMPASS_DRAW (medium), `0x0091` COMPASS_DRAW_BROADCAST (high), `0x0092` COMPASS_PING (medium) were already there from §4.1; S11 added `0x008A` **MAP_EXPLORATION_INIT_DATA** (high), `0x008B` **MAP_EXPLORATION_INIT_BEGIN** (high) and `0x008C` **MAP_EXPLORATION_MARK** (**medium**).

**Why `0x008C` is medium while the other two are high, measured rather than felt.** Applying §4.1's own test — *is the operative word ArenaNet's?* — `asserts.py --grep '(?i)explor'` returns **0 of 19,758** readable sites, and `'(?i)fog'` returns **0** too. **The client never says "exploration" and never says "fog"**; those are GWW's player-facing words and ours, exactly the finding that put `COMPASS_PING` at medium. What the binary *does* say is `accumMapInitDims` / `accumMapInitOffset` / `accumMapInitData` (`ChCliApi:77`, `:1641`) and `mapBits` (`:207`, `:226`). So `0x008A`'s and `0x008B`'s operative tokens **are** ArenaNet's and only the `MAP_EXPLORATION_` prefix is ours, whereas `0x008C`'s label is ours in **both** halves — `mapBits` is the noun it writes, but neither `MARK` nor `EXPLORATION` appears anywhere in the binary. **The MECHANISM is CORROBORATED at all three; the NAME is not equally earned at all three**, and the `name_confidence` field is about the name.

**Two filing details worth carrying.** `0x008A` and `0x008B` are **name-only** rows — no `fields` key — so the codec *merges* them over `messages.json`'s imported layout instead of replacing it; duplicating a layout into a naming row is the copy that `test_codec.py` refuses and that PLAN S11 warned could be wrong. `0x008C` already carried the 11-byte shape correction from before this arc, so naming it required adding `("GAME_SMSG", "140")` to `test_codec.py`'s `LAYOUT_FIXED` — and it is the **strongest** of that list's three exemptions rather than another concession: `0x0092`'s correction rests on the binary alone (0 live occurrences), while `0x008C` has **47** and its fields are read back off the wire.
...
- ~~**The authored tile is a PATTERN, not our terrain. STILL OPEN, and it is now the only open item in Tier 3.**~~ **CLOSED the same day by §6g.5 (A3)** — `tilerender.py` shaded the sculpt area's own 64x64 heightfield into tile 116842 and map 143's compass drew it (50.7 % against the pristine control's 0.1 %, two luminance levels 34 apart), so a top-down renderer DOES exist in `toolkit/` and the sentence below is superseded; left as written because §6g.5 sits above it in this file and the order hid the closure (DESKWORK-Q1 note, 2026-09-24). A1 proves the client renders art we wrote at a slot we chose; it does not render the authored map's own heightfield. Generating the tile *from* our terrain is a further step nothing here attempts, and nothing in `main` as of 2026-08-17 supersedes it — the 311 commits since Tier 3 landed went to models, skeletons and units, and **no top-down/hillshade renderer exists anywhere in `toolkit/`**. The pieces that would feed one do: `mapexport.py` already emits `heights.f32` at one sample per cell, which is the compass's own scale (§6b.2).
- ~~**One tile, one map, one archive copy** … no run has looked at one.~~ **CLOSED — the collateral is now MEASURED, statically and for free, which is what the old wording missed by assuming only a run could answer it.** Sweeping all 888 area rows for world-1 footprints overlapping tile (1,0)'s cells (x 512..1023, y 0..511) returns **exactly 6**: maps **143, 147, 149, 150, 151, 779**. Five of them are neighbours of the map we authored for, and **149/150/151 carry byte-identical rects to 143's** `(960, 448, 1280, 992)`, so their compasses would have shown the *same* crop of our checkerboard; 147 and 779 share `(768, 192, 1312, 672)`. The blast radius was five extra maps, confined to one throwaway archive that no longer exists. — OBSERVED.
- ~~**The 0.1 % floor is not zero and is not explained.**~~ **CLOSED — the residual is ordinary terrain, and the hypothesis the bullet offered is confirmed by measurement rather than left as a guess.** Re-measuring the A2 disc: **24 pixels** pass the loose `g − max(r,b) > 40` test, but only **6** lie within 60 per channel of the authored green `(0, 220, 60)`, and their median is `(51.5, 162.5, 40.5)` — warm, desaturated, nothing like our tile. The A1 disc separates cleanly on the same test: **6,071** greenish of which **5,552 (91.4 %)** are within 60 of the authored green, median `(0, 249, 67)`. **Magenta — the unambiguous half — is 3,656 in A1 and exactly 0 in A2.** So the floor is terrain passing a permissive filter, not tile surviving. — OBSERVED.
...
- ~~**`[CompassMap+0x80]` is UNREAD.**~~ **CLOSED 2026-08-14 by rung S12 and this bullet was simply never updated** — §6c.4 records it: `+0x80` is **not** an input to the tile getter but a **cache generation stamp**, written into the per-slot record at `[ebx + esi*8 + 0x14]`, zeroed by the constructor at `0x008C1C6B` and bumped by `0x008C2A9E inc dword [edi+0x80]`. — OBSERVED. *(Two sections of one document disagreeing for a day is the failure the top of `CLAUDE.md` is about, caught here by re-reading §7 rather than by any check.)*
- ~~**`0x00470F50` / `0x00470B60` … carry hypothesis 2's one named residual**~~ — **the residual is SPENT, closed 2026-08-14 by rung S13 (§6e), and not by disassembling either function.** It supposed "a size- or id-dependent refusal that would fail on a ~110 KB atlas tile while succeeding on the 1,936 B fallback". The refusal is real and it is **id-dependent**, but it happens **upstream of both functions**: the archive index never yields a row, because the plain tile id is not bound. Disassembling them is no longer on the arc's path — though it is still the only way to answer the *size* half, which nothing has tested.
