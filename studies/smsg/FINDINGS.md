...
**So the experiment is now specified rather than blocked**: send `GAME_SMSG 0x016E` with payload byte `0x04` to set the latency bit, toggle the graph frame with the key, and read the number. The server half is ours; the keypress is the part still to be identified at a running client.
