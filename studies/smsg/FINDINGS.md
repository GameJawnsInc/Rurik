...
**Still open:** What the 8-dword record's individual slots mean. Nothing on this path asserts on them; the readers are elsewhere in ChCliApi and were not chased.; ~~Whether field 4's 100/120/150 is a scale percentage. A probe that sends one value and photographs the resulting model size would settle it in one launch, and it is cheap because our server already sends this message.~~ **SETTLED STATICALLY 2026-08-22, no probe needed** (`studies/smsgnames` §9): the reader was chased — getter `0x0080CD00` reads the per-agent override at char-record+0x30 and falls back to THIS record's slot +8; caller `0x0082F520` (CpsMonster.cpp) takes the value >> 24 and multiplies by f32(0.01) from `.rdata 0x946e58`, applying it to the composite unless the byte equals 100. The top octet IS a scale percentage, by the client's own arithmetic. The per-agent override message is `0x009A`, now named `AGENT_SET_MODEL_SCALE`.; Whether element+0x18 (the hole) is written by some other message.
...
**SETTLED 2026-08-22, and the name is EARNED as `ITEM_HIGH_DETAIL`** (`studies/smsgnames` §9, `schema/overrides.json` 353, invariants `test_itemdetail.py`): the shared builder `0x848450`'s field writes are read (store offsets for every field 1-9), the installer sets the bit `ItCliApi:66` asserts as `IsDetailHigh()`, field 14 is DECLARED AND NEVER SENT (0 of 2,235 -- the format table has a slot retail never fills), and ItemCode:516 turns out to REFUSE the terminator on the wire (`!count || code[count - 1] != ITEM_CODE_TERMINATOR` -- the client appends its own; 0 of 6,806 corpus code words carry it). Field 4-7 SEMANTICS are still unnamed by any assert; the offsets are what the binary gives.
