...
**The latency readout therefore stays unread, and the unit stays INFERRED** -- but the remaining obstacle has moved from "we do not know what toggles it" to one named, located, keyboard-shaped problem. `--netgraph` on `authsrv` sends the byte whenever the other half is solved.
