...
✅ **THE ARM IS BUILT AND RUNNABLE, 2026-08-21.** `python toolkit/authsrv/authsrv.py --zero-lead …`; the prediction below is printed verbatim at startup so it cannot be rationalised afterwards. It REFUSES to combine with `--heading-grant` or `--client-endpoint` (same arm, both refuted) and allows `--grant-suppress` and `--resync` with a printed note each. The rate limit is `_heading_grant_ok` — rule 2 only, at `GRANT_MIN_INTERVAL = 0.5 s`, sharing the server's one grant clock — and a refused heading grant is DROPPED, never held. **What has not happened is the run.**
