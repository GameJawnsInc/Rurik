...
> ⚠ **CORRECTED 2026-09-23 — the text of §2.1 and §2.2 stands as written; read this first.** The 0.999920 s / 0.999777 s above is how much of the second the rows covered, not the offset's error. With a steady clock the truth lies in `[max, min + 1 + one tick)`, so the error bound is **1 s + one tick − spread**, and a spread near 1 s is the TIGHT case. On these two captures (`20260821T081744`, `20260821T082631`) that is **15.705 ms = 4.52 u** and **15.848 ms = 4.56 u** with the coarsest tick (OBSERVED), not 288 u. Full record, the measurement it rests on, and the list of conclusions that leaned on 288 u: §5, T1's note "SETTLED 2026-09-23 (latest)".
...
> ⚠ **CORRECTED 2026-08-21: this paragraph first read "200 rows, offset spread 11.4 µs = 0.0033 u" and that point estimate does not reproduce.** It is a live sample of scheduling jitter between two adjacent clock reads, not a deterministic figure: independent runs of the same check give 12.4, 14.3 and 31.7 µs, a 2.8× spread.
>
> ⚠ **CORRECTED AGAIN 2026-09-23: the max−min is the scheduler, and `test_movesync` §21 no longer asserts it.** OBSERVED. `event()` reads `perf_counter` first and `time.time()` after it, with the `wall` `strftime` between them, so each row's `wall_unix − t` is the offset *plus* the time between the two reads, never minus it; a preemption in that gap lifts one row. Over 1,000 runs of §21's own 200-row loop (500 idle, 500 loaded; a scratch probe, not committed): **median−min 0.48–3.81 µs in every run**, idle or with 16 busy processes on 12 cores; **max−median** p50 9.3 µs idle / 25.3 µs loaded, max **0.50 ms** idle / **79.6 ms** loaded, always one to six isolated rows, always upward. §21's `max − min < 1 ms` failed 86 of 500 loaded draws, and its `> 100×` ratio 23 of 500 idle and 115 of 500 loaded. That was the red at `73e3d78e` and `57ae1dbe` (0.025 s / 14×, 0.010 s / 59×, 1823.2 µs), on an estimator bit-identical under four hash seeds. The vault's newest 400 server captures show the same shape: max−median up to 23.2 ms, 6 of 400 over 1 ms. Their median−min is 16–36 µs over runs of minutes. §21 now reads **median − min**, how far the float arm's median sits above the least-delayed row, against the same 1 ms and 100×, with a control that plants an 80 ms preemption (not moved) and a 3 ms drift (red). Reading the two clocks back to back, with the `strftime` moved out of the gap, was measured too: typical max−min 10 → 1.4 µs, loaded failures 86 → 7 of 500. That shrinks the window and cannot close it, so the server was not changed. **For readers:** `offset_line`'s printed "residual" is max−min. On a live capture that is the worst single preemption, not the clock term; the median it pairs on is unaffected. The row count was also wrong by one — the printer itself says 201 (the reports plus the origin row `Recorder.__init__` emits) and the test asserts `n == len(reports) + 1`. **What survives is the claim §2.2 actually needs**, `< 1 ms (< 0.3 u)`, by ~30× on the worst draw seen; what does not survive is quoting a single draw as a measurement. The two synthetic/vault figures beside it (0.977 s, 1.000 s) are deterministic and reproduce exactly. This is the "error bars before conclusions" rule, and the fix is to quote the range.
>
> ⚠ **RELABELLED 2026-09-23 (later): the printed line now says which half is which.** `offset_detail` gained two keys on the float arm, `clock_residual` = median − min and `worst_delay` = max − median (both None on the truncated arm), and `offset_line` prints them under those names, the delay in ms only because the median absorbs it. On `20260912T223259-c5` it reads `clock residual 0.004 ms = 0.00 u … worst single-row delay 0.035 ms`, where the old line said `residual 0.039 ms`. `spread` (max − min) and `offset_from_stamps`' two-value return are unchanged, so `pair`, `resyncscore`, `grantsim` and `test_movesync` §1 read what they read before. The "For readers" sentence above describes the line as it printed until this change. `test_movesync` §21 checks the new labels on a fixture where the three numbers differ, and checks the real `Recorder`'s `clock_residual` against its own clock term. **The truncated arm's line has the same kind of label, and it is left alone here.** OBSERVED on the 396 vault server captures that carry `wall_unix` on every row (≥ 50 rows each; a scratch scan, not committed): the truncated max-estimator sat **0.1–17.3 ms below the float median** (p50 0.73 ms, never above it), while its max − min, which the PRE-REALFIX-T1 line prints as "residual", read **0.979–1.001 s**. With a fixed offset the truth lies in `[max, min + 1)`, so a spread near 1 s puts the max *near* the truth, not 1 s from it. The same scan gave the float arm's median − min as 3–36 µs and its max − median up to 23.2 ms. Whether §2.1's "1.00 s / 288 u" therefore overstated the clock slop on pre-T1 pairings is **open, not settled here**. Those captures have no float stamp to check against, and the truncated line is quoted byte for byte in PROBE-GATEFIRE.md block 12, so relabelling it is its own change. One smaller label on the same line, also left: a mixed file's note prints `n_trunc` as the rows that "carry only the truncated stamp", but `n_trunc` counts every row with a `wall`, float rows included (§21's mixed fixture: 40 printed, 38 true).
>
> ⚠ **SETTLED 2026-09-23 (latest): yes, §2.1's "1.00 s / 288 u" overstated pre-T1 clock slop, by about 64× on the two captures it was measured on.** The note above left this open; `toolkit/clientscan/test_truncbound.py` now makes the scan a committed check, and §2.1's text is left as written, with a pointer under it.
>
> - **The arithmetic (OBSERVED on synthetic rows, `test_truncbound` §1).** A truncated row gives the true offset minus the fraction of its second, minus the lag described next. So the max-estimator approaches the truth from below, and with a steady clock the truth lies in `[max, min + 1 + one tick)`. The error bound is **1 s + one tick − spread**. The spread is how much of the second the rows covered. A spread near 1 s is the tight case, and a short capture's small spread *understated* its error: a 0.3 s fixture has a 0.6 s error.
> - **The clock `wall` is read from (OBSERVED, this box, Python 3.14).** `time.gmtime()` with no argument does not read `time.time()`'s clock. It floors `GetSystemTimeAsFileTime`, which matched on 1,748,920 of 1,748,920 bracketed reads. That clock advances once per timer interrupt, so a `wall` second rolls over LATE: 782 of 4.1 M back-to-back reads were late, none early. `NtQueryTimerResolution` reports an interval of 0.5–15.625 ms (1 ms current). This is why the bound the earlier note gave, `[max, min + 1)`, was **refuted as written**: on the both-stamp captures, 1 − spread alone fails on 191 of 511 by up to 1.475 ms, and 36 have a spread above 1 s while the max stays below the truth. `movesync.WALL_TICK` is the coarsest interval, 15.625 ms.
> - **Scored on real rows (OBSERVED, `test_truncbound` §2, 511 both-stamp captures of ≥ 50 rows, pinned at `20260923T154308`).** The truncated max sat **0.105–17.262 ms below** the float median and never above it. It was inside `bound` on every capture by at least 14.150 ms, and none was refused. The latest late rollover was 1.474 ms, and no row's `wall` second ran ahead of its `wall_unix`. The float offsets' lower envelope moved at most 0.024 ms from the first tenth of rows to the last, so the clock held steady. The old line's "residual" exceeded the measured error by at least 56.7×. Known-bad arms: a mean estimator breaks the bound on every capture, 406–640 ms below the truth. A planted 50 ms drift reddens the steady-clock check on every capture and lifts the max above the truth on 516 of 518 (the whole corpus that run, pinned plus newer). Against the shipped code, a mean estimator reddens 7 checks, a bound without the tick 6, and the old label 4.
> - **§2.1's own two captures, and block 12's.** OBSERVED: `20260821T081744` bound **15.705 ms = 4.52 u**, `20260821T082631` **15.848 ms = 4.56 u**, `20260819T145717` **16.095 ms = 4.64 u** (PROBE-GATEFIRE.md block 12, re-derived). RECONSTRUCTION: if the timer ran at the finer interval the both-stamp corpus shows (late rollovers ≤ 1.474 ms), the first two bounds are ≈ 1.55 ms ≈ 0.45 u. The tick in force on a pre-T1 capture was never recorded, so the printed figure takes the coarsest. The overstatement is therefore ≈ 64× on the conservative bound and up to ≈ 640× on the reconstruction.
> - **The pre-T1 corpus (OBSERVED, `test_truncbound` §3, 1,034 captures of ≥ 50 rows).** Bound p50 17.130 ms (4.9 u), p90 23.092 ms, max 715.861 ms (a short capture). 24 have a spread above 1 s by less than a tick: the lag regime, bounded. **Two are refused**, and for them the old reading *understated*. Each splits into two steady runs one clock step apart, visible in the truncated stamps alone. `20260810T151946` steps −1.36 s at row 1,251 of 1,276. `20260817T011449`, a 6.7 h capture, steps +1.25 s at row 208,270 of 498,761. Rows on one side of the step pair ~1.25–1.36 s ≈ 360–390 u off. A single row lifted 1.3 s is refused too, and it does not split, which is the control.
> - **What this does not settle.** That the bound is right on a pre-T1 capture is the inference from the both-stamp corpus, on the same `Recorder.event` code path. There is no float stamp to check it against. A preemption between `event()`'s `perf_counter` and `gmtime` reads could lift one row above the truth; none has in 511 captures (UNVERIFIED on pre-T1 ones). The bound covers the offset only. §2.2's transport and sample-phase terms were always there and do not move.
> - **The instrument.** `offset_detail` gains `bound` (1 s + `WALL_TICK` − spread on the truncated arm, None on the float one). `offset_line`'s truncated arm prints it, `bound +16.095 ms = 4.64 u … the spread is how much of the second the rows covered, not the error`, and prints **NO BOUND** with the least step the stamps imply when the spread exceeds 1 s + one tick. The mixed-file note now prints `n_trunc − n`, 38 on §21's fixture. `authsrv.Recorder.event`'s comment no longer calls the spread 288 u of slop or a drift diagnostic. In `test_movesync` §21, two checks that scored the truncated spread as the error now score `bound`, two messages stop calling it one, and a new check pins the mixed note's 38.
>
> **Conclusions that leaned on the 288 u figure — listed, NOT rewritten** (line numbers as of this commit). Each rests on a clock term that was ≤ ~16 ms = 4.6 u on the captures involved.
>
> - LOAD-BEARING. `studies/movement/FINDINGS.md` §"2026-08-21 — REALFIX-L1 FIRST RUN" (:4017–4022): P2's p50/p90 separation misses were recorded as **UNDECIDABLE-leaning-miss** because of "a 1.00 s spread, worth up to ~288 u on any single pairing". That systematic was ≤ 4.6 u on 081744/082631, so the misses stand or fall on the other terms. Not re-scored here.
> - → **RE-SCORED later on 2026-09-23** (`studies/movement/FINDINGS.md` §"2026-09-23 — REALFIX-L1 RE-SCORED", `test_truncbound.py` §4). **p50 ≤ 150: MISSED and DECIDED**: 267.5 u at every offset the bound admits, 256.1–262.4 u with movetap interpolated to the instant and the transport term; the old whole-second reading reached 83.8 u, which is why it could not be decided before. **p90 ≤ 520: AT the bound, leaning met**: 515.8–523.0 u nearest-sample, 514.4–518.6 u interpolated, under 520 at every offset but by less than the interpolation's own error.
> - LOAD-BEARING. `PLAN-LOG.md` §"REALFIX-L1 FIRST RUN, ⚠ OPERATOR-REFUTED" (:8044): the same verdict, "~1 s movetap↔wire clock systematic (≤ ~288 u/pairing)".
> - LOAD-BEARING. This document's L1 paragraph in the P2 arm block (:1799), "a ~1 s clock-offset systematic worth up to ~288 u per pairing", is part of why that run's separation bounds were read as missed-as-written.
> - LOAD-BEARING. This section, §2.1–§2.2: the table's "vs **1.00 s / 288 u** today". The pre-T1 clock term was ≤ ~16 ms, already below the movetap sample phase it calls binding (50 ms at 20 Hz). So T3's "still 11× better than today" and "`--hz 20` is a *requirement* of the clock fix" rest on the 1 s figure. T1's per-row stamp is still an improvement, from milliseconds to microseconds, and it made the offset per row rather than bounded.
> - LOAD-BEARING, a sequencing decision. §5 row 1 ("blocks everything wire↔tap"), `studies/movement/FINDINGS.md` §"2026-08-21, round 6 … 5. Where the arc goes" (:4256–4258) and `PLAN-LOG.md` round 6 (:8038) all say "T1 float wall stamp kills the 1.00 s offset spread". The instruments-first order rested on it.
> - LOAD-BEARING, in a test's rationale. `TESTS.md`'s `test_movesync` entry, in its §21 paragraph ("§21 IS THE CLOCK ANCHOR"), says a fallback "is how a 1.00 s error survived two whole analysis lanes", "~1 s residual by construction", and "six rows … would have understated the defect". The code comments behind it are corrected in this change. The entry stays as the record.
> - CAVEAT. §2.3 treats "offset = min" (every lag 1 s later) as a live branch excluded only by `ping_summary`. With a spread near 1 s that branch cannot occur: truncation puts every row at or below the truth, so the truth is at or above the max, and the min sits ~1 s below the max.
> - CAVEAT. `studies/skills/FINDINGS.md` (:5848): arm B's missed heal number is put down to cadence with "±1 s alignment on the capture's wall stamp". This is a wire↔screenshot join on a `20260909` capture, which is post-T1 and carries `wall_unix`.
> - RESTATEMENTS, no conclusion rests on them. `studies/movement/FINDINGS.md` :4310 ("the 1.00 s offset spread that cost round 6 its alignment") — round 6's own table was anchored on the `+0x80` flip. Also :4455 ("round 6's 1.00 s slop"), :4530, and `PLAN-LOG.md` :8030 and :8034. This section's T1 paragraph ("1.000 s / 288 u — §2.1's figure, reproduced by the shipped printer") was true of the printer, and the printer was wrong. PROBE-GATEFIRE.md block 12 quoted the old line and is re-derived in this change.
