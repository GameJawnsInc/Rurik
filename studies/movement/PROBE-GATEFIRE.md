...
(`SELFTEST_FLOOR` at `movetap.py:1481` and `movesync.py:1182`), so any net loss of a check
...
`sorted(glob.glob(pattern), key=os.path.getmtime)[-1]` (`movesync.py:706`) over that shared
...
clock offset 1787165837.301 from 8393 truncated `wall` stamp(s) [PRE-REALFIX-T1, max-estimator]: bound +16.095 ms = 4.64 u at 288 u/s -- the true offset lies at most that far above it if the clock held steady (1 s + one 15.625 ms clock tick - spread 0.999530 s; the spread is how much of the second the rows covered, not the error) -- this capture predates the float stamp
...
(`movesync.print_wire_only`, `movesync.py:1017`), not a subset of the paired one. An operator
