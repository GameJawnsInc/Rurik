...
parser (`movetap.py:4289-4296`) is `[-h] [--pid PID] [--hz HZ] [--seconds SECONDS]
...
are the `+0x48` change line and the fence-transition line above (`movetap.py:4422` and
`:4433`). A transition that involved a failed read is additionally suffixed
...
1. It printed an `ALIASING: phi 0.011 over 558 usable pairs, p(open) 0.287, white 0.409,
   ...
   COMMENT (`movetap.py:3381`, the word "Whitespace-normalised") -- **no printable string in
...
**tally dicts**, not integers — it calls `.get(True, 0)` on them at `movetap.py:4836`. A
...
(`movesync.print_wire_only`, `movesync.py:1496`), not a subset of the paired one. An operator
...
|---|---|---|
| `movetap` refuses to start: `unrecognized arguments: --block` (or `--note`) | You copied the pre-2026-08-20 step 10. Those flags never existed | The real parser is `[--pid] [--hz] [--seconds] [--selftest] [--any-build]` (`movetap.py:4289-4296`). Record the block letter by hand against the file stamp |
...
**`movetap.py:512`** — the previous draft cited `:217`), return 1. Neither was in the
...
4. RESIDUE 3 (movesync, already self-reported by that lane's fixer). `print_appender_witness`'s LIMIT 1 sentence prints four numbers nothing reads back: `Spacing p50 {p50:.3f}s, max {dmax:.3f}s; {far} of {len(dts)} judged pair(s) span 2.5s or more.` (movesync.py:1113). `far` -> `len(dts)` and `p50` -> `max` are both GREEN. These qualify how strongly a ZERO in arm (a) reads. Arm (a)'s and arm (b)'s counts AND both denominators are pinned (`_WITNESS_FIELDS` covers them, and substituting `judged` for `reach_pairs` in either position is red), and arm (b) is the aliasing witness §6 block 11 actually reads for the polling-vs-hook decision.
