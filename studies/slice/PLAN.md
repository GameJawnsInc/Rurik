...
| **SLICE-B7** | **Hero follow and hero casting.** The long pole, and the only item here that is new logic rather than re-pointing proven plumbing. Three sub-parts: a follow tick for an allied agent; a cast path that is caster-parameterised rather than player-only; and `allies_of` learning that the player HAS allies. Everything downstream of it — the bar delivery, the panel, the profession — is already proven | an arc |
...
| **SLICE-U2** | What each loadable creature shell DRAWS | ~~The model parade — registered, not yet run~~ **RAN 2026-09-11 and SCORES: [FINDINGS.md](FINDINGS.md) SLICE-F7, [RUN-PARADE.md](RUN-PARADE.md) §4a.** 15 of 15 legible, control drew its box, fifteen nameplates, ten joined to a template on evidence. **The slice's pair is Bandit Raider + the Academy Monk body spawned hostile** | Most render a recognisable creature; a minority draw the white untextured box that a withheld composite produces, and that failure is self-announcing. **HELD: 14 creatures, 1 box, and the box was the one we withheld** |
