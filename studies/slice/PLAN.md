...
| **SLICE-H4** | **The hero fights.** The engagement rule (F28): a party body opens on the leader's target when the leader starts or presses, or on whoever hit the party; the monk auto-attacks between casts. `land_skill`'s four `PLAYER_AGENT_ID` damage/effect sites parameterised so a hero's damage skill lands on ITS target; the monk hero's bar grows toward the retail one (Word of Healing 282, Heal Other 286, Healing Breeze 288, Resurrection Signet 2) as `skill_effect` rows exist | TODO |
