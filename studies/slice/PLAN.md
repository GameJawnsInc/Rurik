...
| **SLICE-H6** | **The hero across the loop**: present on both maps through the `0x01A5` re-entry, and the owner's hand-driven pass with the hero: outpost roster → zone → corridor fight → return. A hero-specific live capture (F28's last paragraph) stays optional | TODO |
