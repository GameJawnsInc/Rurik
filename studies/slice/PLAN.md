...
| **SLICE-U2** | What each loadable creature shell DRAWS | **The model parade — REGISTERED 2026-09-11, roster derived, not yet run: [RUN-PARADE.md](RUN-PARADE.md).** 14 bodies, every one resolved against the archive first; the nameplate is half the deliverable, because the client resolves `enc_name` and draws the name for us | Most render a recognisable creature; a minority draw the white untextured box that a withheld composite produces, and that failure is self-announcing |
