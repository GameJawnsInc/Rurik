...
| **SLICE-H3** | **Hostiles fight the party.** `enemy_attack_tick` and the chase aim at the player only; retail's hostiles named a henchman 162 times to the player's 13. A hostile picks its target among the party (nearest body that can be reached, then the one that hit it — RECONSTRUCTION until `henchjoin.py --rows` says otherwise), the chase follows THAT body, the swing lands on it (`hit_enemy`'s ally path, `test_pools` §10b), it dies (`kill_agent`) and stays down; the monk's Resurrection Signet (skill 2, 3.0 s, `[id, 0]`) raises a party body, and the hero raises the player | TODO |
