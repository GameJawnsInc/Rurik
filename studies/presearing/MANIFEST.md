...
**The full player corpus spans 21 distinct `type_code` values. Pre-Searing's 44 reach exactly 9 of them.** The other 12 (9, 11, 15, 16, 20, 21, 22, 24, 25, 26, 27, 28) are attested in the client and semantically UNKNOWN, with one exception: **`type_code 15` is Shout**, confirmed via skill id 316 "To the Limit!" — which WIKI confirms is a *post*-Searing reward, consistent with zero Shouts in the 44. **Naming the remaining 11 is DONE, 2026-08-21, and it did not use this method** — [studies/skills §35](../skills/FINDINGS.md), `toolkit/clientscan/typenames.py`. The client names its own types in a single 29-case switch at `0x004F9DD0` keyed on `type_code - 1`, and **the ten codes named here are the control that validates it**: all ten resolve exactly to the words above, and the same check at index bias 0 or 2 scores zero. So this section's Rosetta work is what made the switch readable rather than being superseded by it. The eleven: 9 Well Spell, 11 Ward Spell, 16 Skill, 20 Pet Attack, 21 Trap, 22 a *computed* name ArenaNet calls a "global skill", 24 Item Spell, 25 Weapon Spell, 26 Form, 27 Chant, 28 Echo. Still unnamed, and unnamed **by the client**: 17 and 18 short-circuit to the null record.
...
9. ~~**Semantic names for 11 of the 21 client `type_code` values**~~ **DONE 2026-08-21** — and not by the method proposed here. The client names its own types; see §8 above and [studies/skills §35](../skills/FINDINGS.md). The ten this document named became the control that proved the switch's index bias.
