...
The prediction was written before the table was re-read (the lane's scratch `prediction.md`): 148/146/242/449 → 4, 248/280 → 8, 55/238 → 6, 143/144 → 4, no prediction for the other served rows. **Re-derived with `toolkit/clientscan/areatable.py --rows 0 --out` on the pinned pristine build 38797** (`s_missionClientData` at VA `0x0096DE38`, 888 records, the structural and the code locators agreeing): the eight named maps read exactly as predicted; **144 read 8, not 4** (the prediction's one miss — an authored slot whose retail row is a type-2 explorable); the rest: 194/474/558/90/167/168 → 8, 27 → 6, 165/166/310 → 4, and **143 → 1** (the Ascalon Academy slot: a SOLO map on retail, `min_party 1, max_party 1`). `min_party` is 1 on every served row but 167/168, whose retail rows (campaign 7, type 3) say 7 — recorded beside `max_party`, and the first cut's "1 everywhere" was caught by the test that re-reads the rows out of the binary, not by a reader. OBSERVED (client-table, build 38797).
...
1. **The per-map cap on an 8-cap map.** `python toolkit/authsrv/authsrv.py --bind 127.0.0.3 --port 6112 --vault vault/captures/gamesrv --map 248 --area outpost_henchmen --party slice` (the other terminals per `RUNBOOK.md`). PREDICTION: the log's `[party] the party cap here is 8: map 248's own AreaInfo max_party …` on the first party click; the panel's counter reads **(2/8)**; three henchman adds go THROUGH (counter 5/8, `size now 3/4/5`) where 148 refused the third. If `--area outpost_henchmen`'s rows are placed on 148 only (the rows' `map`), widen them first or read the cap line and the panel's denominator alone — the denominator is the client's own reading of row 248 and is the half that needs no henchman. Control: relaunch with `--constant-party-cap`: the cap line says 4 and names the flag, the third add is refused (`HENCHMAN_ADD(…) refused … (4: the constant 4 for every map …)`), the panel still says (N/8) — the disagreement the flag reproduces.
2. **The leave, map 148, `--area outpost_henchmen --party slice`.** Add two henchmen (4/4). Take ONE discovery screenshot of the party window for the Leave button's position (the harness: `--shots` with a `shot:1` step after the adds; PtJoin's control sits in the party window's lower strip on retail). Click it. PREDICTION, the wire: `c2s 0x80a2` then `c2s 0x801f [40]` within a millisecond; the log: `PARTY_LEAVE: 2 hired henchman(s) dropped … size now 2`, then `HERO_KICK(40): the client's HEROES bound -- every party hero leaves, [6] …` and hero 6's own `HERO_KICK: hero 6 … size now 1` (`PERSIST: hero 6 kicked` under `--persist`). The screen: both henchman rows gone, the hero's row gone, the counter at **1/4**, the three NPCs still standing, no dialog. Then re-add a henchman (the row back, 2/4) and re-add the hero from Party Search (`HERO_ADD` → 3/4, as the fix pass's runsheet step 5). If the client sends `0x80a2` with NO `[40]`: record it — the outpost gate or the case is not what was read. If the panel keeps the hero's row after `[40]`: the sentinel is not "every hero" on this build's UI and the arm must come out.
