...
| **`0xA2`** | `[]` | `0x0085BEF0` ← `ja` at `0x008585FD` (in `0x008585D0`, PyCliParty:615; the one reference in the image — a conditional tail-jump, which `codescan --xrefs` does not scan; "0 direct callers (pointer-reached)" until 2026-09-25) | 0 (**1 on our client**, `20260913T093718` c1) | LEAVE_GROUP | **PARTY_LEAVE, OBSERVED on our client, CORROBORATED by the read; armed as RECONSTRUCTION** (the leave, below) |
...
the same day — "The kick" below; the LEAVE (`0xA2`) was armed the day after
(desk-partycap, 2026-09-25 — "The leave" below). "No run has sent it", written
here on 2026-09-23, was wrong: `vault/captures/gamesrv/authsrv-20260913T093718-c1.jsonl`
had carried one `0x00A2` (t=29.754946) beside a `0x001F [40]` for ten days, both in
that run's unhandled census.
...
cap). The cap `OUTPOST_PARTY_CAP` was **one constant, 4**, the client's AreaInfo
`max_party` for the maps we serve (148/146/242/449; 248/280 read 8, 55/238 read
6 — a per-map table was the next step, and landed 2026-09-25: "The per-map cap"
below, `content/partycap.toml`, `party_cap`); `--henchman-cap N` (N ≥ 1) overrides.
...
**The per-map cap — the served map's own `max_party`, `content/partycap.toml`, `party_cap`; `--constant-party-cap` reverts (desk-partycap, 2026-09-25).**
The prediction was written before the table was re-read (the lane's scratch `prediction.md`): 148/146/242/449 → 4, 248/280 → 8, 55/238 → 6, 143/144 → 4, no prediction for the other served rows. **Re-derived with `toolkit/clientscan/areatable.py --rows 0 --out` on the pinned pristine build 38797** (`s_missionClientData` at VA `0x0096DE38`, 888 records, the structural and the code locators agreeing): the eight named maps read exactly as predicted; the prediction had **three misses** (the lane's write-up said one; its review, RV-4, counted them against the file): **144 read 8, not 4** (an authored slot whose retail row is a type-2 explorable), **143 read 1, not 4** (the Ascalon Academy slot: a SOLO map on retail, `min_party 1, max_party 1`), and `min_party` on 167/168 read 7, not the predicted "1 everywhere"; the rest, unpredicted: 194/474/558/90/167/168 → 8, 27 → 6, 165/166/310 → 4. `min_party` is 1 on every served row but 167/168, whose retail rows (campaign 0, continent 3, type 3 — the first cut wrote "campaign 7", the min_party value misfiled; RV-3) say 7 — recorded beside `max_party`, and the first cut's "1 everywhere" was caught by the test that re-reads the rows out of the binary, not by a reader. OBSERVED (client-table, build 38797).

*Where the numbers live.* `content/partycap.toml`, kind `map_party_cap`, **one row per served map** (every `[map.N]` in `content/maps.toml`, 19 rows; `test_henchparty` (n) refuses a served map without one), each `source = "client-table"`, `extractor = "toolkit/clientscan/areatable.py"`, `build = 38797`, `verified` naming the row and the fields — the three conditions of the 2026-08-11 ruling, two of them enforced by `content.py` and the third checked per row by the test. The authored slots (143, 144, 165–168) carry the LABEL's retail row, because the client reads its area table by the map id it was told and its panel's denominator will be that id's: on 143 every add is refused and the panel says "(1/1)" unless `--henchman-cap` raises it. `authsrv._load_map_party_caps` builds `MAP_PARTY_CAPS` at startup and refuses a row below 1 (it would refuse every add, the hero's included).

*The rule.* `henchparty.party_cap(map_caps, map_id, constant, per_map)` → `(cap, why)`: the row's `max_party` when the map has one; else the constant 4 with a `why` that SAYS "NO map_party_cap row … UNVERIFIED for this map"; `per_map=False` (`--constant-party-cap`, and `--henchman-cap N`, which turns the per-map read off as well) is the constant for every map — exactly the pre-2026-09-25 behaviour. `authsrv.party_cap(state)` wraps it and prints the resolved cap ONCE per connection (again on a zone), and both cap refusals (`handle_henchman_add`, `handle_hero_add`) resolve through it right before the check and name the `why` in their refusal line. **Why the client's number is the one to refuse at — RECONSTRUCTION** (the lane's first cut said CORROBORATED; its review, RV-5, corrected it: neither witness is a refusal): the party window's counter is the client's own reading of the same field — "(2/4)" on 148 (CONFIRM-2 §2), where row 148 reads 4 — which is the same client table counted once more (the denominator's reader is not traced), not a second lineage; and on retail's 242 (4) three adds filled 1 → 4 and nothing was attempted past it, which shows 4 reachable, not a 5th refused. Retail's refusal AT `max_party` is NOT FOUND (`henchparty.py` THE REFUSAL AT THE CAP; "The refusal at the cap" above). Which maps the server can serve at a cap other than 4: **248 (the Great Temple, 8) and 280 (the Isle, 8)** have geometry and spawns in `maps.toml`; 55 (6) has no spawn. **A hireable set on 248** — the run's precondition, which the review (RV-1) found missing: the three `outpost_henchmen` rows pin `map = 148` and `spawn_row_on_map` places a row on its own map only, so on 248 the Henchmen tab would have been empty, no `0x009F` could leave the client, and the per-map arm and its `--constant-party-cap` control would have looked identical — is `content/world.toml`'s **`outpost_henchmen_248`**: the same three templates at the 148 set's own offsets from 248's arrival, agent ids 34/35/36, each probed walkable on file 165811's compiled mesh with `place_on_mesh` moving none (the lane's scratch `rv1_mesh.py`, prediction first; `test_henchparty` §5 re-checks the three rows against the archive). Tests: `test_henchparty` (n) — 248 admits a 5th member with the cap line printed once naming the row, the same adds on 148 stop at 4, map 999 falls back to 4 saying NO row / UNVERIFIED, `--constant-party-cap` on 248 stops at 4 (the flag's KNOWN-BAD arm), `--henchman-cap 6` on 248 stops at 6, a zone 148 → 248 answers 4 then 8, the pure rule's five answers, every row's provenance, the hero add on 248 through at 5 (vaulted) and on 148 refused; §5 (vaulted) re-reads every row out of the pinned client and goes red on a mismatch in either field; `test_heroadd` §2b the hero-side five; locks on both handlers (`party_cap(state)` before the check, the constant never compared directly — the old text is the mutation), `party_cap`, `_load_map_party_caps`, `main()`'s two flags, `serverargs.py`. Owed: one client launch on 248 (the runsheet below, PLAN-LOG).

**The leave — `c2s 0x00A2 PARTY_LEAVE`, the party window's Leave; armed as RECONSTRUCTION, `--no-party-leave` reverts (desk-partycap, 2026-09-25).**
The prediction (the lane's scratch, written before the client was read): P1 the wrapper is reached through a pointer (a UI callback table or vtable), from a PyCliParty function testing the "is mine" bit; P2 the UI action is the party window's Leave, offered when the party has any member beyond the player; P3 nothing is removed locally before the send; P4 the client waits for a party-rebuilding s2c rather than row removals; P5 heroes leave too. **P1 REFUTED** — the reference is direct, a conditional tail-jump; **P2 held** (the sum below); **P3 held**; **P4 half-refuted** — the client's OWN protocol splits the leave in two sends and the second is a hero kick, so row removals are what it expects for the heroes; **P5 held**, by that second send.

*The request — OBSERVED on our client, read from the sender.* `vault/captures/gamesrv/authsrv-20260913T093718-c1.jsonl`: opcode 162 (`0x80A2`, no payload) at t=29.754946, then `0x001F [40]` at t=29.755031 — 85 µs apart, both in that run's unhandled census (`GAME_CMSG:0x00a2 1, GAME_CMSG:0x001f 2`; the other `0x001F` was `[3]`, a plain hero kick at t=27.085). 0 of 96 live connections carry `0x00A2` (c2striage). Static, build 38797: the `0xA2` wrapper `0x0085BEF0` (`mov [ebp-4], 0xa2; push 4` → the game framer, 20 bytes) has **exactly one reference in the image** — a raw any-byte scan of every `E8`/`E9`/`0F 8x` rel32 and every VA and RVA word (the lane's `refscan2.py`, scratch) finds the conditional tail-jump **`ja 0x0085BEF0` at `0x008585FD`** and nothing else; `codescan --xrefs` scanned `call`/`jmp` rel32 and data words only at the time of this read (`b50da5c8`), so its "0 direct callers" was the tool's blind spot, and the table's "(pointer-reached)" a reading of that zero (the `0xAB` wrapper `0x0085C010` is the same case: a `jne 0x85c010` at `0x0085A8C4`; the chip filed from this lane landed on `main` the same day as `b4c13a1b` — `--xrefs` now scans jcc rel32 `0F 80..8F` and labels each row call/jmp/jcc, and the `0xAB` row above names its `jne`). The jump sits in **`0x008585D0`** (asserts PyCliParty:615 `m_partyClient` at `0x008585DE` on `[this+0x50]`, the own party): `ecx = [party+0x2c] + [party+0x1c] + [party+0x0c]; cmp ecx, 1; ja <send>; ret` — the sum of the party's **heroes** (`+0x24`/`+0x2c`, the array `0x01C2`'s worker `0x00858F50` appends to), **henchmen** (`+0x14`/`+0x1c`, the stride-`0x34` array `0x01BF` fills) and **players** (`+0x04`/`+0x0c`, stride 12, the array PyCliParty:606 `player != term` walks at `0x00858570`), sent only when it exceeds 1 — a lone player's click sends nothing. It removes no row and sets no request-in-flight bit (the `0xA3` sender beside it, `0x0085A7EC`, ORs bit 1 of `[party+0x10]` after its send; this one does not). Its one caller is the thunk **`0x00856910`** (`call 0x47f660; ecx = [ctx+0x4c]+4; jmp 0x8585d0`), whose one caller is **`0x0056E958`, case 1 of the four-way switch in `0x0056E900`** (on the control's kind at `+0x34`, jump table `0x0056E998` → `0x0056E92C` / `0x0056E958` / `0x0056E96C` / `0x0056E982`; a click sound `push 0xa; call 0x633b50` first; PtJoin.cpp's two asserts, :102 `selection.memberType == MEMBER_TYPE_INVITE_IN` and :103 `selection.partyId`, sit at `0x0056ECB9`/`D1` in the same module; the handler itself is reached by its two direct calls, `0x0056E001` / `0x0056E14A` — the lane's first cut added "a vtable word at `0x0097816F`", and its review (RV-7) showed the bytes there are two ALIGNED dwords `0x000056E8` / `0x000056E9` (`e8 56 00 00 e9 56 00 00`), so the only window equal to `0x0056E900` is the unaligned one at `+3`, a coincidence and not a table; the fixer's own scan finds that one unaligned word and nothing aligned). **Case 1 then does `push 0x28; call 0x0080E2A0`** — the hero-KICK sender (ChCliApi:4459 `hero <= HEROES` at `0x0080E2AC`, `cmp esi, 0x28; jle`, and :4460 `hero != 0`; outpost-gated by `call 0x84d9b0` as `handle_hero_kick`'s docstring records; the wrapper `0x0091FF30` is `0x001F`'s): **HEROES = 40, the add's bound is `<` (:4446) and the kick's is `<=`.** The bytes (`6a28` at `0x0056E95D`, `83fe28 7e` at `0x0080E2A7`) and the wire word are OBSERVED; **that an index EQUAL to the bound means EVERY hero is RECONSTRUCTION from them, CORROBORATED by GWCA's own `KickAllHeroes() { return KickHero(0x26); }`** (UPSTREAM, MIT — `vault/mirrors/GregLando113__GWCA/Source/PartyMgr.cpp:360`, `JaborGW__GWCA` :354 — over the client's own kick sender; GWCA's `HeroID` enum ends at `ZeiRi = 37`, so its `0x26` = 38 is one past ITS last hero: the same sentinel shape at an older bound; the review's RV-6). That is the `[40]` the 2026-09-13 run carried and `overrides.json` 31 had called UNEXPLAINED (explained there now). Cases 2 and 3 fire frame-bus events `0x100001B3` / `0x100001BD` and send nothing; case 0 works a list (`[this+0x28]`). So the party window's Leave is two sends: `0x00A2 []` for the party, then `0x001F [40]` for the heroes — in an outpost; in a field only the first (the hero kick is outpost-gated), and what retail's server does with a field's leave (a return to the outpost, by the game's rule) is UNVERIFIED and on no tape. The UPSTREAM name (GWCA `PARTY_LEAVE_GROUP` at its numbering, ours + 2; Headquarter and OpenTyria at ours) is CORROBORATED by the read; OpenTyria defines the opcode and handles it nowhere.

*What the client waits for.* Nothing it can name: the sender sets no bit and removes no row, so the panel keeps every row until a reply — the kick's picture, and the 2026-09-13 run's (the words landed in the unhandled census and the party stayed). The s2c it would consume for the henchmen is the roster-remove `0x01C0` its worker `0x00858DE0` already performs for our party id (CONFIRMED on the client, CONFIRM-2 §9), and for the heroes the hero kick's own batch (OBSERVED on retail, CONFIRMED on ours); a party-rebuilding message (Headquarter names `0x01D0` PARTY_PLAYER_REMOVE, `0x01D2`/`0x01D3` the build window) is on no tape for a leave and its handlers carry the PyCliParty:1228/1238 window asserts — not sent.

*What the server sends — RECONSTRUCTION, DEFAULT ON.* `handle_party_leave` (in an outpost): every hired henchman's `0x01C0 [1, agent]` row in hire order, then ONE `0x00B0` — `henchparty.party_leave_batch`, the kick's row-then-size once for all rows; every message one the client has consumed from us for this party id. The standing NPCs keep standing and stay hireable (no `0x0021`); nothing is persisted, as the hire is not; with no hired henchman the click drops nothing here and the line says so. The heroes are the companion send's business: `handle_hero_kick`'s **`HEROES_ALL` arm** (`hid == 40`, before the owned-hero lookup, gated with the leave) kicks every party hero through the handler itself — each its OBSERVED batch, each kick persisted as a click's is — and answers `[40]` with nothing when no hero is in the party. A FIELD's leave is refused with nothing sent (the return to the outpost is not modelled); the LAUNCH henchman stays, as the kick leaves it (RV-6), and the line says so; other players there are none. `--no-party-leave` gates both arms: `0x00A2` dispatched and ignored, `[40]` ignored as an unowned index — the 2026-09-13 picture. `schema/overrides.json` names GAME_CMSG 162 PARTY_LEAVE, medium. Tests: `test_henchparty` §1l (the batch's bytes, 17 for two; the refusals), §2 (o) (the leave with a hero and two henchmen: two rows then `0x00B0 [14, 2]`, NPCs standing and hireable, the hero still in; `[40]` → hero 6's batch and `0x00B0 [14, 1]`, kicked and — under `--persist`, a real store — written; two heroes → two batches, sizes 2 then 1; a second `[40]` and a second leave send nothing; the re-hire after; the launch henchman stays and is named; a field refused; `--no-party-leave` leaves `[40]` ignored and the hero in; the leave with an opaque store touches it not at all), §4 locks (the arm and its gate, the constants 0x00A2 / 40, `main()`'s `global`, the field gate FIRST and the pop and the batch and no store, the `HEROES_ALL` arm before the owned check and gated, `serverargs.py`, the allowlist — each with a mutation that reddens it), §5 (vaulted: the four client sites' bytes and the `ja`'s target). **Owed: one client click** — the runsheet's Leave launch (PLAN-LOG): the henchman rows leaving the panel, the hero's row leaving, the counter at 1, the NPCs standing, the re-hire and the re-add; and `--no-party-leave`'s control.

**Runsheet (owner's hands or the harness, one loopback session — not run here; the questions are pre-registered).**
1. **The per-map cap on an 8-cap map.** `python toolkit/authsrv/authsrv.py --bind 127.0.0.3 --port 6112 --vault vault/captures/gamesrv --map 248 --area outpost_henchmen_248 --party slice` (the other terminals per `RUNBOOK.md`; the area is the 248 set the review's RV-1 required — `--area outpost_henchmen` places NOTHING on 248, its rows pin `map = 148`, and the log would say `3 row(s) belong to another map than 248`). PREDICTION: the server's `AREA: outpost_henchmen_248 -- 3 spawn row(s)` at startup and, on arrival, three `0x0071` marks for agents 34/35/36; the Henchmen tab lists three; the panel's counter reads **(2/8)** at load (the player and hero 3); the first add prints `[party] the party cap here is 8: map 248's own AreaInfo max_party …`; three henchman adds go THROUGH (`size now 3`, `4`, `5`; counter 5/8) where 148 refused the third. Control: relaunch with `--constant-party-cap`: the cap line says 4 and names the flag, the third add is refused (`HENCHMAN_ADD(…) refused … (4: the constant 4 for every map …)`), the panel still says (4/8) — the disagreement the flag reproduces. The arm and its control differ on the THIRD add and nowhere else; that is the reading.
2. **The leave, map 148, `--area outpost_henchmen --party slice`.** Add two henchmen (4/4). Take ONE discovery screenshot of the party window for the Leave button's position (the harness: `--shots` with a `shot:1` step after the adds; PtJoin's control sits in the party window's lower strip on retail). Click it. PREDICTION, the wire: `c2s 0x80a2` then `c2s 0x801f [40]` within a millisecond; the log: `PARTY_LEAVE: 2 hired henchman(s) dropped … size now 2`, then `HERO_KICK(40): the client's HEROES bound -- every party hero leaves, [3] …` and hero 3's own `HERO_KICK: hero 3 … size now 1` (`PERSIST: hero 3 kicked` under `--persist`) — hero 3 is `[party.slice]`'s (`content/world.toml`, `hero = 3`, Tahlkora on the (2/4) panel of CONFIRM-2 §2; the lane's first cut wrote hero 6, which is `test_henchparty`'s fixture hero — the review's RV-2). The screen: both henchman rows gone, the hero's row gone, the counter at **1/4**, the three NPCs still standing, no dialog. Then re-add a henchman (the row back, 2/4) and re-add hero 3 from Party Search (`HERO_ADD` → 3/4, as the fix pass's runsheet step 5). If the client sends `0x80a2` with NO `[40]`: record it — the outpost gate or the case is not what was read. If the panel keeps the hero's row after `[40]`: the sentinel is not "every hero" on this build's UI and the arm must come out.
3. **Control:** relaunch with `--no-party-leave`, repeat 2: `PARTY_LEAVE ignored (--no-party-leave)` and `HERO_KICK(40) ignored: 40 is the client's HEROES bound …`, nothing sent, every row stays — the 2026-09-13 picture.
4. **The field:** walk into the explorable with the party, click Leave: `PARTY_LEAVE refused: in a FIELD …`, nothing sent, the party stays; record whether the client sent `[40]` there too (the read says it does not).
