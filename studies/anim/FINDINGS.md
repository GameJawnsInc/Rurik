...
| P-N40A': every index < the local n18 | 2,823 / 3,415 — **592 files carry indices ≥ m_seqCount**. ~~Not a layout failure: the consumer resolves its current-sequence selector through the FA8 link array (`0x0078007F`), so a shell's events can be keyed by a linked model's larger sequence space.~~ **RETRACTED 2026-08-17 — the "larger sequence space" reading is WRONG.** In a 2,500-head sample **all 149 out-of-range values are exactly 65,536 = `0x10000`**, 110 of 149 terminal in the sorted array: a **sentinel above any real count**, not a cross-file index. The largest union measured anywhere in the corpus is 627. Independently supported by A1's blind replication — **0 genuine out-of-range selectors across 252/252 FA8 shells**, and the index space is **PER-FILE**, bounded by `MdlSeq:300 seqIndex < m_skel->m_seqCount`, with no fall-through to linked files. **The `0x0078007F` link-array mechanism named here is CORRECT and is now corroborated** — a byte in the sequence record selects the file (disk `u8@+0x00` → runtime `+0x04`, 1-based); only the inference about what the excess VALUES mean was wrong. Full trace: [studies/archivewrite/FINDINGS.md §8](../archivewrite/FINDINGS.md). The worm anchor (all in-bounds) is pinned in the test |
