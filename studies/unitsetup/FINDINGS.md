...
4. **Game channel up.** The server volunteers `INSTANCE_LOAD_HEAD` → player-name bracket → `INSTANCE_LOAD_INFO` (agent id 1, map id, explorable flag), and sets `state["pos"]`/`state["plane"]` from `MAP_STATIC_CONFIG` right there (`authsrv.py:5357-5412`, OBSERVED).
