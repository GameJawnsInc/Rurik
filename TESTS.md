...
=== AMENDMENT 3 of 3 — test_contentids, section 6 and the floor (builder's, unchanged) ===
  ...
  **85** with `captures/gamesrv` present; §7-§15 declare one `LEDGER.skip`
  ...
  only reads -- and requires `DebugSetProcessKillOnExit` so detaching leaves
  the operator's session alive. Floor **42**, dropping to 33 on a bare machine
  where §1 declares its skip. No client needed to run the tests. ~1 s),
