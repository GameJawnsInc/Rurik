...
=== AMENDMENT 3 of 3 — test_contentids, section 6 and the floor (builder's, unchanged) ===
  ...
  **§2e is MOVECODE-1z-ap, THE PLANE-BLIND CLIP** (FINDINGS §1z-ap): `pm.walkable()`
  ...
  0x002B and 0x0029 as two locked writes and the tick thread's 0x001E landed between
  them (seq 2723/2724/2725, 0.3 ms apart) with a STOP-ECHO's arrival due. Five
  sections: the gate's bookkeeping; `hold_tick` against a REAL condition and a second
  thread (a tick ordered after the closing destination, the bound on a pair nobody
  closes, the own-thread pair never waited on, a fake clock); `census()` on synthetic
  rows and on the crash capture itself (exactly ONE split, seq 2723) and session 4's
  (none) — the known-bad arm measured on the thing; the retail control over the LIVE
  corpus through `studies/movecode/review/stalepair_retail.py` (pairs ≥ the 2,403
  floor, splits == 0 as an INVARIANT, a zero wire gap on every pair — the same
  ...
  **JARIN-S (2026-09-14, studies/slice F40).** `toolkit/authsrv/test_mechanics.py` §7b REWRITTEN
  ...
  524); and (F40.2) that the signet raises the player at 25 % energy -- [52, me, 0.25], the
  callout a quarter of the maximum, the rate ahead of the gain, through the deferred-refill arm
  too -- while the timer's rise refills to 1.0, and a hero raised by the signet gets its rate and
  [52, hero, 0.25] on the wire (floor 524 -> 527). `test_guards.py` §6 counts the rise's flags byte. `test_morale.py`
  ...
  **SLICE-B7c (2026-09-12).** `toolkit/authsrv/test_mechanics.py` §28 (floor 162 → 174,
  ...
  asymmetry arriving), refuses a full-health party, and takes the WORST ally by fraction
  rather than the lowest id; then the tick casts, and does NOT cast with a full-health
  party, does NOT cast with an empty bar (every hero run before this), and does NOT touch
  the same body under `ALLEGIANCE_HOSTILE`. **The landing check conditions the player
  first, and that is a finding rather than a fixture detail** -- Restore Condition heals
  PER CONDITION REMOVED, so against a clean target it cures nothing and heals nothing.
  Written without the condition the check failed, correctly, which is the evidence that
  the slice needs an unconditional heal wired before a monk hero looks like one.
  ...
  *"Reapplied conditions will last the original time period, unless the reapplied
  ...
  §19, which skips by name without a vault; floor 39 → 99. **§20 is SKILLS-HN
  (2026-09-09): the heal batch on retail's wire** (`healjoin.py`, predictions
  P1–P4 in its docstring), pinned as FLOORS because the live corpus grows: ≥ 800
  property-55 events, 99%+ positive, 85%+ with no same-agent sibling outside what
  this server already sends at a cast end (measured 90.0%), ≥ 150 bare `[58, 55]`
  ...
  **0 closes under a live 479 pinned as the alarm that goes red the day the rate
  ...
  Floor 106 → 129. **§§24–26 are SKILLS-RC (2026-09-10): Restore Condition's
  mechanic and the target byte.** §24 drives `resolve_heal` on two hostiles: no
  condition on the ally → nothing removed, NOTHING healed, nothing on the wire;
  one Bleeding → one `0x0044` BEFORE one 55, 58 sent (40 landing), the ally cured;
  Bleeding + Deep Wound → two removals, `0x009F 42 = 100` giving the maximum back
  and the 20 to the pool first, then 116 sent and 70 landing; the revert heals the
  CASTER the flat 58 and leaves the Bleeding. §25 pins the client's target bytes
  on the eleven wiki-checked skills (six at 4 = other ally, five at 3 = ally, with
  Healing Signet 0 and Scourge Sacrifice 5 as controls — skips by name without
  the vault's skill rows), `cast_recipient` on every kind (other ally: the
  selected ally, never the caster, a foe or nobody; ally: else the caster; self;
  foe), `allies_of` (the player has none; a hostile's exclude itself, corpses and
  other allegiances), and the player's 276 at a foe resolving nothing. §26 runs
  `enemy_attack_tick`: a LONE hostile with 276 sends no cast, the slot stays
...
- Tests run this pass: test_deploy (92, green), test_srclint (22, green -- it lints the whole tree, so any source edit is its business), test_content (43) and test_contentids (31/2 skips) to confirm the RUN note's precondition counts. The full suite was NOT run, per the house rule.: an EMPTY set
  ...
  **SLICE-H12 (2026-09-13, floor 488 -> 502): knock-down and block.** A walking body's knock-down
  ...
  `toolkit/authsrv/test_playerswing.py` (**MOVECODE-1z-dm (2026-09-10) added §18, floor 162 → 170, and RE-AIMED §12's known-bad arm**: the player's reach geometry reads the CLIENT's frame (`_npc_frame`) instead of the position model, at all four sites — `approach_tick`'s distance, `_approach_send`'s geometry, the press row's distance and `attack_tick`'s 144 u reach gate. The **known-bad arm first** (the model 150 u from a target the body stands 20 u from: the swing is refused and the server walks the body toward it), then the shipped read (the swing opens, no follow), the row's two distances agreeing under the revert so it cannot be misread as evidence for an off arm, the three-operand `approach` row, a bare-state check — *no guard, no report, no click leg* — that exists because the first cut passed `now` to the helper at a site `attack_tick` binds it twenty lines **below**, raising `NameError` on the world tick, and the flag/revert/flags-row pin. §12's known-bad arm asserted `not rec.rows` — an EMPTY recorder — when its subject is that the SWING DROP is silent; 1z-dm's `approach` row now rides the same tick (the target is 400 u away, so the approach sends), so the assertion was re-aimed to swing/press rows and keeps its exact subject. **MOVECODE-1z-dg (2026-09-10) added §17, floor 153 → 162**: the chain pause charges the WHOLE moving span whatever the target holds, and the client's re-press after a move RESUMES the chain on that clock. §5's rig never saw the starvation because it keeps `attacking` through the move; §17's rig runs the real lifecycle on a synthetic clock — the move's report forgets the target through `cancel_on_move(moved=60)`, the body moves for 1.5 s, the stop clears the latch and `begin_attack` re-presses 40 ms later. The **known-bad arm first**: the START-to-START gap across the move is 1.80 s (the stop plus a tick — every moving tick left at `no-target` and the re-press reset the clock, 1z-dc's 19 %); shipped: 3.20 s against interval + span = 3.25 (retail's own signature, 1z-dc.3); the arms separate; the resumed swing's `chain_pause` row says `charged 1.45, left: no-target 30` where the known-bad row says `charged 0.0` on the same ticks; the re-press is ANSWERED 1.4 s later as `swing` (its row carries the wait); a press on a DIFFERENT target after the move is a retarget and swings at once; a STILL report remembers nothing (it kept the target, 1z-dd). **MOVECODE-1z-df (2026-09-10) added §16, floor 142 → 153**: the chain pause's read of the keyboard latch on displacement — RUN-1zDC's arm A charged the pause p50 1.43 s a cycle on a body with 0.0 u of tape spread, because the tap's still report armed `kbd_moving_at` and a tap into a wall sends no `0x0047`. The predicate is a REPEATED still report (`kbd_still_streak` ≥ 2, fed beside 1z-db's displacement read, reset by the stop arm): a walk-start's first report is still too, so a single one keeps charging — the naive rule would never arm the pause for a quarterstep. The **known-bad arm first** (streak 2 reads as moving; the tick refuses the open as `moving`), then the shipped read (not moving; the same tick OPENS the swing; the pause charged nothing), the single-still control, the helper's count/reset/None cases, and two source pins (fed once in the 0x003D arm, reset beside the 0x0047 arm's disarm). Retail: 4 repeated-still reports in 61 connections, 0 inter-swing gaps containing one — NOT FOUND (`stillwindup.py --streaks`). **MOVECODE-1z-dd (2026-09-10) extended §14, floor 137 → 142**: RUN-1zDB found the still report killing the swing through a SECOND door — `cancel_on_move` gated the wire STOP on displacement and then forgot the TARGET ungated, so `attack_tick` dropped the same swing as `move-ended-order` 0.02 s later, silently (leg A t=30.36: log line 396 "SUPPRESSED … keeps its windup", line 400 "SWING DROPPED … move-ended-order … lands_in 0.062"). The **known-bad arm first** (0.00 u forgets the target), then the shipped keep, a real 60 u move still forgetting it (ANIMREF-RE 39's 28 of 28 re-presses were real moves), the click arm's `moved=None` still forgetting it, and the swing a still report left alone LANDING on the next tick with no `move-ended-order` row (`hit_enemy` stubbed, the recorder read). Retail's control is NOT FOUND — 0 of 903 player windups carry a still report (`studies/movecode/review/stillwindup.py`, prediction first) — so the extension rests on 1z-db's rule and the test says so. **MOVECODE-1z-dc (2026-09-10) added §15, floor 133 → 137**: the chain pause now says how much it charged and what left early — a moving tick returning at `reach` is counted under that name, one at `dead-player` under its own, a CONTROL that a STILL tick counts nothing, and the summary riding the swing that OPENS then resetting (one row per swing, not 20 per second). The fixture carries its own lesson: a swing can only open on a tick that is NOT moving, because while the body moves the clock is frozen — a test that tried to have both was wrong. **MOVECODE-1z-db (2026-09-09) added §14, floor 128 → 133**: a keyboard report that moved NOTHING no longer cancels the auto-attack chain — the **known-bad arm first** (0.00 u still cancels, which is what 50 of our 104 chain cancels did), then the suppression at 0.00 and 0.50 u (the measured gap between the still and moving populations is EMPTY: 11.8 % of consecutive reports repeat to the decimal, 0.1 % land in 0.001–1 u), a **real 60 u move still cancelling** so the rule is visibly not weakened, and the click arm's `moved=None` control. **MOVECODE-1z-da (2026-09-09) added §13, floor 123 → 128**: the swing lifecycle pinned against ArenaNet's own — our corpus's landed/total as floors, the SILENT drop as a CEILING at 3 % (retail's own rate is 0.8 %, so 1z-cx's four whiffs were never anomalous as a rate, only in leaving no row), the CANCEL population as a floor (14.9 % against retail's 6.1 %, the arc's real divergence), and retail's half — 1,332 starts, 92.7 % landing damage, with the attacker slot MEASURED (`0x00A0` is [prop, attacker, target], 876 to 28; victim-first turns 92.7 % into 0.8 %). §13 skips by name without the gamesrv corpus, §13b without the live one. **SWINGCANCEL (2026-09-09) added §12, floor 116 → 123**: an in-flight swing DROP writes a `swing_verdict` row naming the branch and the operands it tested — the **known-bad arm runs first**, with `_swing_dropped` stubbed out, asserting the silence RUN-1zCG session 8 captured; then the `reach` drop's row with its `dist`/`reach`/`into_windup`/`lands_in`, a CONTROL that an undropped swing writes nothing, and the `dead-player` and `cancel:movement` branches named. The gap it closes: `_press_refused` returns early once its press is `answered`, and a swing in flight is by definition one whose press was answered — studies/movecode §1z-cx.) Otherwise:  (the player's auto swing is TWO
  ...
  **§3 is what the file is really guarding.** The corpus holds a
  ...
  a 50 onto 100/100 goes out as 0.5 and moves the book nowhere, a 50 onto 70/100
  sends 0.5 (the amount, not the 30 that landed), a 250 onto 100 caps at 1.0
  (`_fraction` refuses above it), and `--no-overheal-number` restores both old
  behaviours. Floor 40 → 44. Otherwise: skill damage: the client's own
