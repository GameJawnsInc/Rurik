...
`toolkit/authsrv/test_agtrack_guard.py` (**§9 is MOVECODE-1z-ah's STATIONARY WAIVER — the retract.** RUN-1zAB run A's fatal leg replayed verbatim: the guard predicted the snap 0.425 s before the arrival and was refused by the report-freshness gate ALONE, which the section pins BY NAME (`blocked_by`) because §1z-ag had to replay a capture to learn it and published two wrong explanations first. The waiver lifts that one gate when the last two accepted reports agree to within the client's own `ZERO_DIST_SQ` — a body MEASURED still, whose re-pin harm is then 1.0 u rather than `RUN_SPEED × age`. Pinned: the 1.0 u boundary COUNTS (the client's own `distSq <= 1.0`) and 1.1 u does not; a placement is not the first of the two measurements (the section's own pre-existing stale-report check caught the looser draft); a click in flight refuses; the harm on run A's leg is **0.000 u** and the 520 u arrival then never matures; and the KNOWN-BAD ARM — a walking body, reports ~512 u apart — never satisfies the waiver and never even gets a re-pin proposed, which is the warp `AgMsg.cpp` 584 makes possible and this repo shipped once. Also §9: a `0x002C` clears the keyboard leg record. **§14 is MOVECODE-1z-bn's WALK-START CLAUSE, and it narrows §9's waiver on the pair §9 never tested.** §9's known-bad arm is a body *walking* the lead, reports ~512 u apart — real, still green below, and not the arm the waiver actually fires on. A keyboard leg OPENS with a `0x003D` walk-start sitting on the previous leg's `0x0047` stop, **0.000 u apart because the body has not moved yet**, and the waiver reads that as two measurements of a still body, lifting the freshness gate 1–2.4 s into the client's own committed glide. The corpus splits totally by the pair's ORDER (§1z-bl census, harm = distance from the point we re-pinned to, to where the body was by `AgAgent::position_at`): `{0x0047 → 0x003D}` n=19, p50 **366.6 u**, 18 of 19 over 100 u; `{0x003D → 0x0047}` n=3 and `{0x003D → 0x003D}` n=1, **0.0 u**. So the clause does not overrule the waiver — it restores the waiver's own stated bound. Pinned: all four orderings with the points IDENTICAL throughout (only `{stop → walk-start}` is refused); the kinds ride the sig's third element and a 2-tuple sig (§9's own shape, and `guardretro`'s) reads as a walk-start rather than raising; an unknown previous kind is `None`, not `False`, so one report after a placement cannot trip it; a REFUSED report advances neither kind; the clause is INSIDE the waiver, so a fresh report and a non-coincident pair are untouched in both arms; and `--waiver-walkstart-stands` restores the old behaviour on the same pair, which makes the arm a real A/B. The KNOWN-BAD ARM is RUN-1zBL's leg shape at the corpus-median 1.93 s report age: clause OFF the re-pin is DUE (the defect), clause ON it is `stale-report`, and the arrival risk is PREDICTED in **both** arms — the clause moves the precondition, not the prediction. Two traps this section walks into on purpose and pins in its own comments: the flag is a module global read at CALL time, so `old, new = bl(False), bl(True)` gives both arms the LAST arm's answer (this check went green against itself once, exactly that way); and a synthetic three-walk-start leg on one point must STILL waive, because `{0x003D → 0x003D}` is a kept pair. **§14 also carries the correction that §9's fixture forced (MOVECODE-1z-bq):** that fixture INVENTS its report kinds — all three walk-starts — and the capture it names does not. RUN-1zAB run A (`authsrv-20260903T202121-c1`) carries `0x003D` / **`0x0047`** / `0x003D`, so its real pair at the risk tick is `{0x0047 → 0x003D}`, **the one this clause refuses — the waiver's own founding specimen is an instance of the defect the clause fixes**, and correctly so: the client reports 520.0 u away 3.05 s later, putting the body ~394 u downrange rather than parked. The section pins both arms of that, and the §9 fixture stays synthetic on purpose because it is the suite's only kept-pair case — what changed is that it is no longer described as the specimen. Retrodicted over the corpus by `studies/movecode/review/waiverretro.py`, whose own control is the captures' real `0x002C` fires rather than the guard's verdict rows: `stationary()` is read only by `_repin_block`, so no verdict can move when the clause moves and a verdict control here is a check that cannot fail. The rest is **the derived pre-emit grant rule,
  ...
  -- FINDINGS 1z-s). Floor 49 from the 2026-08-30 green run. ~2 s **§13 (MOVECODE-1z-bf, 2026-09-04): the gate-2 tolerance is a recorded, revertable switch** — `AGTRACK_GATE2_SEAM` ON by default and swept by `capture_flags()`, `--agtrack-gate2-exact` present and zeroing the mirror's constant (source lock), and the mirror's gate 2 reading `on_mesh` under the tolerance and `walkable()` without it (source lock). Floor 74 → 77), **§14 (MOVECODE-1z-bn) and §15 (MOVECODE-1z-bs): the stationary waiver's WALK-START clause and its NEWEST-MUST-BE-STOP clause** — the four report-pair orderings under each build (points identical throughout), each revert flag shown to be a real A/B on the same pair, RUN-1zBL's leg shape DUE with 1z-bn off and BLOCKED/stale-report with it on, the founding specimen with the kinds the capture actually carries (1z-bq), and a leg opening on a DOUBLE walk-start under a 520 u lead: DUE on a 1.46 s-old report under the 1z-bn build, BLOCKED under the shipped one and identically under a deleted waiver, while a {walk-start → stop} opening still waives. Sections 9 and 14 are interrogated with the 1z-bs clause OFF so they keep pinning the builds RUN-1zAB/1zBO/1zBP measured; every arm is built AND read under its own module flag (a global read at call time). Floor 99 → 116 from the 2026-09-05 green run),
